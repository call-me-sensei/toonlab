...
- If supported shaders or approved assets are insufficient, document the
  custom gap for developer feedback.
...
- The host app owns renderer setup, scene layout and major geometry, asset
  placement, lighting/shadows, cameras, collision, asset loading, decoder
  hosting, storage, routing, persistence, collaboration, and final art
  direction.
- Lighting, VFX, camera, game-feel, and renderer configuration are not stable
  0.4.10 package or style-bundle domains. Treat project-local implementations
  as custom adapters and record a gap when they affect the selected look.
...
- Keep artifact scope explicit: Shader Labs author IP profiles, Asset Labs
  ...
  their Generation Labs own geometry/material data. The stable 0.4.10 package
...
- In an experimental composed-world test, keep the app-owned sun/light adapter coordinated with
  Sky, Water, Weather, and vegetation scene inputs. Do not imply that 0.4.10
  ships a stable Lighting bundle slot or public Lighting runtime.
