...
1. Call `list_my_creations`, then `search_assets` with `source: 'library'`,
...
5. Local `generate_asset` creates deterministic procedural recipes. It is not
   ToonLab Pro's managed image/3D generation and does not spend cloud credits.

## ToonLab Pro remote sequence

1. Call Pro `list_my_creations` for the signed-in cloud library.
...
- Search results may show denied candidates so the developer can see what was
  considered; visibility is not approval.
- Use generation only when the policy permits it and discovery did not close
  the named gap.
- Record a custom gap before introducing custom models, textures, shaders, or
