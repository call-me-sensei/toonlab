...
## Experimental assembly boundary in 0.4.10
...
same art direction and runtime conditions. ToonLab 0.4.10 intentionally does
...
controls, physics, collision, placements, and render loop. Add only the public
systems listed in
...
When ground adoption is enabled, every manually assembled terrain/ground mesh
must set `mesh.userData.groundFieldWrite = true` before creating the environment
ground-field pass. Create and update that pass before rendering the grass, and
verify that it found at least one writer; a pass with zero writers leaves ground
coverage unavailable. Match the target's clump density, scale range, surface
...
14. **Keep gameplay VFX host-owned**: 0.4.10 has no stable checkpoint-ring or
...
| Character walks through rocks/trees | host physics has no blockers for placed assets | register tree, rock, and prop blockers in host physics or explicitly use the stable root `createWorldCollision` helper |
