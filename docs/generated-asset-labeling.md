...
This document defines the required output contract for procedural generators,
asset assembly, reconstruction, imports, and coding agents that prepare assets
for ToonLab. Geometry is not production-ready merely because it renders. It
must carry enough durable semantic information for the correct shader profile
to be selected and for that shader to address the intended parts.
...
The current generation libraries have not all migrated to this contract. Until
they do, their Lab and npm-library statuses remain **Migration required**.
...
The accepted P18 daisy is a useful comparison fixture but uses one retained
material for petal, center, and stem. It therefore cannot prove independent
controls for those roles. Additional correctly labeled validation assets are
required before the full Flower Shader contract can be approved.
...
When creating or modifying a generator, a coding agent must report:
...
9. Lab status and npm-library status independently.
