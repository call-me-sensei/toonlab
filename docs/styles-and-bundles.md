...
A future per-profile split is valid only as an explicit override state with a
visible owner and a reset-to-shared operation. A copied value that silently
drifts from the base is not a supported configuration model. Applying one
role profile must not replace another role profile, while applying its shared
snapshot intentionally updates the common base.
...
`vegetationShader` and `landscapeMaterial` remain compatibility slots for
older documents. New bundles use the independent Tree, Grass, Flower, and
Ground shader slots. The `tree`, `grass`, and `flowers` compatibility slots
predate the strict asset/style boundary; new style bundles should keep asset
recipes and species identity in asset/project data instead.
...
Today, resolving a bundle returns the settings for each populated slot. It
does not traverse a scene, classify imported assets, or apply every shader
automatically.

The host currently routes labeled roots through the appropriate runtime:
...
This is the target direction, not a claim that every existing shader or lab
...
Its Call Me Sensei starting profile is the exact editable P18 spire graph
baseline, not a generic rock approximation. In Rock Shader Lab, the surrounding
...
documents. New bundles populate `treeShader`, `grassShader`, and
`flowerShader`; they do not serialize tree recipes, grass planting data, or
flower species into those slots. The former `vegetationShader` aggregate is a
compatibility input, not the canonical new authoring surface.

The three slots resolve against the bundle's shared vegetation treatment.
