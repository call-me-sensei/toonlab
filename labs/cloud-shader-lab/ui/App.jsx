...
const SECTION_ICONS = Object.freeze({
  composition: 'stage-look',
  lighting: 'stage-look',
  motion: 'stage-animation',
  shape: 'stage-shape',
});
...
function DocumentMenu({ actions, anchor, onClose, state }) {
  ...
  async function importJson() {
    ...
    else for (const error of result.errors ?? ['Could not import the cloud shader.']) {
    ...
    <Popover anchor={anchor} onClose={onClose} title="Document" width={300}>
      <div className="gr-doc-menu">
        <div className="gr-save-row">
          <TextField onCommit={setName} placeholder="Cloud shader name…" value={name} />
          <Button
            kind="primary"
            onClick={() => {
              const result = actions.savePresetAs(name);
              if (result.ok) onClose();
              else for (const error of result.errors ?? ['Could not save the cloud shader.']) {
                toast(error, { tone: 'danger' });
              }
            }}
          >
        ...
        <Button
          ...
          onClick={() => {
            downloadBlob(
              ...
              `${state.name.replace(/\s+/g, '-').toLowerCase() || 'cloud'}.cloud-shader.json`,
              'application/json',
            );
            onClose();
          }}
...
function TopBar({ actions, state }) {
  ...
  return (
    <header className="gr-topbar tk">
      <BrandLockup labName="Cloud Shader Lab" />
      <button
        type="button"
        className="gr-title"
...
function SectionRail({ activeSection, onSectionChange }) {
  return (
    <nav className="gr-rail tk" data-testid="section-rail">
      {CLOUD_SHADER_SETTING_GROUPS.map((section) => (
        <button
          key={section.id}
          type="button"
...
function Inspector({ actions, sectionId, state }) {
  const section = CLOUD_SHADER_SETTING_GROUPS
    .find((entry) => entry.id === sectionId)
    ?? CLOUD_SHADER_SETTING_GROUPS[0];
  const localIds = new Set(state.localPresets.map((entry) => entry.id));
  return (
    <aside className="gr-inspector tk" data-testid="inspector">
      <PresetRowShell
        label="Style"
        title="Reusable cloud appearance. Weather condition and time are preview-only."
      >
        <Select
          onChange={(id) => {
          ...
          testId="cloud-style-select"
          value={state.presetId ?? ''}
        />
        {localIds.has(state.presetId) && (
          <IconButton
            icon="trash"
            label="Delete this saved cloud shader"
            onClick={() => actions.deletePreset(state.presetId)}
          />
        )}
      </PresetRowShell>
      <div className="cloud-boundary-note">
        <strong>Cloud shader only.</strong>
        {' '}These values own the P18 background-cloud and cloud-shell treatment.
        Sky color, source mesh/texture/atlas, time of day, weather, fog, and
        precipitation remain preview or runtime inputs.
      </div>
      ...
      <SchemaGroup
        fields={CLOUD_SHADER_FIELD_SCHEMA[section.id]}
...
function CloudPreviewBar({ actions, engine, state }) {
  const conditionActive = state.view.weather !== 'authored';
  return (
    <PreviewBar
      hint="Left-drag rotate · wheel zoom · right-drag pan"
      title="Cloud-readable review scene. Framing, sky context, source assets, time, weather condition, and particles are preview-only."
    >
      <SegmentedControl
        onChange={(viewMode) => actions.setView({ viewMode })}
        options={[
          { label: 'Cloud Focus', value: 'cloud' },
          { label: 'P18 Framing', value: 'horizon' },
        ]}
        testId="cloud-preview-view"
        value={state.view.viewMode}
      />
      <span
        className="gr-stagebar-select--debug"
        title="Optional world-condition stress test. It is never saved in the cloud shader."
      >
        <Select
          ...
          testId="cloud-preview-condition"
      ...
      <PreviewToggle
        ...
        testId="cloud-preview-particles"
        title="Show the selected condition's precipitation/electrical particles. Off by default so clouds remain readable."
      />
      <LabTimeOfDayControl
    ...
    <footer className="gr-status tk" data-testid="status-bar">
      <span className="gr-status-message">
        {state.status || (state.engineReady
          ? 'Live cloud shader preview ready.'
          : 'Loading cloud shader preview…')}
      </span>
      <span className="gr-status-spacer" />
      <span className="gr-status-meta">
        {CLOUD_SHADER_FIELD_COUNT} authored settings · {conditionLabel(state.view.weather)} preview
      </span>
    </footer>
  );
}
...
export function App({ engine, store }) {
  ...
  const { actions } = store;
  const [sectionId, setSectionId] = useState('composition');

  useEffect(() => {
    document.title = `${state.name} — Cloud Shader Lab`;
  }, [state.name]);
  ...
  return (
    <div className="tk">
      ...
      <CloudPreviewBar actions={actions} engine={engine} state={state} />
      <ToastStack />
    </div>
  );
}
