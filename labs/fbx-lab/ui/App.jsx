  ...
  return (
    <>
      <header className="fx-topbar">
        <span className="fx-title">FBX Editor</span>
        {state.fileName && <span className="fx-file" title={state.fileName}>{state.fileName}</span>}
        <Button onClick={() => openFbxFile(engine)}>Open FBX…</Button>
        {hasModel && (
          <>
            <SegmentedControl
              onChange={actions.setGizmoMode}
              options={GIZMO_OPTIONS}
              value={state.gizmoMode}
            />
            <Button kind="ghost" onClick={actions.frameSelection} title="Frame selection / all (F)">
              Frame
            </Button>
            {state.undoCount > 0 && (
              <Button kind="ghost" icon="undo" onClick={actions.undoDelete} title="Undo delete (⌘Z)">
                Undo delete
              </Button>
            )}
          </>
        ...
        {hasModel && (
          <>
            ...
            <Button
              ...
              title="Also handy: export the edited scene as GLB"
            >
              GLB
            </Button>
            <Button
