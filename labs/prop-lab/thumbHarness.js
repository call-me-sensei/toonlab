// Dev-only harness behind prop-lab/thumbs.html: renders ONE prop recipe
// with the real environment toon shader on a transparent canvas, framed
// for a gallery card. scripts/generate-prop-thumbs.mjs drives it per
// preset and captures WebP card art. Not part of any build input.

import * as THREE from 'three';

import { applyEnvironmentShader } from '../../src/environment/environmentMaterialAdapter.js';
import {
  PROP_TYPES, buildProp, createPropAsset, createPropSettings,
...
async function boot() {
  ...
  const settings = createPropSettings(recipe.settings);
  ...
  // Linear types render the short default run (asset.build's 3-point line);
  // point types render the hi-detail hero build.
  ...
  // Soft contact-shadow blob so props don't float on the card.
