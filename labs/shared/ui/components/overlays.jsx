    ...
    <div ref={ref} className="tk-popover" data-testid={testId} style={{ left, top, width }}>
      {title && (
        <div className="tk-popover-header">
          {title}
          <button type="button" className="tk-icon-button" aria-label="Close" onClick={onClose}>
            <Icon name="close" />
          </button>
        </div>
      )}
...
export function Modal({
  children, onClose, testId, title, width = 560,
}) {
  useEffect(() => {
    const onKey = (event) => {
      if (event.key === 'Escape') onClose();
  ...
  return (
    ...
    >
      <div className="tk-modal" data-testid={testId} style={{ width: `min(${width}px, calc(100vw - 48px))` }}>
        <div className="tk-popover-header">
