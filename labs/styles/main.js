...
import { getWeatherStyleOptions } from '../../src/weather/weatherPresets.js';
...
import { getCloudShaderPresetOptions } from '../../src/cloud/cloudShaderSettings.js';
...
const GROUPS = Object.freeze([
  { id: 'Character', jp: '人物', slots: ['toon'] },
  {
    ...
    jp: '世界',
    slots: [
      'treeShader', 'grassShader', 'flowerShader', 'tree', 'grass', 'flowers',
      'vegetationShader', 'rock', 'debris', 'groundShader',
    ],
  },
  { id: 'Atmosphere', jp: '大気', slots: ['water', 'sky', 'cloud', 'weather', 'environment'] },
...
const BUILTINS = Object.freeze({
  ...
  flowers: DEFAULT_OPTIONS,
  vegetationShader: options(getVegetationShaderPresetOptions),
  rock: options(getRockShaderPresetOptions),
  debris: DEFAULT_OPTIONS,
  water: options(getWaterStyleOptions),
  sky: options(getSkyPresetOptions),
  cloud: options(getCloudShaderPresetOptions),
  weather: options(getWeatherStyleOptions),
...
const CREATION_TYPES = Object.freeze({
  ...
  tree: ['tree-recipe'],
  grass: ['grass-preset'],
  vegetationShader: ['vegetation-shader-preset'],
  rock: ['rock-shader-preset'],
  water: ['water-preset'],
  sky: ['sky-preset'],
  cloud: ['cloud-shader-preset'],
...
function bundles() {
  return entries.filter(isBundle);
}
...
function card(bundle) {
  ...
  if (selected.length) {
    ...
    noSystems.textContent = 'No systems picked yet';
    button.append(noSystems);
  }
  const meta = document.createElement('span');
  meta.className = 'stb-card-meta';
  const updatedAt = bundle._local?.updatedAt ? new Date(bundle._local.updatedAt).toLocaleDateString() : 'local';
  meta.textContent = `${selected.length} system${selected.length === 1 ? '' : 's'} · ${updatedAt}`;
  button.append(meta);
  return button;
}
...
function renderIndex() {
  ...
  if (all.length && !shown.length) {
    ...
    message.textContent = 'Create one and pull your whole project’s look from a single portable document.';
  }
}
...
function labHrefFor(slotId) {
  return {
    ...
    tree: '/tree-lab/',
    grass: '/grass-lab/',
    vegetationShader: '/vegetation-shader-lab/',
...
function slotField(slotId, selectedPayload) {
  const slot = STYLE_BUNDLE_SLOTS[slotId];
...
function renderEditor(bundle) {
  ...
  slotGroups.replaceChildren(...GROUPS.map((group) => {
    ...
    fields.append(...group.slots.map((id) => slotField(id, bundle.slots?.[id])));
...
async function importBundle(event) {
  ...
  const imported = {
    ...parsed.value,
    id: entries.some((entry) => entry.id === parsed.value.id)
...
async function deleteBundle() {
  if (!currentBundle || !confirm(`Delete the bundle “${currentBundle.label}”?`)) return;
...
async function load() {
  ...
  const candidate = entries.find((entry) => entry.id === requestedId);
...
document.getElementById('deleteBundle').addEventListener('click', deleteBundle);
