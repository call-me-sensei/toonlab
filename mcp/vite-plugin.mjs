...
export function toonlabWorkspacePlugin({ rootDirectory = process.cwd(), workspace = '.toonlab' } = {}) {
  ...
  async function runGeneration(body) {
    ...
    if (!['ark', 'gemini', 'openai', 'tripo'].includes(provider)) {
    ...
    if (modelKinds.has(kind) && provider !== 'tripo') {
      throw Object.assign(new Error('3D generation requires Tripo.'), { statusCode: 400 });
    }
  ...
  async function advanceGenerationJob(job) {
    if (job?.provider !== 'tripo' || job.status !== 'running' || !job.result?.taskId) {
      return job;
    }
    let task;
    try {
      task = await pollTripoTask(job.result.taskId);
    } catch (error) {
      ...
      if (pollFailures >= 5) {
        return finishGenerationJob(job.id, {
          error: `Tripo polling failed repeatedly: ${error?.message ?? String(error)}`,
          result: { ...job.result, pollFailures },
        });
      }
    ...
    try {
      if (task.status === 'success') {
        ...
        if (!outputUrl) throw new Error('Tripo completed without a downloadable model');
      ...
      if (['failed', 'cancelled', 'banned', 'expired'].includes(task.status)) {
        return finishGenerationJob(job.id, {
          ...
          result: { provider: 'tripo', task, taskId: job.result.taskId },
        });
      }
      return updateGenerationJob(job.id, {
        provider: 'tripo',
      ...
      return finishGenerationJob(job.id, {
        error: `Tripo result could not be stored: ${error?.message ?? String(error)}`,
