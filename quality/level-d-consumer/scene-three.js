...
bindReload('#view', 'view', reviewView, 'overview');
...
async function main() {
  ...
  const runtime = createSceneStyleRuntime({ quality, renderer, scene, sky, water });
  const character = await createCharacterRuntime({
    animation: { roles: ['idle', 'walk'] }, parent: scene, renderer,
    styleTarget: { targetId: 'baseline/character' }, toon: false,
  ...
  const treeBarkReady = signatureBarkCount === trees.length;
  ...
  globalThis.__toonlabReview = Object.freeze({ camera, controls, runtime, scene, sky });
  document.body.dataset.toonlabSkyPreset = 'partly-cloudy';
  document.body.dataset.toonlabSkyStyle = CALL_ME_SENSEI_STYLE_BUNDLE.slots.sky.style;
  ...
  renderer.setAnimationLoop(() => {
    ...
    document.querySelector('#cloud-shadow').textContent = cloudShadowReady
      ? 'Visible clouds · shared receivers'
      : 'Blocked';
    ...
    document.querySelector('#tree-shadow').textContent = treeShadowReady
      ? `${trees.length}/${trees.length} · cast + receive`
      : `Blocked · ${coveredTreeTargets}/${eligibleTreeTargets}`;
    ...
    if (audit.ok && cloudShadowReady && treeBarkReady && treeShadowReady) {
      ...
      status.textContent = `${backend} · ${quality} · strict bundle defaults · surface/shadows/sky ready`;
    } else if (frames >= 180) {
