// Generates WebP card art for every built-in prop preset by driving
// prop-lab/thumbs.html (real environment toon shader, transparent
...
// Usage: node scripts/generate-prop-thumbs.mjs [presetIdFilter,...]
...
import { BUILT_IN_PROP_PRESETS } from '../src/propgen/propPresets.js';

const base = `${process.env.BASE_URL || 'http://localhost:5177'}/prop-lab/thumbs.html`;
const outDir = resolve(dirname(fileURLToPath(import.meta.url)), '../labs/prop-lab/ui/thumbs');
...
for (const preset of BUILT_IN_PROP_PRESETS) {
  ...
  console.log(`${preset.id.padEnd(28)} ${(webp.length / 1024).toFixed(1)} KB`);
}
