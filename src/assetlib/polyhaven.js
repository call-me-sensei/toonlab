...
// texture sets, and HDRIs, all CC0. The asset FILES are CC0 (mirror freely);
// the public API has its own ToS — identify your app, credit Poly Haven next
// to the content, don't rehost the API metadata:
//   https://github.com/Poly-Haven/Public-API/blob/master/ToS.md
// The API sends `access-control-allow-origin: *`, so this client runs in the
// browser (which identifies via the page Referer) and in Node (send
// ASSETLIB_USER_AGENT from assetRef.js).
//
...
import { ASSET_REF_KINDS, pickResolution } from './assetRef.js';
 ...
 * (the payload is ~500 KB and the API caches it for 12 h anyway). */
export function fetchPolyhavenIndex({
  ...
  if (!indexCache.has(type)) {
    const promise = (async () => {
      const response = await fetchImpl(`${POLYHAVEN_API_URL}/assets?type=${type}`, { headers });
    ...
    promise.catch(() => indexCache.delete(type));
    indexCache.set(type, promise);
  }
  return indexCache.get(type);
}

/** Raw `GET /files/{id}` document (formats × resolutions × files). */
export async function fetchPolyhavenFiles(id, { fetchImpl = fetch, headers = {} } = {}) {
  const response = await fetchImpl(`${POLYHAVEN_API_URL}/files/${encodeURIComponent(id)}`, { headers });
