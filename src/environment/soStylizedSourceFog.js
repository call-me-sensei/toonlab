// Pin-exact reconstruction of So Stylized's M_StylizedFogPP post material.
//
...
export const SO_STYLIZED_FOG_AUDIT_SHA256 =
  '46369127911617732b22b3d4fe1430ea63b647d8c23b25384310c62b9cd658dc';

export const SO_STYLIZED_FOG_MASTER_PATH =
  '/Game/SoStylized/Environment/Sky/Materials/M_StylizedFogPP.M_StylizedFogPP';

export const DEFAULT_SO_STYLIZED_FOG_VOLUME_BASE_URL =
  '/assets-local/sostylized/fog-volume';

const SUN_MOON_ATLAS =
  '/Game/SoStylized/Environment/Sky/Curves/Atlas_SunMoon.Atlas_SunMoon';

export const SO_STYLIZED_FOG_SCALAR_DEFAULTS = Object.freeze({
...
export const SO_STYLIZED_FOG_VECTOR_DEFAULTS = Object.freeze({
...
export const SO_STYLIZED_FOG_PORT_CONTRACT = Object.freeze({
  ...
  masterPath: SO_STYLIZED_FOG_MASTER_PATH,
  nodeCount: 272,
  profileCount: 9,
  ...
  remainingBridges: Object.freeze([
    'UE 256-wide curve-atlas bake versus the exported 65-sample curve rows',
    'UE volume-texture mip/compression selection versus an uncompressed base-level Data3DTexture',
    'UE AfterDOF translucency composition and exact post-pass resolution scheduling',
  ]),
});
...
function scalar(profile, name) {
  return finite(
    ...
    SO_STYLIZED_FOG_SCALAR_DEFAULTS[name] ?? 0,
  );
}
...
function vector(profile, name) {
  ...
  return Array.isArray(value) && value.length >= 3
    ? value
    : SO_STYLIZED_FOG_VECTOR_DEFAULTS[name] ?? [0, 0, 0, 1];
}
...
function ueIf(a, b, greaterValue, equalValue, lessValue) {
...
export function soStylizedFogLerpFive(values, alpha) {
  if (!Array.isArray(values) || values.length !== 5) {
    throw new Error('soStylizedFogLerpFive requires exactly five values.');
  }
...
export function resolveSoStylizedSourceFogProfile(library, reference = 'Classic') {
  if (!library) throw new Error('resolveSoStylizedSourceFogProfile requires a source library.');
  const resolved = library.resolveMaterial(resolveFogAlias(reference));
  if (!resolved || resolved.family !== 'fog') {
    throw new Error(`Unknown So Stylized fog profile: ${reference}`);
  }
  return resolved;
}

export async function loadSoStylizedFogVolumeTexture({
  baseUrl = DEFAULT_SO_STYLIZED_FOG_VOLUME_BASE_URL,
  fetchImpl = globalThis.fetch,
} = {}) {
  if (typeof fetchImpl !== 'function') {
    throw new Error('loadSoStylizedFogVolumeTexture requires fetch.');
  }
  ...
  if (manifest.schema !== 'toonlab.sostylized-fog-volume-source' || manifest.version !== 1) {
    throw new Error('Invalid So Stylized fog-volume manifest.');
  }
  const layout = manifest.layout ?? {};
  if (layout.sourceWidth !== 4096 || layout.sourceHeight !== 64
    || layout.sliceWidth !== 64 || layout.sliceHeight !== 64 || layout.depth !== 64
    || layout.axis !== 'x') {
    throw new Error('The So Stylized fog-volume strip layout is not the authored 64x64x64 layout.');
  }
  ...
  volume.userData.soStylizedSource = {
    manifest,
 ...
 * `sceneColor`; it is converted to UE CalcSceneDepth (view-space W) in cm.
 */
export function createSoStylizedSourceFogPostNode({
  library,
  ...
  if (!library || !sceneColor || !sceneDepth || !state?.uniforms) {
    throw new Error(
      'createSoStylizedSourceFogPostNode requires library, sceneColor, sceneDepth, and state.',
    );
  }
  const profile = typeof profileReference === 'object'
    ? profileReference
    : resolveSoStylizedSourceFogProfile(library, profileReference);
  ...
  for (const name of requiredUniforms) {
    if (!u[name]) throw new Error(`So Stylized fog state is missing ${name}.`);
  }

  // UE CalcSceneDepth returns clip-space W: linear distance along view Z in
  ...
  // ComponentMask_2 is the pin-exact UE B/Z channel. UE Z maps to Three Y.
  ...
  const shiftedAtmosphere = vec4(
    ueHueShift(atmosphereColor.rgb, atmosphereHueAmount),
  ...
  if (switchValue(profile, '3DTexturedFog?', true)) {
    ...
    if (volumeTexture) {
      // Noise coordinates are authored in UE axes/centimeters. Convert Three
      // (X,Y,Z) back to UE (X,-Z,Y) before dividing by the cm parameters.
      const cameraUeCm = vec3(
        cameraPosition.x,
        cameraPosition.z.negate(),
        cameraPosition.y,
      ).mul(100);
      const pixelUeCm = vec3(
        worldPosition.x,
        worldPosition.z.negate(),
        worldPosition.y,
      ).mul(100);
      const towardPixelUe = normalize(pixelUeCm.sub(cameraUeCm));
      const angle = u.windAngle.mul(Math.PI * 2);
      const timeOffset = vec3(cos(angle), sin(angle).negate(), 1)
        .mul(u.time)
        .mul(0.03);
      const nearPoint = cameraUeCm.add(
        towardPixelUe.mul(scalar(profile, '3D Fog Distance')),
      );
      ...
      if (switchValue(profile, 'DualLayer3DFog?', true)) {
        const farPoint = cameraUeCm.add(
          towardPixelUe.mul(scalar(profile, '3D Fog Distance') * 1.9),
        );
  ...
  const sunCurveTime = ueIf(
    currentTime,
    dayLength,
    ueIf(nightProgress, 0.5, 0, 0, 1),
    0,
    dayTimeProgress,
  );
  const moonCurveTime = ueIf(
    currentTime,
    dayLength,
    nightProgress,
    0,
    ueIf(dayTimeProgress, 0.5, 0, 0, 1),
  );
  ...
  const shiftedMoon = vec4(
    ueHueShift(moonCurve.rgb, remap(
      moonDistance,
      0,
      1,
  ...
  return Object.freeze({
    bridges: Object.freeze([...SO_STYLIZED_FOG_PORT_CONTRACT.remainingBridges]),
    contract: SO_STYLIZED_FOG_PORT_CONTRACT,
    // UE's material root consumes the graph as Emissive Color (float3). Give
