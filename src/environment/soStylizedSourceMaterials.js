// Family-specific WebGPU reconstructions of the licensed So Stylized source
...
import {
  DEFAULT_SO_STYLIZED_SOURCE_BASE_URL,
  SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT,
  soStylizedScalar as scalar,
  soStylizedSwitch as switchValue,
  soStylizedTexturePath as texturePath,
  soStylizedVector as vector,
} from './soStylizedSourceLibrary.js';
import {
  loadSoStylizedUnityRockMaterialIndex,
  resolveSoStylizedUnityRockMaterial,
} from './soStylizedUnityRockMaterialResolver.js';
import {
  buildSoStylizedUnityGrassMaterial,
  isSoStylizedUnityGrassProfile,
  soStylizedUnityGrassCastsShadow,
} from './soStylizedUnityEnvironmentMaterials.js';
import {
  buildSoStylizedUnityPineBarkMaterial,
  buildSoStylizedUnityPineLeavesMaterial,
  isSoStylizedUnityPineBarkProfile,
  isSoStylizedUnityPineLeavesProfile,
} from './soStylizedUnityTreeMaterials.js';
import {
  installSoStylizedUnityUrpLighting,
} from './soStylizedUnityUrpLighting.js';
import {
...
import { installUeSourceSubsurfaceLighting } from './ueSourceSubsurfaceLighting.js';
import { ueSourceDitherTemporalAA } from './ueSourceTemporal.js';
...
const LANDSCAPE_FUNCTION_TEXTURES = Object.freeze({
  autoCliffNoise: '/Game/SoStylized/Textures/Noise/T_NoiseStylized.T_NoiseStylized',
  desertDirtNormal: '/Game/SoStylized/Environment/Landscape/Textures/T_DesertDirt_Rocks_N.T_DesertDirt_Rocks_N',
  desertSandNormal: '/Game/SoStylized/Environment/Landscape/Textures/T_DesertSand_N.T_DesertSand_N',
  desertSandRoughness: '/Game/SoStylized/Textures/Noise/T_ChromaNoise_Bilinear.T_ChromaNoise_Bilinear',
  desertSandVariance: '/Game/SoStylized/Textures/Noise/T_NoiseRough_HighContrast.T_NoiseRough_HighContrast',
  dirtNormal: '/Game/SoStylized/Environment/Landscape/Textures/T_Dirt1_N.T_Dirt1_N',
  dirtRoughness: '/Game/SoStylized/Environment/Landscape/Textures/T_Dirt1_R.T_Dirt1_R',
  grassVariance: '/Game/SoStylized/Textures/Noise/T_NoiseRough.T_NoiseRough',
  sandNormal: '/Game/SoStylized/Environment/Landscape/Textures/T_Sand_N.T_Sand_N',
  sandRoughness: '/Game/SoStylized/Environment/Landscape/Textures/T_Sand_R.T_Sand_R',
  sparkleChroma: '/Game/SoStylized/Textures/Noise/T_ChromaNoise2x_Nearest.T_ChromaNoise2x_Nearest',
  sparkleMask: '/Game/SoStylized/Textures/Masks/T_SphereMask.T_SphereMask',
  snowSpecular: '/Game/SoStylized/Textures/Noise/T_ChromaNoise_Blurred.T_ChromaNoise_Blurred',
});

const CLOUD_LITE_TEXTURE =
  '/Game/SoStylized/Environment/Sky/Textures/CloudLayers/T_CloudLayer03.T_CloudLayer03';
 ...
 * reinstalled here. Materials built through this module run in the UE-authored
 * source stage; Unity-derived graphs therefore require the UE-to-URP Lambert
 * energy / captured-SH boundary rather than the PI-premultiplied Unity-stage
 * boundary.
 */
export function rehydrateSoStylizedSourceMaterialLighting(material) {
  if (!material?.isNodeMaterial) return material;
  const defaultLitLighting = material.userData?.ueSourceDefaultLitLighting;
  ...
  material.userData.soStylizedSourceLightingClone = {
    ...
    sourceStageInputAdapter: unityLighting ? 'ue-captured-scene-sh' : null,
  };
  return material;
}

// UE materials use shared sampler sources extensively. The target WebGPU
...
// texture. Keep every UE texture and coordinate input, but reproduce bilinear
...
function sourceTextureSample(map, coordinates, {
  ...
  // WebGPU creates an sRGB texture format for exported UE textures whose
...
function sourceSceneProfile(profile, sourceAssetName, sourceSceneVariant = null) {
  if (sourceAssetName !== SNOWPINES_SOURCE_ASSET || !profile) return profile;
  // Unreal's glTF exporter serializes the static-mesh slot name (`MI_Grass`),
  // while the authored SnowPines level overrides that slot with the NoRVT
  ...
  // black fallback.
  const isSnowPinesGrass = profile.family === 'foliage'
    && /MI_Grass(?:_NoRVT)?(?:_LOD[12])?\./.test(profile.path);
  if (!isSnowPinesGrass) return profile;
  ...
  // in the SnowPines level uses the NoRVT colormap override below.
...
function unrealDirection(values, fallback = [0, 0, 1]) {
...
function sourceInstanceIdentity(profile, object = null) {
  // Unreal's glTF export expands foliage instances into separate nodes and
  ...
  const ueX = world.x * 100;
  const ueY = -world.z * 100;
  const key = `${profile?.path ?? ''}|${object?.name ?? ''}|${ueX.toFixed(4)}|${ueY.toFixed(4)}`;
  return {
    actorUE: [ueX, ueY],
...
function ueHueShift(colorNode, amountNode) {
  // UE 5.8's Engine HueShift material function is RotateAboutAxis around
...
function sourceDesaturate(colorNode, fractionNode) {
  // SnowPines has r.LegacyLuminanceFactors=1, so UE's Desaturation uses the
...
function sourceHueShiftAmount(profile, object = null, sourceActorIdentity = null) {
  ...
  const [ueX, ueY] = identity.actorUE;
  const rawSeed = identity.random + ueX * 0.713145 + ueY * 0.713145;
...
function sourceHueVariance(colorNode, profile, sourceActorIdentity = null) {
  ...
  return ueHueShift(colorNode, hueAmount);
}
...
function sourceTemporalDither(amountNode, state) {
  return ueSourceDitherTemporalAA(clamp(amountNode, 0, 1), state);
}

function sourceUeWorldXY(worldOffsetUeMeters = null) {
  if (worldOffsetUeMeters) {
    // The parity contract uses canonical metres (x,y,z), which Three renders
    // as (x,y,-z). Unreal receives the canonical basis as (z,x,y), so the
    // retained Landscape horizontal basis is UE X/Y = (-Three Z, Three X).
    return vec2(positionWorld.z.negate(), positionWorld.x).add(vec2(
      Number(worldOffsetUeMeters[0]) || 0,
      Number(worldOffsetUeMeters[1]) || 0,
    ));
  }
  // Ordinary glTF source meshes retain UE (X,Y) -> Three (X,-Z).
  return vec2(positionWorld.x, positionWorld.z.negate());
}

function planarAtUeWorldXY(map, sourceWorld, scaleMeters, offset = null) {
...
function planar(map, scaleMeters, offset = null) {
  return planarAtUeWorldXY(map, sourceUeWorldXY(), scaleMeters, offset);
}
...
function planarCentered(
  ...
  offsetX = 0,
  offsetY = 0,
  worldOffsetUeMeters = null,
) {
  const scale = vec2(
    Math.max(Number(scaleXMeters) || 1, 0.001),
    Math.max(Number(scaleYMeters) || 1, 0.001),
  );
  const sourceWorld = sourceUeWorldXY(worldOffsetUeMeters);
...
function landscapeLayerCoordinates(
  ...
  const sourceContract = sourceAssetName === SNOWPINES_SOURCE_ASSET
    ? SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT
    : null;
  ...
  // Canonical metres map to Unreal centimetres as (z,x,y). Three renders
  // canonical Z with the opposite sign, therefore the retained Landscape
  // horizontal basis is UE X/Y = (-Three Z, Three X).
  const ueWorldMeters = sourceUeWorldXY(worldOffsetUeMeters);
  const ueWorldCm = ueWorldMeters.mul(100);
  return ueWorldCm
    .sub(vec2(originCm[0], originCm[1]))
    .div(vec2(quadScaleCm[0], quadScaleCm[1]));
}

function landscapeWindColor(profile, state, maps, sourceWorld = sourceUeWorldXY()) {
...
function sourceProfileCastsShadow(profile) {
  // Unity's grass prefabs explicitly disable cast shadows on every LOD. Other
  // families retain their scene/component shadow metadata.
  return soStylizedUnityGrassCastsShadow(profile);
}
...
function unpackSourceNormal(sampleNode) {
  // Unreal normal textures use the DirectX (+Y) convention. Source PNGs are
...
function configureSourceSubsurface(material, colorNode, opacityNode, {
  ...
  // Keep the Unreal graph inputs exact and isolate renderer translation here.
  // The thickness fields remain populated for graph inspection/backward
  // compatibility, but the installed lighting model evaluates UE 5.8's
  ...
  return installUeSourceSubsurfaceLighting(material, {
...
function foliageWindColor(profile, state, maps) {
  ...
  const windSize = Math.max(scalar(profile, 'Wind Size', 8000) / 100, 0.001);
  const world = sourceUeWorldXY(state.userData?.worldOffsetUeMeters).div(windSize);
...
function finalizeMaterial(material, profile, family) {
  ...
  // The supplied sky and cloud domes are ordinary Unreal surfaces with
  ...
  material.userData.soStylizedSource = {
    family,
...
export function resolveSoStylizedShaderSwipeBaseline(material) {
...
function recordSourceContract(material, contract) {
  material.userData.soStylizedSource.contract = contract;
  return material;
}
...
export function createSoStylizedSourceEnvironmentState(library, overrides = {}) {
...
export function updateSoStylizedSourceEnvironmentState(state, values = {}) {
...
export function advanceSoStylizedSourceEnvironmentState(state, delta = 0) {
...
async function buildLeaves(profile, context) {
  ...
  if (retainedPineLeafMask) {
    // T_Leaf_Pine inherits TEXTUREGROUP_World. UE 5.8's active device
  ...
  if (switchValue(profile, 'UseGradient?', true)) {
    ...
    // UE's Lerp is A * (1 - Alpha) + B * Alpha, so preserving the authored
  ...
  // enabled. UE 5.8 then multiplies the transformed world normal by
  ...
  // PerInstanceFadeAmount=1 and MI_PineLeaves Opacity Multiply=1. UE displays
  ...
  // Unreal ShadowReplace keeps the source alpha silhouette for shadow maps.
  ...
  return recordSourceContract(finalizeMaterial(material, profile, 'leaves'), {
    ...
    subsurfaceLighting: 'UE 5.8 direct transmission + front/back captured-SkyLight SH',
    ...
    temporalDither: analyticallyResolvedFullVisibility
      ...
      : 'UE DitherTemporalAA(Opacity Multiply); PerInstanceFadeAmount fixed to 1',
    twoSidedNormal: switchValue(profile, 'TwoSidedNormals?', true)
      ...
      : 'M_Leaves +Z, then UE material-boundary TwoSidedSign; net face-corrected geometric normal',
    ueMaterialBoundaryNormal: 'MaterialTemplate.ush: Parameters.WorldNormal *= Parameters.TwoSidedSign',
    ueMaterialTemplateSha256: '2d237cc8c53a024341a6a3828a251a655fbc9a266c0a2d7ed7e244be90bf292d',
    vertexColor: hasVertexColors ? 'r:wpo-weight' : 'absent',
  });
}

async function buildFoliage(profile, context) {
  if (isSoStylizedUnityGrassProfile(profile)
    && context.sourceAssetName !== SNOWPINES_SOURCE_ASSET) {
    const material = await buildSoStylizedUnityGrassMaterial(profile, context);
    finalizeMaterial(material, profile, 'foliage');
    Object.assign(material.userData.soStylizedSource, {
      reconstruction: 'unity-s-foliage',
      sourceEngine: 'Unity 6000.5 / URP 17.5',
      sourceMaterial: material.userData.soStylizedUnityGrass?.sourceMaterial ?? 'MV_Grass',
      sourceShaderGraph: 'S_FoliageShader',
    });
    return recordSourceContract(material, {
      alpha: 'Unity ordered 4x4 dither * radial 80-100m fade',
      alphaClip: 0.9,
      baseColor: 'lerp(MV_Grass.Bottom, noise-gradient/tip-distance, UV0.y)',
      castShadow: false,
      emission: 'BaseColor * 0.03',
      gradientUv: 0,
      lighting: 'URP Lit specular-workflow adapter',
      roughness: '1 - Unity Smoothness(.05) = .95',
      source: `${material.userData.soStylizedUnityGrass?.sourceMaterial ?? 'MV_Grass'} `
        + '+ MV_Grass + M_Foliage + compiled S_FoliageShader',
      specular: '[.17273237,.511,.057577446]',
      vertex: 'deterministic Unity GradientNoise wind + .2m world-Y vertex-color lift',
      vertexColor: context.hasVertexColors ? 'rgb:wpo-mask' : 'absent/fallback-one',
    });
  }
  const { library, state, hasVertexColors } = context;
  const retainedDaisyMask = context.sourceAssetName === SNOWPINES_SOURCE_ASSET
  ...
  if (useTexture) {
    ...
    if (switchValue(profile, 'UseRVTColor?', false)) {
      // UE's RVT sample is only authoritative where the landscape writer has
      ...
      rootColor = planarCentered(
        ...
        0,
        0,
        state.userData?.worldOffsetUeMeters,
      )
        .rgb
    ...
    if (!tipColor) {
      ...
      tipColor = ueHueShift(tipColor, float(scalar(profile, 'Tip Hue Shift', -0.06)));
    }
  ...
  if (maps['Height Texture']) {
    const heightNoise = planarAtUeWorldXY(
      maps['Height Texture'],
      sourceUeWorldXY(state.userData?.worldOffsetUeMeters),
  ...
  if (hasVertexColors) {
    // UE multiplies XYZ WPO by COLOR_0.rgb before coordinate conversion.
  ...
  return recordSourceContract(finalizeMaterial(material, profile, 'foliage'), {
    ...
    temporalDither: retainedDaisyMask
      ...
      : 'UE DitherTemporalAA(PerInstanceFadeAmount); input fixed to 1',
  ...
  if (switchValue(profile, 'Moss?', false) && maps.MossTexture) {
    ...
    const directionMask = switchValue(profile, 'WorldAligned?', true)
      ? clamp(
        dot(
          normalWorld,
          unrealDirection(vector(profile, 'MossDirection', [0, 0, 1])),
        )
  ...
  if (switchValue(profile, 'Snow?', false) && maps['Snow Texture']) {
    ...
    snowMask = switchValue(profile, 'SnowWorldAligned?', true)
      ? clamp(
        dot(
          normalWorld,
          unrealDirection(vector(profile, 'Snow Direction', [0, 0, 1])),
        )
...
async function buildSnow(profile, context) {
  ...
  return recordSourceContract(finalizeMaterial(material, profile, 'snow'), {
    colorProjection: 'absolute-ue-world-xy',
...
async function buildLandscape(profile, context) {
  ...
  const isSnowPines = context.sourceAssetName === SNOWPINES_SOURCE_ASSET;
  // SnowPines is an Unreal-authored Landscape with ten painted weight layers,
  // AutoCliff, height blending, rain wetness, and scene-specific textures.
  // Unity Terrain/Lit belongs to M_Demonstration_Mega and cannot be combined
  // with these masks for an apple-to-apple scene comparison.
  const unityTerrainLayers = null;
  if (unityTerrainLayers) {
    for (const [layerName, layer] of Object.entries(unityTerrainLayers)) {
      maps[`unityTerrain${layerName}Diffuse`] = layer.diffuseMap;
      if (layer.normalMap) maps[`unityTerrain${layerName}Normal`] = layer.normalMap;
    }
  }
  let landscapeWeightSet = null;
  if (isSnowPines) {
    if (typeof library.loadLandscapeWeightmapTextures !== 'function') {
      throw new Error('SnowPines requires the authoritative Landscape weightmap loader.');
    }
  ...
  const sourceWorldXY = sourceUeWorldXY(state.userData?.worldOffsetUeMeters);

  const grassScale = scalar(profile, 'Global Scale', 1600) / 100;
  const rockScale = scalar(profile, 'Rock Scale', 2500) / 100;
  const grassPrimary = planarAtUeWorldXY(
    maps['Grass Color Texture'],
    sourceWorldXY,
    grassScale,
  ).rgb;
  const grassSecondary = planarAtUeWorldXY(
    maps['Grass Color Texture'],
    sourceWorldXY,
    grassScale * 1.75,
  ).rgb;
  ...
  if (maps.grassVariance) {
    grassVariance = clamp(planarAtUeWorldXY(
      maps.grassVariance,
      sourceWorldXY,
  ...
  if (maps['Hue Texture']) {
    const hueNoise = planarAtUeWorldXY(
      maps['Hue Texture'],
      sourceWorldXY,
    ...
    grassColor = ueHueShift(grassColor, hueAmount);
  }
  ...
  let cliffMask = switchValue(profile, 'AutoCliff?', true)
    ...
    : float(0);
  // AutoGrass drives the procedural grass/RVT output in Unreal. It does not
  ...
  if (weightBindings) {
    ...
    if (unityTerrainLayers) {
      const unitySurface = (layerName) => {
        const layer = unityTerrainLayers[layerName];
        const layerUv = sourceWorldXY.div(Math.max(layer.tileSize, 0.001));
        const diffuseMap = maps[`unityTerrain${layerName}Diffuse`];
        const normalMap = maps[`unityTerrain${layerName}Normal`];
        return {
          ...
          // URP Terrain/Lit uses the standard dielectric F0 before its metallic
          // blend; Three's physical material reaches that with intensity 1.
          specular: float(1),
        };
      };
      const exact = Object.fromEntries(
        Object.keys(unityTerrainLayers).map((layerName) => [
          layerName,
          unitySurface(layerName),
        ]),
      );
      surfaces = {
        ...exact,
    ...
    const weightSize = vec2(
      SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.width,
      SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.height,
    );
    const weightUv = clamp(
      landscapeCoord.add(0.5).div(weightSize),
      vec2(
        0.5 / SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.width,
        0.5 / SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.height,
      ),
      vec2(
        (SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.width - 0.5)
          / SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.width,
        (SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.height - 0.5)
          / SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.height,
      ),
    );
    ...
    for (const layerName of SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.layers) {
      ...
      modifiedWeights[layerName] = !unityTerrainLayers
        && SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.heightBlendLayers.includes(layerName)
    ...
    paintedWeights = Object.fromEntries(
      SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.layers.map((layerName) => [
        layerName,
  ...
  let normalNode;
  if (unityTerrainLayers) {
    // Unity's supplied landscape is stock URP Terrain/Lit. It has no Unreal
  ...
  return recordSourceContract(finalizeMaterial(material, profile, 'landscape'), {
    autoCliff: unityTerrainLayers
      ? 'disabled; absent from Unity URP Terrain/Lit'
      : 'saturate(remap(VertexNormalWS.z,.85,.8)-noise)',
    autoGrassAffectsSurface: false,
    inferredSnow: false,
    layerBlendOrder: unityTerrainLayers
      ? 'Unity Terrain/Lit normalized splat weights'
    ...
    weightmaps: {
      ...
      blend: weightBindings
        ? (unityTerrainLayers
          ? 'Unity Terrain/Lit normalized splat weights; no mask/height maps'
      ...
      heightBlendLayers: [...SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.heightBlendLayers],
      requiredLayers: [...SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.layers],
      ...
      textureNames: weightPackTextures
        ...
        : [],
      weightBlendLayers: [...SO_STYLIZED_SNOWPINES_WEIGHTMAP_CONTRACT.weightBlendLayers],
...
async function buildSky(profile, context) {
  ...
  return recordSourceContract(finalizeMaterial(material, profile, 'sky'), {
    ...
    shadingModel: 'unlit',
    sidedness: 'front',
    temporalDither: 'UE exact graph, Good64x64TilingNoiseHighFreq, Random=1',
  });
}
 ...
 * shell instances.
 *
 * Unlike Unity's S_StylizedBasic counterpart, Unreal consumes the roughness
...
async function buildStylizedBasic(profile, context) {
  ...
  material.colorNode = colorNode;
  material.metalnessNode = clamp(metalnessNode, 0, 1);
  // M_StylizedBasic outputs UE roughness directly. Do not apply the
  // smoothness inversion used by Unity's S_StylizedBasic port.
  ...
  return recordSourceContract(finalizeMaterial(
    ...
    roughness: 'RoughnessMap? texture.r : Roughness; direct UE roughness',
    shadingModel: 'MSM_DEFAULT_LIT',
    sidedness: 'front',
    specular: 'SpecularMap? texture.r : Specular',
    sourceGraph: '/Game/SoStylized/Materials/M_StylizedBasic',
 ...
 */
export async function createSoStylizedBasicMaterialFromPbr(
  sourceMaterial,
  {
    library,
  ...
  if (!library) {
    throw new Error('createSoStylizedBasicMaterialFromPbr requires a source library.');
  }
  ...
  const material = await buildStylizedBasic(profile, {
    ...
    state: state ?? createSoStylizedSourceEnvironmentState(library),
  });
  ...
  material.userData.soStylizedSource.reconstruction =
    'imported-pbr-input-through-M_StylizedBasic';
  material.userData.soStylizedSource.contract.inputAuthority = {
...
async function buildUnityRock(profile, context) {
  if (!unityRockMaterialIndexPromise) {
    unityRockMaterialIndexPromise = loadSoStylizedUnityRockMaterialIndex()
      .catch((error) => {
        unityRockMaterialIndexPromise = null;
        throw error;
      });
  }
  const index = await unityRockMaterialIndexPromise;
  const resolution = resolveSoStylizedUnityRockMaterial(profile, {
    allowFallback: true,
    index,
    sourceAssetName: context.sourceAssetName,
  });
  if (!resolution?.materialRecord) {
    throw new Error(`No Unity S_Rock profile matches ${profile.path}.`);
  }
  const material = await loadUnityRockMaterial({
    ...
    coordinates: {
      zSign: 1,
      // Both the Unity material parameters and the glTF/Three scene are in
      // metres. These values are intentionally not converted from UE units:
      // Unity's authored 500/15000/20000 distance thresholds must remain
  ...
  Object.assign(material.userData.soStylizedSource, {
    reconstruction: 'unity-s-rock',
    unityMaterial: resolution.unityMaterialName,
    unityProfileId: resolution.profileId,
    unityMatchKind: resolution.matchKind,
    unityExactProfile: resolution.isExact,
    sourceDistanceScale: 1,
  });
  return recordSourceContract(material, {
    alpha: 'opaque',
    baseColor: 'Unity S_Rock Shader Graph',
    distance: 'radial world distance; Unity metres preserved 1:1',
    layers: 'Grass -> Snow -> Sand sequential SG_SubLayer chain',
    lighting: 'URP Lit metallic-workflow adapter',
    normal: 'ShaderGraph triplanar + distance flatten + NormalBlend',
    sourceEngine: 'Unity 6000.5 / URP 17.5',
    sourceMaterial: resolution.unityMaterialName,
    sourceProfileExact: resolution.isExact,
  });
}

async function buildUnityMountain(profile, context) {
  if (!unityRockMaterialIndexPromise) {
    unityRockMaterialIndexPromise = loadSoStylizedUnityRockMaterialIndex()
      .catch((error) => {
        unityRockMaterialIndexPromise = null;
  ...
  if (!resolution?.materialRecord) {
    throw new Error(`No Unity S_Mountain profile matches ${profile.path}.`);
  }
  const material = await loadUnityMountainMaterial({
    ...
    coordinates: {
      zSign: 1,
      // This showcase geometry came through UE's glTF exporter. Its UV V is
      // the inverse of the supplied Unity FBX, which matters only where the
  ...
  Object.assign(material.userData.soStylizedSource, {
    reconstruction: 'unity-s-mountain',
    unityMaterial: resolution.unityMaterialName,
    unityProfileId: resolution.profileId,
    unityMatchKind: resolution.matchKind,
    unityExactProfile: resolution.isExact,
  ...
  return recordSourceContract(material, {
    baseColor: 'Unity S_Mountain Shader Graph',
...
async function buildUnityPineLeaves(profile, context) {
  const result = await buildSoStylizedUnityPineLeavesMaterial(profile, context);
  finalizeMaterial(result.material, profile, 'leaves');
  Object.assign(result.material.userData.soStylizedSource, {
    reconstruction: result.reconstruction,
    unityExactProfile: true,
    unitySourceGraph: result.sourceGraph,
    unitySourceMaterial: result.sourceMaterial,
  });
  return recordSourceContract(result.material, result.contract);
}

async function buildUnityPineBark(profile, context) {
  const result = await buildSoStylizedUnityPineBarkMaterial(profile, context);
...
async function buildTemplate(profile, context) {
  ...
  switch (profile.family) {
    case 'rock':
      // Unity is the portable shader authority for the rock family. Resolve
      // the exact MV_* counterpart of every imported MI_* slot and execute the
      // supplied S_Rock graph/property chain. The literal UE reconstruction
      // remains available in Rock Lab for forensic A/B checks, but is not the
      // production showcase binding.
      return buildUnityRock(profile, context);
    case 'mountain':
      return buildUnityMountain(profile, context);
    case 'leaves':
      return isSoStylizedUnityPineLeavesProfile(profile)
        && context.sourceAssetName !== SNOWPINES_SOURCE_ASSET
        ? buildUnityPineLeaves(profile, context)
        : buildLeaves(profile, context);
    case 'foliage': return buildFoliage(profile, context);
    case 'bark':
      return isSoStylizedUnityPineBarkProfile(profile)
        && context.sourceAssetName !== SNOWPINES_SOURCE_ASSET
        ? buildUnityPineBark(profile, context)
...
export async function createSoStylizedSourceMaterial(profileReference, {
  hasUv2 = false,
  hasVertexColors = false,
  library,
  sourceActorIdentity = null,
  ...
  if (!library) throw new Error('createSoStylizedSourceMaterial requires a source library.');
  const profile = sourceSceneProfile(
    library.resolveMaterial(profileReference),
    sourceAssetName,
    sourceSceneVariant,
  );
  if (!profile) throw new Error(`Unknown So Stylized material profile: ${profileReference}`);
  const environmentState = state ?? createSoStylizedSourceEnvironmentState(library);
...
export async function applySoStylizedSourceMaterials(root, {
  library,
  sourceActorIdentity = null,
  sourceAssetName,
  sourceSceneVariant = null,
  ...
  if (!library) throw new Error('applySoStylizedSourceMaterials requires a source library.');
  if (!sourceAssetName) throw new Error('applySoStylizedSourceMaterials requires sourceAssetName.');
  const environmentState = state ?? createSoStylizedSourceEnvironmentState(library);
  const jobs = [];
  const unresolved = [];
  let materialCount = 0;
  ...
  root.traverse((object) => {
    ...
    const materialJobs = originals.map((original, materialIndex) => {
      ...
      return createSoStylizedSourceMaterial(resolved.profile, {
    ...
    jobs.push(Promise.all(materialJobs).then(() => {
      ...
      object.userData.soStylizedSourceAsset = sourceAssetName;
    }));
  });
...
export async function applySoStylizedNamedSourceMaterials(root, {
  library,
  sourceActorIdentity = null,
  ...
  if (!library) throw new Error('applySoStylizedNamedSourceMaterials requires a source library.');
  const environmentState = state ?? createSoStylizedSourceEnvironmentState(library);
  ...
  root.traverse((object) => {
    ...
    jobs.push(Promise.all(originals.map(async (original, materialIndex) => {
      ...
      if (!materials.has(key)) {
        materials.set(key, createSoStylizedSourceMaterial(profile, {
          hasUv2,
          hasVertexColors,
          library,
          sourceActorIdentity,
...
/**
 * Repairs only the explicit magenta failure sentinels emitted by Unreal's
 ...
 */
export async function repairSoStylizedAuthoredBakeMaterials(root, {
  library,
  sourceAssetName = 'authored-scene',
  state = null,
} = {}) {
  if (!library) throw new Error('repairSoStylizedAuthoredBakeMaterials requires a source library.');
  const environmentState = state ?? createSoStylizedSourceEnvironmentState(library);
  ...
  root.traverse((object) => {
    ...
    jobs.push(Promise.all(originals.map(async (original, materialIndex) => {
      ...
      if (!liveMaterials.has(key)) {
        liveMaterials.set(key, createSoStylizedSourceMaterial(profile, {
          hasUv2,
          hasVertexColors,
          library,
          sourceAssetName,
...
export const SO_STYLIZED_SOURCE_MATERIAL_BASE_URL =
  DEFAULT_SO_STYLIZED_SOURCE_BASE_URL;
