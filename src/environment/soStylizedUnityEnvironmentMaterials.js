// Unity-authoritative So Stylized foliage and Terrain/Lit inputs.
//
// Values below are taken from MV_Grass + M_Foliage and the eight TL_*.terrainlayer
// assets in the supplied Unity 6000.5 project. Licensed textures are populated
// by scripts/unity/extract-environment-baseline.mjs under assets-local/.
...
export const DEFAULT_SO_STYLIZED_UNITY_ENVIRONMENT_BASE_URL =
  '/assets-local/sostylized-unity/environment-baseline';

export const SO_STYLIZED_UNITY_GRASS = Object.freeze({
...
export const SO_STYLIZED_UNITY_GRASS_VARIANTS = Object.freeze({
  grass: Object.freeze({
    sourceMaterial: 'MV_Grass',
  }),
  snow: Object.freeze({
...
export const SO_STYLIZED_UNITY_TERRAIN_LAYERS = Object.freeze({
...
function loadUnityEnvironmentTexture(name, {
  baseUrl = DEFAULT_SO_STYLIZED_UNITY_ENVIRONMENT_BASE_URL,
  normal = false,
} = {}) {
  ...
  if (!textureCache.has(cacheKey)) {
    textureCache.set(cacheKey, new THREE.TextureLoader().loadAsync(url).then((map) => {
      assertSoStylizedUnityTextureUploadReady(
        map,
        `Unity environment texture ${url}`,
      );
      map.name = `SoStylizedUnity:${name}`;
      map.colorSpace = normal ? THREE.NoColorSpace : THREE.SRGBColorSpace;
      map.wrapS = THREE.RepeatWrapping;
      map.wrapT = THREE.RepeatWrapping;
      map.magFilter = THREE.LinearFilter;
      // Every copied source texture declares Unity FilterMode.Bilinear (not
      // trilinear) and anisotropic level 1 in its TextureImporter.
      ...
      throw new Error(
        `Missing extracted Unity environment texture ${url}. Run `
        + '`node scripts/unity/extract-environment-baseline.mjs`.',
        { cause: error },
      );
    }));
  }
...
export function resolveSoStylizedUnityGrassVariant(profile) {
...
export function isSoStylizedUnityGrassProfile(profile) {
  return Boolean(resolveSoStylizedUnityGrassVariant(profile));
}

export function soStylizedUnityGrassCastsShadow(profile) {
  // P_Grass1/P_Grass2 override MeshRenderer.m_CastShadows to Off on every LOD.
  return !isSoStylizedUnityGrassProfile(profile);
}

export async function loadSoStylizedUnityTerrainTextures(options = {}) {
  if (!terrainTexturePromise) {
    terrainTexturePromise = Promise.all(Object.entries(SO_STYLIZED_UNITY_TERRAIN_LAYERS)
      .map(async ([name, layer]) => [name, {
        ...layer,
        diffuseMap: await loadUnityEnvironmentTexture(layer.diffuse, options),
        normalMap: layer.normal
          ? await loadUnityEnvironmentTexture(layer.normal, { ...options, normal: true })
          : null,
      }]))
...
const unityDither = wgslFn(`
  fn unityFoliageDither(inputValue: f32, pixelPosition: vec2<f32>) -> f32 {
...
const unityGradientNoise = wgslFn(`
  fn unityFoliageGradientNoise(sourceUv: vec2<f32>, scale: f32) -> f32 {
    let p = sourceUv * scale;
    let ip = floor(p);
    var fp = fract(p);
    let d00 = dot(unityFoliageGradientDirection(ip), fp);
    let d01 = dot(unityFoliageGradientDirection(ip + vec2<f32>(0.0, 1.0)), fp - vec2<f32>(0.0, 1.0));
    let d10 = dot(unityFoliageGradientDirection(ip + vec2<f32>(1.0, 0.0)), fp - vec2<f32>(1.0, 0.0));
    let d11 = dot(unityFoliageGradientDirection(ip + vec2<f32>(1.0, 1.0)), fp - vec2<f32>(1.0, 1.0));
    fp = fp * fp * fp * (fp * (fp * 6.0 - 15.0) + 10.0);
    return mix(mix(d00, d01, fp.y), mix(d10, d11, fp.y), fp.x) + 0.5;
  }

  fn unityFoliageGradientDirection(p: vec2<f32>) -> vec2<f32> {
    let x = unityFoliageHashTchou(p);
    return normalize(vec2<f32>(x - floor(x + 0.5), abs(x) - 0.5));
  }

  fn unityFoliageHashTchou(p: vec2<f32>) -> f32 {
...
const unityHueNormalized = wgslFn(`
  fn unityFoliageHueNormalized(inputColor: vec3<f32>, offset: f32) -> vec3<f32> {
...
function unityGrassGradient(value) {
  const keys = SO_STYLIZED_UNITY_GRASS.gradient;
 ...
 * literal Unity Shader Graph logic and values.
 */
export async function buildSoStylizedUnityGrassMaterial(profile, {
  hasVertexColors = false,
  state = null,
} = {}) {
  const variantId = resolveSoStylizedUnityGrassVariant(profile) ?? 'grass';
  const variant = SO_STYLIZED_UNITY_GRASS_VARIANTS[variantId];
  const values = { ...SO_STYLIZED_UNITY_GRASS, ...variant };
  const noiseMap = await loadUnityEnvironmentTexture('T_NoiseRough_SplatterMap.png');
  ...
  const gradientTip = unityGrassGradient(tipNoise);
  ...
  colorNode = unityHueNormalized(colorNode, hueOffset);
  ...
  const ditheredOpacity = unityDither(distanceVisibility.mul(2), screenCoordinate.xy);
  ...
  const windNoise = unityGradientNoise(animatedUv, float(values.windIntensity));
  ...
  material.userData.soStylizedUnityGrass = {
