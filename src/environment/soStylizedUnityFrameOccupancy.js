...
export const SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT = Object.freeze({
 ...
 */
export function analyzeSoStylizedUnityFrameOccupancy(
  pixels,
  width,
  height,
  {
    maximumDominantFraction = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT
      .maximumDominantFraction,
    minimumLumaRange = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT.minimumLumaRange,
    minimumLumaStandardDeviation = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT
      .minimumLumaStandardDeviation,
    minimumRgbRange = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT.minimumRgbRange,
    quantizationShift = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT.quantizationShift,
  } = {},
) {
 ...
 */
export async function verifySoStylizedUnityFrameOccupancy({
  renderer,
  render,
  height = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT.probeHeight,
  sampleFrames = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT.sampleFrames,
  warmupFrames = 1,
  width = SO_STYLIZED_UNITY_FRAME_OCCUPANCY_CONTRACT.probeWidth,
} = {}) {
  ...
  probe.texture.name = 'ToonLab Unity comparison final-frame occupancy probe';
  ...
  try {
    ...
    const report = analyzeSoStylizedUnityFrameOccupancy(pixels, width, height);
    if (!report.exact) {
      const error = new Error(
        'The live reconstruction post output is an empty/near-constant frame: '
        + JSON.stringify(report),
      );
      error.name = 'SoStylizedUnityFrameOccupancyError';
