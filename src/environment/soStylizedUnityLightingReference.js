// CPU reference for the active Unity 6000.5 / URP 17.5 lighting path.
//
// This deliberately contains no Three nodes. It is the numerical oracle used
// to verify the WebGPU lighting adapter against the supplied Unity package,
...
/** Unity's Linear project conversion used for Light.color before intensity. */
export function unityLightingSrgbChannelToLinear(value) {
...
export function unityLightingSrgbToLinear(color) {
  return color.map(unityLightingSrgbChannelToLinear);
}

// Precise values exported from the supplied M_Demonstration_Mega scene.
const UNITY_SUN_COLOR_SRGB = freezeVector([
  1,
  0.9443990588188171,
  0.8443396091461182,
]);
const UNITY_SUN_COLOR_LINEAR = freezeVector(
  unityLightingSrgbToLinear(UNITY_SUN_COLOR_SRGB),
);
const UNITY_SUN_FINAL_COLOR = freezeVector(
  scaleColor(UNITY_SUN_COLOR_LINEAR, 1.5),
);
const UNITY_RAY_DIRECTION = freezeVector([
  -0.6295879005986494,
  -0.7071067357898932,
  -0.3218992693690794,
]);
const THREE_RAY_DIRECTION = freezeVector([
  UNITY_RAY_DIRECTION[0],
  UNITY_RAY_DIRECTION[1],
  -UNITY_RAY_DIRECTION[2],
]);

export const SO_STYLIZED_UNITY_LIGHTING_REFERENCE = Object.freeze({
  ...
  brdf: Object.freeze({
    ...
    // For stock Three Lambert only. A literal custom URP BRDF instead divides
    ...
    threeCustomUrpInputScale: Math.PI,
  }),
  ...
  sun: Object.freeze({
    colorSrgb: UNITY_SUN_COLOR_SRGB,
    colorLinear: UNITY_SUN_COLOR_LINEAR,
    finalColorLinear: UNITY_SUN_FINAL_COLOR,
    intensity: 1.5,
    // Unity +Z-forward, direction in which the light rays travel.
    unityRayDirection: UNITY_RAY_DIRECTION,
    unitySurfaceToLight: freezeVector(UNITY_RAY_DIRECTION.map((value) => -value)),
    // The Unity exporter reflects Z when producing glTF/Three coordinates.
  ...
  sources: Object.freeze({
    ...
    brdf:
      'URP 17.5 ShaderLibrary/BRDF.hlsl:9-96,177-214',
    lighting:
      'URP 17.5 ShaderLibrary/Lighting.hlsl:32-100,302-403',
    ambientProbe:
      'Core RP 17.5 ShaderLibrary/AmbientProbe.hlsl:6-56 + SphericalHarmonics.hlsl:17-24',
    ambientOcclusion:
      'URP 17.5 ShaderLibrary/AmbientOcclusion.hlsl:28-68',
    ssao:
      'URP 17.5 Runtime/Passes/ScreenSpaceAmbientOcclusionPass.cs:84-103,435-447 + ShaderLibrary/SSAO.hlsl:29-47,385-447,528-535',
    threePhysicalSetup:
...
/** Literal URP BRDFData initialization for metallic or specular workflow. */
export function initializeUnityUrpBrdf({
  albedo = [1, 1, 1],
  metallic = 0,
...
/** Literal URP DirectBRDFSpecular scalar (desktop/full-real branch). */
export function evaluateUnityUrpDirectSpecularScalar({
  brdf,
  normal = [0, 1, 0],
...
export function evaluateUnityUrpDirectLighting({
  brdf,
  lightColor = [1, 1, 1],
  normal = [0, 1, 0],
  lightDirection = [0, 1, 0],
  ...
  const specularScalar = evaluateUnityUrpDirectSpecularScalar({
    brdf,
    normal,
    lightDirection,
    viewDirection,
...
/**
 * SSAO texture placement used by URP UniversalFragmentPBR.
 * The sample is already a visibility value after Unity's intensity/contrast
 * estimator and bilateral blur; intensity must not be applied again here.
 */
export function evaluateUnityUrpSsaoFactors(sample, {
  directLightingStrength = 0.25,
  materialOcclusion = 1,
} = {}) {
...
/** Unity's transfer from normalized obscurance to the final AO visibility. */
export function evaluateUnitySsaoVisibilityFromObscurance(obscurance, {
  intensity = 0.4,
  contrastExponent = 0.6,
  viewDepth = 0,
  falloffDistance = 100,
