// Unity-authoritative coupling for visible, depth-as-color, and ShadowCaster
// evaluation of reconstructed Shader Graph materials.
//
...
export const SO_STYLIZED_UNITY_PASS_COUPLING_SCHEMA_VERSION = 1;
...
/**
 * Render-state and graph-output facts read from the supplied Shader Graphs and
 * their Unity 6000.5 / URP 17.5 generated passes. `renderFace=0` is Cull Off;
 * `renderFace=2` is Cull Back (front faces survive).
 */
export const SO_STYLIZED_UNITY_MATERIAL_PASS_CONTRACTS = Object.freeze({
  'Shader Graphs/S_FoliageShader': freezeContract({
  ...
  'Shader Graphs/S_Leaves': freezeContract({
  ...
  'Shader Graphs/S_Bark': freezeContract({
    alphaChannel: 'opaque',
    alphaClip: false,
    cull: 'Back',
    renderFace: 2,
  ...
  'Shader Graphs/S_Snow': freezeContract({
    alphaChannel: 'opaque',
    alphaClip: false,
    cull: 'Back',
    renderFace: 2,
  ...
  'Shader Graphs/S_StylizedBasic': freezeContract({
    alphaChannel: 'opaque',
    alphaClip: false,
    cull: 'Back',
    renderFace: 2,
  ...
  'Universal Render Pipeline/Lit': freezeContract({
    alphaChannel: 'opaque',
    alphaClip: false,
    cull: 'Back',
    renderFace: 2,
...
export function resolveSoStylizedUnityMaterialPassContract(shaderName) {
  return SO_STYLIZED_UNITY_MATERIAL_PASS_CONTRACTS[String(shaderName ?? '')] ?? null;
}

/** CPU boundary oracle for Unity Shader Graph's `clip(alpha - threshold)`. */
export function evaluateSoStylizedUnityAlphaClip(alpha, threshold) {
  ...
  if (!Number.isFinite(resolvedAlpha) || !Number.isFinite(resolvedThreshold)) {
    throw new TypeError('Unity alpha clip evaluation requires finite numbers.');
  }
...
function createCoupledDepthColorMaterial(source, coupling) {
  ...
  depthMaterial.name = `${source.name || 'SoStylizedUnity'}:CoupledDepth`;
  depthMaterial.lights = false;
  depthMaterial.fog = false;
  depthMaterial.side = coupling.side;
  // Unity's generated DepthOnly and ShadowCaster passes use the same Cull
  ...
  depthMaterial.userData.soStylizedUnityPassCoupling = {
 ...
 *
 * For alpha-cut families, `alphaNode` is the complete connected Shader Graph
 * Alpha output (texture channel plus every dither/fade multiplier), not merely
 * the source texture. Unity uses `clip(alpha - threshold)`, so equality must
 * survive; a shared `>=` boolean mask is used instead of Three's alphaTest.
 */
export function installSoStylizedUnityMaterialPassCoupling(material, {
  alphaChannel = null,
  alphaNode = null,
  alphaThreshold = null,
  ...
  if (!material?.isNodeMaterial) {
    throw new TypeError('Unity material pass coupling requires a Three NodeMaterial.');
  }
  const contract = resolveSoStylizedUnityMaterialPassContract(shaderName);
  if (!contract) {
    throw new TypeError(`No audited Unity pass contract exists for ${shaderName ?? 'missing shader'}.`);
  }
  ...
  const prior = material.userData?.soStylizedUnityPassCoupling;
  ...
  const metadataSnapshot = {
    ...
    positionMode,
    schemaVersion: SO_STYLIZED_UNITY_PASS_COUPLING_SCHEMA_VERSION,
    shaderName,
  ...
  material.userData.soStylizedUnityPassCoupling = {
    ...metadataSnapshot,
    runtime,
  };
...
export function createSoStylizedUnityPassCouplingReport(root) {
  ...
  root.traverse((object) => {
    ...
    for (const material of materialsOf(object)) {
      const shaderName = material?.userData?.soStylizedUnityPassCoupling?.shaderName
        ?? material?.userData?.soStylizedUnityMaterial?.sourceShader
        ?? material?.userData?.soStylizedUnitySceneTree?.shaderName
        ?? null;
      if (!resolveSoStylizedUnityMaterialPassContract(shaderName)) continue;
      knownMaterials.add(material);
      if (material.userData?.soStylizedUnityPassCoupling?.exact) {
  ...
  const report = {
    alphaClipMaterialCount: coupled.filter((material) => (
      material.userData.soStylizedUnityPassCoupling.alphaClip
    )).length,
    casterMeshCount,
    coupledMaterialCount: coupledMaterials.size,
    coupledMeshCount,
    depthVariantCreateCount: coupled.reduce((sum, material) => (
      sum + material.userData.soStylizedUnityPassCoupling.runtime.depthVariantCreateCount
    ), 0),
    ...
    wpoMaterialCount: coupled.filter((material) => (
      material.userData.soStylizedUnityPassCoupling.positionMode === 'deformed'
    )).length,
  };
  root.userData.soStylizedUnityPassCoupling = { ...report };
  return report;
}
