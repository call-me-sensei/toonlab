// Provenance and identity gate for the native Unity Mega A/B.
//
...
export const SO_STYLIZED_UNITY_MEGA_CURRENT_COMPARISON_EVIDENCE = Object.freeze({
  schema: 'toonlab.sostylized-unity.mega-comparison-evidence',
  schemaVersion: 2,
  baseUrl: '/assets-local/sostylized-unity/mega-scene-native-pc-current',
  sourceScene: 'Assets/SoStylized-Unity/Demo/M_Demonstration_Mega.unity',
  sceneName: 'M_Demonstration_Mega',
...
// Unity exposes Transform.eulerAngles using its documented Z-X-Y application
// order. The combined quaternion is therefore Qy * Qx * Qz.
function quaternionFromUnityEulerDegrees(rotation) {
...
export function parseSoStylizedUnityNativeCaptureReport(text) {
  if (typeof text !== 'string' || !text.trim()) {
    throw new TypeError('A non-empty native Unity capture report is required.');
  }
  const fields = {};
...
export function createSoStylizedUnityMegaComparisonIdentityReport({
  baseUrl = SO_STYLIZED_UNITY_MEGA_CURRENT_COMPARISON_EVIDENCE.baseUrl,
  frameParityReport,
  manifest,
  ...
  nativeImageWidth,
  nativeReferenceUrl,
  viewportHeight = SO_STYLIZED_UNITY_MEGA_CURRENT_COMPARISON_EVIDENCE.viewport.height,
  viewportWidth = SO_STYLIZED_UNITY_MEGA_CURRENT_COMPARISON_EVIDENCE.viewport.width,
} = {}) {
  ...
  const native = parseSoStylizedUnityNativeCaptureReport(nativeCaptureReport);
  ...
  const nativeCameraQuaternion = quaternionFromUnityEulerDegrees(native.camera.rotation);
  ...
  const gates = Object.freeze({
    ...
    scene: native.scene === manifest.sourceScene
      ...
      && manifest.sourceScene === evidence.sourceScene
      && manifest.sceneName === evidence.sceneName
