// Numerical source-frame verification for the Unity Mega comparison.
//
...
} from './soStylizedUnityMegaScene.js';

export const SO_STYLIZED_UNITY_MEGA_PARITY_VIEWPORT = Object.freeze({
  width: 1920,
  height: 1080,
  aspect: 1920 / 1080,
});
...
// world quaternion within about 2.82e-4 rad of Unity Transform.rotation. This
...
// Unity's Terrain renderer, Terrain trees, and native details use only
...
/**
 * Project one Unity world-space point with the source camera convention.
 * Unity cameras look down local +Z; the returned x/y are OpenGL-style NDC.
 */
export function projectSoStylizedUnityMegaSourcePoint(
  sourceWorldPoint,
  cameraNode,
  cameraRecord,
 ...
 */
export function attachSoStylizedUnityMegaCameraToRenderScene(
  camera,
  renderScene,
  {
    cameraNode = null,
  ...
  if (cameraNode) {
    const expectedPosition = reflectSoStylizedUnityMegaPosition(cameraNode.worldPosition);
    const expectedQuaternion = reflectSoStylizedUnityMegaQuaternion(cameraNode.worldRotation);
  ...
  camera.userData.soStylizedUnityCameraFrame = report;
  return report;
}
...
function compareNodeFrame(sceneRoot, manifest) {
  const objects = sceneRoot?.userData?.soStylizedUnityNodeObjects;
  ...
  for (const node of manifest.nodes ?? []) {
    ...
    const expectedPosition = reflectSoStylizedUnityMegaPosition(node.worldPosition);
    const expectedQuaternion = reflectSoStylizedUnityMegaQuaternion(node.worldRotation);
...
function compareTerrainFrame(terrainRuntime, manifest) {
  ...
  if (position?.count === resolution * resolution
    ...
    for (const sourceZ of terrainProbeIndices(resolution)) {
      for (const x of terrainProbeIndices(resolution)) {
        ...
        const expectedWorld = reflectSoStylizedUnityMegaPosition(
  ...
  for (const index of new Set([
    ...
    const expected = reflectSoStylizedUnityMegaPosition(sourceWorld.toArray());
  ...
  for (const entry of terrainRuntime.details?.prototypeEntries ?? []) {
    ...
    for (const index of new Set([
      ...
      const sourceLocal = reflectedLocal.clone();
      sourceLocal.z = -sourceLocal.z;
      const expected = reflectSoStylizedUnityMegaPosition(
  ...
  return {
    ...
    transformAuthority: 'UnityEngine.Terrain.GetPosition(): translation only',
  };
}
 ...
 */
export function createSoStylizedUnityMegaFrameParityReport({
  camera,
  manifest,
  renderScene,
  sceneRoot,
  ...
  for (const landmark of collectProjectionLandmarks(manifest, terrainRuntime)) {
    const source = projectSoStylizedUnityMegaSourcePoint(
      landmark.sourceWorld,
      cameraNode,
      cameraRecord,
      { aspect },
    );
    const reflected = reflectSoStylizedUnityMegaPosition(landmark.sourceWorld);
  ...
  camera.userData.soStylizedUnityFrameParity = report;
  return report;
}
