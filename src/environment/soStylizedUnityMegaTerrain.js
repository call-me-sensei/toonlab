// Runtime reconstruction of the supplied So Stylized Unity Mega terrain.
//
// This module intentionally consumes the Unity scene export directly. It does
// not route the terrain through ToonLab's procedural terrain painter or through
// the legacy Unreal landscape masks. Heights, holes, splat weights, layer
...
export const DEFAULT_SO_STYLIZED_UNITY_MEGA_SCENE_URL =
  '/assets-local/sostylized-unity/mega-scene';

export const SO_STYLIZED_UNITY_MEGA_TERRAIN_CONTRACT = Object.freeze({
  schema: 'toonlab.sostylized-unity.scene-export',
  schemaVersion: 2,
  sourceScene: 'Assets/SoStylized-Unity/Demo/M_Demonstration_Mega.unity',
  heightmapResolution: 513,
  holesResolution: 512,
  alphamapResolution: 2048,
  alphamapLayers: 5,
  coordinateSystem:
    'glTF/Three right-handed Y-up: Unity local +Z is reflected to Three local -Z',
  defaultSplatPrecision: 'float32',
  fastSplatPrecision: 'uint8',
  terrainShader: 'Universal Render Pipeline/Terrain/Lit',
});

// URP Terrain/Lit renders more than four layers as multiple independently lit
...
class SoStylizedUnityMegaTerrainPassMaterial extends MeshPhysicalNodeMaterial {
...
async function fetchChecked(fetchFn, url) {
  ...
  if (!response?.ok) {
    throw new Error(`Unable to load Unity Mega terrain asset ${url} (${response?.status ?? 'no response'}).`);
  }
  return response;
}

/** Decode a little-endian .f32 sidecar without changing any source values. */
export function decodeSoStylizedUnityFloat32(arrayBuffer) {
  if (!(arrayBuffer instanceof ArrayBuffer)) {
    throw new TypeError('Unity float sidecar must be supplied as an ArrayBuffer.');
  }
  if (arrayBuffer.byteLength % 4 !== 0) {
    throw new RangeError(`Unity float sidecar byte length ${arrayBuffer.byteLength} is not divisible by four.`);
  }
...
export function decodeSoStylizedUnityInt32(arrayBuffer) {
  if (!(arrayBuffer instanceof ArrayBuffer)) {
    throw new TypeError('Unity integer sidecar must be supplied as an ArrayBuffer.');
  }
  if (arrayBuffer.byteLength % 4 !== 0) {
    throw new RangeError(`Unity integer sidecar byte length ${arrayBuffer.byteLength} is not divisible by four.`);
  }
...
function validateTerrainRecord(terrain) {
  ...
  if (holesResolution !== resolution - 1) {
    throw new RangeError(
      `Unity heightmap ${resolution} requires ${resolution - 1} hole cells; received ${holesResolution}.`,
    );
  }
  if (!Array.isArray(terrain.size) || terrain.size.length < 3) {
    throw new TypeError('Unity terrain size must be [x, y, z].');
  }
...
/**
 * Construct the full-resolution reflected-Z Unity heightfield.
 *
 * Unity's exported holes are sampled per quad: 1 is solid and 0 is a hole,
 ...
 */
export function buildSoStylizedUnityMegaTerrainGeometry(terrainRecord, heights, holes) {
  ...
  if (!(heights instanceof Float32Array) || heights.length !== vertexCount) {
    throw new RangeError(
      `Unity terrain heights must contain ${vertexCount} float32 samples; received ${heights?.length ?? 0}.`,
    );
  }
  if (!(holes instanceof Uint8Array) || holes.length !== cellCount) {
    throw new RangeError(
      `Unity terrain holes must contain ${cellCount} uint8 samples; received ${holes?.length ?? 0}.`,
    );
  }
  ...
  for (let sourceZ = 0; sourceZ < resolution; sourceZ += 1) {
    ...
    for (let x = 0; x < resolution; x += 1) {
      ...
      // Source normal is normalize(-dH/dX, 1, -dH/dZ). Reflecting Unity +Z
      ...
      // This is Unity TerrainLit's X-axis tangent after the coordinate
  ...
  geometry.name = `${terrain.terrainDataName ?? terrain.name ?? 'UnityMegaTerrain'}:513x513`;
  ...
  geometry.userData.soStylizedUnityMegaTerrain = {
    coordinateReflection: 'position.z = -unityLocalZ',
    heightScale: sizeY,
...
export function splitSoStylizedUnityMegaSplatWeights(source, width, height, layers) {
  assertInteger(width, 'alphamap width');
  assertInteger(height, 'alphamap height');
  if (layers !== 5) {
    throw new RangeError(`The Unity Mega terrain requires exactly five splat layers; received ${layers}.`);
  }
  const pixelCount = width * height;
  if (!(source instanceof Float32Array) || source.length !== pixelCount * layers) {
    throw new RangeError(
      `Unity Mega alphamaps must contain ${pixelCount * layers} float32 values; received ${source?.length ?? 0}.`,
    );
  }
...
function configureControlTexture(map, metadata) {
  ...
  map.userData.soStylizedUnityControlMap = metadata;
  map.needsUpdate = true;
  return map;
}

/** Read one exact exported control texel in Unity Terrain layer order. */
export function sampleSoStylizedUnityMegaTerrainSplat(
  controlTextures,
  width,
  height,
  x,
  ...
  if (!Array.isArray(controlTextures) || controlTextures.length !== 2) {
    throw new TypeError('Two Unity Terrain control textures are required.');
  }
  ...
  if (!first || !second || first.length < (pixel + 1) * 4 || second.length <= pixel) {
    throw new Error(`Unity Terrain control texel ${texelX},${texelZ} is unavailable.`);
  }
...
async function loadFloatControlTextures(terrain, baseUrl, fetchFn) {
  ...
  const source = decodeSoStylizedUnityFloat32(await response.arrayBuffer());
  const { firstFour, fifth } = splitSoStylizedUnityMegaSplatWeights(
    source,
  ...
  const first = configureControlTexture(
    ...
    {
      name: 'SoStylizedUnityMega:Control0:Float32',
      precision: 'float32',
  ...
  const second = configureControlTexture(
    ...
    {
      name: 'SoStylizedUnityMega:Control1:Float32',
      precision: 'float32',
      source: terrain.alphamaps,
      layers: [4],
    },
...
async function loadUint8ControlTextures(terrain, baseUrl, fetchFn) {
  if (!Array.isArray(terrain.controlMaps) || terrain.controlMaps.length < 2) {
    throw new Error('Unity Mega uint8 splat mode requires two exported control maps.');
  }
  ...
  const first = configureControlTexture(
    ...
    {
      name: 'SoStylizedUnityMega:Control0:Uint8',
      precision: 'uint8',
  ...
  const second = configureControlTexture(
    ...
    {
      name: 'SoStylizedUnityMega:Control1:Uint8',
      precision: 'uint8',
...
function unityWrapMode(mode) {
  if (/mirror/i.test(mode ?? '')) return THREE.MirroredRepeatWrapping;
  if (/clamp/i.test(mode ?? '')) return THREE.ClampToEdgeWrapping;
  return THREE.RepeatWrapping;
}

function applyUnityTextureImporter(map, textureRecord) {
  assertSoStylizedUnityTextureUploadReady(
    map,
    `Unity Mega terrain texture ${textureRecord.exactSourceCopy ?? textureRecord.name}`,
  );
  const importer = textureRecord.importer ?? {};
  map.name = `SoStylizedUnityMega:${textureRecord.name}`;
  map.colorSpace = importer.sRGBTexture ? THREE.SRGBColorSpace : THREE.NoColorSpace;
  map.wrapS = unityWrapMode(importer.wrapMode);
  map.wrapT = unityWrapMode(importer.wrapMode);
  ...
  if (/point/i.test(importer.filterMode ?? '')) {
    ...
    // Unity FilterMode.Bilinear samples one mip with bilinear filtering.
    map.magFilter = THREE.LinearFilter;
    map.minFilter = map.generateMipmaps
  ...
  map.flipY = true;
  map.userData.soStylizedUnityTexture = {
...
async function loadTerrainLayers(manifest, terrain, baseUrl, textureLoader) {
  if (!Array.isArray(terrain.layers) || terrain.layers.length !== 5) {
    throw new RangeError(`Unity Mega terrain must declare five layers; received ${terrain.layers?.length ?? 0}.`);
  }
  const cache = new Map();
  const loadTextureRecord = async (index) => {
    ...
    if (!textureRecord?.exactSourceCopy) {
      throw new Error(`Unity terrain texture index ${index} has no exactSourceCopy.`);
    }
    if (!cache.has(index)) {
      const url = joinAssetUrl(baseUrl, textureRecord.exactSourceCopy);
      cache.set(index, textureLoader.loadAsync(url)
        .then((map) => applyUnityTextureImporter(map, textureRecord))
        .catch((error) => {
          cache.delete(index);
          throw new Error(`Unable to load Unity terrain texture ${url}.`, { cause: error });
        }));
    }
  ...
  return Promise.all(terrain.layers.map(async (layer) => {
    if (!layer || !Number.isInteger(layer.index)) {
      throw new TypeError('Unity terrain layer record is incomplete.');
    }
    return {
      ...layer,
...
function buildSoStylizedUnityMegaTerrainPassMaterial({
  terrain,
  controlTextures,
  layers,
  ...
  if (!Array.isArray(controlTextures) || controlTextures.length !== 2) {
    throw new TypeError('Unity Mega terrain material requires two control textures.');
  }
  if (!Array.isArray(layers) || layers.length !== 5) {
    throw new TypeError('Unity Mega terrain material requires five loaded terrain layers.');
  }
  if (!Array.isArray(layerIndices)
    || layerIndices.length === 0
    || layerIndices.some((index) => !Number.isInteger(index) || index < 0 || index >= 5)) {
    throw new RangeError('Unity Mega terrain pass requires one or more layer indices in [0,4].');
  }
  const sourceUv = uv();
  ...
  for (const index of layerIndices) {
    ...
    if (!layer.diffuseMap) throw new Error(`Unity terrain layer ${layer.name} has no diffuse texture.`);
    ...
    if (layer.normalMap) {
      ...
      const unpacked = decodeSoStylizedUnityNormalNode(
        normalSample,
        flipGreenChannel ? -1 : 1,
      );
      tangentNormal = applySoStylizedUnityNormalScaleNode(
        unpacked,
        layer.normalScale ?? 1,
      );
    }
  ...
  const material = new SoStylizedUnityMegaTerrainPassMaterial(
    sourceWeightedOutput ? passWeight : null,
  );
  material.name = `SoStylizedUnityMega:URP Terrain Lit:${passName}`;
  ...
  material.userData.soStylizedUnityMegaTerrain = {
    sourceShader: 'Universal Render Pipeline/Terrain/Lit',
  ...
  material.userData.soStylizedUnityNormalIntegration = {
    ...createSoStylizedUnityNormalIntegrationMetadata({
      coordinateZSign: -1,
      decode: 'RG + per-layer importer green transform + reconstructed positive Z; URP UnpackNormalScale',
      family: 'unity-mega-terrain',
      textureFlipY: true,
    }),
    terrainTangent:
      'reflected Unity TerrainLit X tangent; tangent.w=1; bitangent follows increasing Unity V',
  };
 ...
 * uses buildSoStylizedUnityMegaTerrainMaterials() below because URP Terrain/Lit
 * renders this five-layer terrain as an opaque base pass plus one add pass.
 */
export function buildSoStylizedUnityMegaTerrainMaterial(options = {}) {
  return buildSoStylizedUnityMegaTerrainPassMaterial(options);
}

/** Build the exact two-pass URP Terrain/Lit material set for five layers. */
export function buildSoStylizedUnityMegaTerrainMaterials(options = {}) {
  const base = buildSoStylizedUnityMegaTerrainPassMaterial({
    ...options,
    layerIndices: [0, 1, 2, 3],
    passName: 'base-0-3',
    sourceWeightedOutput: true,
  });
  const additive = buildSoStylizedUnityMegaTerrainPassMaterial({
    ...options,
    layerIndices: [4],
    passName: 'add-4',
    sourceWeightedOutput: true,
  ...
  additive.userData.soStylizedUnityMegaTerrain.addPass = {
    blend: 'One One',
    clip: 'weight <= 0.005',
    depthTest: 'LessEqual',
    depthWrite: false,
    sourceShader: 'Hidden/Universal Render Pipeline/Terrain/Lit (Add Pass)',
  };
...
/**
 * Apply Unity Terrain's renderer transform, which is deliberately not the
 * Transform component's complete TRS. Unity Terrain supports translation
 ...
 */
export function applySoStylizedUnityMegaTerrainPosition(root, terrain, node) {
  if (!root || !terrain || !node) {
    throw new Error('Unity Mega terrain root, record, and node are required.');
  }
  ...
  if (!Array.isArray(position) || position.length < 3) {
    throw new Error('Unity Mega terrain has no Terrain.GetPosition() authority.');
  }
  ...
  root.userData.soStylizedUnityTerrainTransform = {
    authority: 'UnityEngine.Terrain.GetPosition(): translation only',
...
export function getSoStylizedUnityMegaTerrainPopulation(manifest, terrainIndex = 0) {
...
export const SO_STYLIZED_UNITY_MEGA_DETAIL_COVERAGE_MAX = 255;
export const SO_STYLIZED_UNITY_MEGA_DETAIL_TRANSFORM_STRIDE = 6;
export const SO_STYLIZED_UNITY_MEGA_DETAIL_PLACEMENT_MODES = Object.freeze({
  nativeExact: 'native-exact',
  deterministicSourceStyle: 'deterministic-source-style',
});

const SO_STYLIZED_UNITY_MEGA_NATIVE_DETAIL_API =
...
function validateDetailDensityField(terrain, prototype, densityField) {
  ...
  if (!(densityField instanceof Int32Array) || densityField.length !== expectedLength) {
    throw new RangeError(
      `Unity detail ${prototype?.index ?? '?'} must contain ${expectedLength} int32 samples; `
...
function resolveDetailDensityScale(terrain, prototype, densityScale) {
  ...
  if (!Number.isFinite(globalScale) || globalScale < 0) {
    throw new RangeError('Unity detail density scale must be a finite non-negative number.');
  }
...
function detailCellExpectedCount(terrain, prototype, coverageValue, densityScale) {
  if (!Number.isInteger(coverageValue)
    || coverageValue < 0
    || coverageValue > SO_STYLIZED_UNITY_MEGA_DETAIL_COVERAGE_MAX) {
    throw new RangeError(
      `Unity CoverageMode detail value must be in [0,255]; received ${coverageValue}.`,
    );
  }
  ...
  return (coverageValue / SO_STYLIZED_UNITY_MEGA_DETAIL_COVERAGE_MAX)
    * prototypeDensity
    * targetCoverage
    * cellArea
    * densityScale;
 ...
 *
 * Unity's source map is uint8 coverage serialized through GetDetailLayer as
 ...
 */
export function countSoStylizedUnityMegaDetailInstances({
  terrain,
  prototype,
  densityField,
  densityScale = null,
  ...
  for (let z = 0; z < resolution; z += 1) {
    for (let x = 0; x < resolution; x += 1) {
      ...
      if (coverageValue < 0 || coverageValue > SO_STYLIZED_UNITY_MEGA_DETAIL_COVERAGE_MAX) {
        throw new RangeError(
          `Unity detail ${prototype?.index ?? '?'} cell ${x},${z} has invalid coverage ${coverageValue}.`,
        );
      }
...
/** Build deterministic reflected-Z transforms, ordered in Unity patch order. */
export function buildSoStylizedUnityMegaDetailPlacements({
  terrain,
  prototype,
  densityField,
  sampleHeightLocal,
  ...
  if (typeof sampleHeightLocal !== 'function') {
    throw new TypeError('Unity detail placement requires sampleHeightLocal(x, reflectedZ).');
  }
  const resolvedDensityScale = resolveDetailDensityScale(terrain, prototype, densityScale);
  const summary = countSoStylizedUnityMegaDetailInstances({
    terrain,
    prototype,
    densityField,
    densityScale: resolvedDensityScale,
  ...
  const transforms = new Float32Array(
    summary.instanceCount * SO_STYLIZED_UNITY_MEGA_DETAIL_TRANSFORM_STRIDE,
  );
      ...
      for (let z = minCellZ; z < maxCellZ; z += 1) {
        for (let x = minCellX; x < maxCellX; x += 1) {
          ...
          for (let slot = 0; slot < count; slot += 1) {
            ...
            const offset = instanceIndex * SO_STYLIZED_UNITY_MEGA_DETAIL_TRANSFORM_STRIDE;
  ...
  if (instanceIndex !== summary.instanceCount) {
    throw new Error(
      `Unity detail ${prototype.index} count changed between placement passes `
  ...
  return Object.freeze({
    ...
    transformStride: SO_STYLIZED_UNITY_MEGA_DETAIL_TRANSFORM_STRIDE,
    transforms,
  });
}
...
function validateNativeDetailTransformSet(terrain, prototype, transformSet) {
  ...
  if (!record || record.api !== SO_STYLIZED_UNITY_MEGA_NATIVE_DETAIL_API) {
    throw new Error(
      `Unity detail ${prototype?.index ?? '?'} has no exact `
      + `${SO_STYLIZED_UNITY_MEGA_NATIVE_DETAIL_API} record.`,
    );
  }
  if (!(transforms instanceof Float32Array)) {
    throw new TypeError(
      `Unity detail ${prototype.index} native transforms must be a Float32Array.`,
    );
  }
  if (record.strideFloats !== SO_STYLIZED_UNITY_MEGA_DETAIL_TRANSFORM_STRIDE) {
    throw new RangeError(
      `Unity detail ${prototype.index} native stride ${record.strideFloats} is unsupported.`,
    );
  }
  if (record.density !== terrain.detailObjectDensity
    || !String(record.densityAuthority).includes('Terrain.detailObjectDensity')) {
    throw new Error(
      `Unity detail ${prototype.index} native density authority does not match the Terrain.`,
    );
  }
  ...
  if (transforms.length !== expectedFloats || record.byteLength !== expectedFloats * 4) {
    throw new RangeError(
      `Unity detail ${prototype.index} native transform payload shape drifted.`,
    );
  }
  ...
  if (record.patchCountPerAxis !== patchCountPerAxis
    ...
    throw new RangeError(
      `Unity detail ${prototype.index} native patch inventory drifted.`,
    );
  }
  ...
  for (let index = 0; index < record.patches.length; index += 1) {
    ...
    if (patch.index !== index
      ...
      throw new Error(
        `Unity detail ${prototype.index} native patch ${index} metadata drifted.`,
      );
    }
  ...
  if (expectedOffset !== record.transformCount) {
    throw new RangeError(
      `Unity detail ${prototype.index} native patch counts do not cover its payload.`,
    );
  }
  for (let offset = 0; offset < transforms.length; offset += 1) {
    if (!Number.isFinite(transforms[offset])) {
      throw new RangeError(
        `Unity detail ${prototype.index} native transform contains a non-finite field.`,
      );
    }
  }
...
/**
 * Reflect Unity's exact native detail transforms into the terrain-local Three
 * basis. No placement, density, height, jitter, rotation, or scale value is
 * regenerated here; the six source fields come directly from Unity 6000.5.
 */
export function buildSoStylizedUnityMegaNativeDetailPlacements({
  terrain,
  prototype,
  transformSet,
} = {}) {
  ...
  for (
    ...
    offset += SO_STYLIZED_UNITY_MEGA_DETAIL_TRANSFORM_STRIDE
  ) {
  ...
  return Object.freeze({
    ...
    nativeUnityVersion: record.unityVersion,
 ...
 * draw count is repacked with Unity's exported detail-object distance and
 * per-patch frustum test, mirroring Terrain's native bounded detail renderer.
 */
export function instantiateSoStylizedUnityMegaTerrainDetails({
  manifest,
  terrainIndex = 0,
  prefabLibrary,
  densityFields = null,
  transformSets = null,
  placementMode = SO_STYLIZED_UNITY_MEGA_DETAIL_PLACEMENT_MODES.nativeExact,
  sampleHeightLocal,
  densityScale = null,
  detailDistance = null,
  onDetailMesh = null,
  ...
  if (!Object.values(SO_STYLIZED_UNITY_MEGA_DETAIL_PLACEMENT_MODES).includes(placementMode)) {
    throw new RangeError(`Unsupported Unity detail placement mode ${placementMode}.`);
  }
  const nativeExact = placementMode
    === SO_STYLIZED_UNITY_MEGA_DETAIL_PLACEMENT_MODES.nativeExact;
  if (nativeExact
    && (!Array.isArray(transformSets) || transformSets.length !== prototypes.length)) {
    throw new RangeError(
      `Unity Mega parity mode requires ${prototypes.length} native transform sets; `
  ...
  if (!nativeExact
    ...
    throw new RangeError(
      `Unity Mega deterministic detail mode requires ${prototypes.length} density fields; `
  ...
  if (!Number.isFinite(resolvedDetailDistance) || resolvedDetailDistance < 0) {
    throw new RangeError('Unity detail-object distance must be a finite non-negative number.');
  }
  ...
  for (let prototypeIndex = 0; prototypeIndex < prototypes.length; prototypeIndex += 1) {
    ...
    const placements = nativeExact
      ...
      : buildSoStylizedUnityMegaDetailPlacements({
    ...
    sourceRoot.traverse((sourceMesh) => {
      ...
      const prefabNodeIndex = Number(sourceMesh.userData?.unityPrefabNode);
      ...
      geometry.setAttribute('iUnityObjectPosition', objectPositions);
      ...
      mesh.count = 0;
      applySoStylizedUnityRendererCastEligibility(mesh, sourceNode?.renderer, manifest);
      mesh.frustumCulled = false;
      mesh.renderOrder = sourceMesh.renderOrder;
      mesh.layers.mask = sourceMesh.layers.mask;
      mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
      mesh.userData.soStylizedUnityTerrainDetail = {
        coordinateReflection: 'position.z and rotation.y are negated',
    ...
    prototypeGroup.userData.soStylizedUnityTerrainDetail = {
  ...
  const metadata = {
    ...
    activeSelectionHash,
    coordinateReflection: 'Unity Terrain local +Z -> Three local -Z',
    ...
    patchDistanceAuthority: nativeExact
      ? 'Unity-returned per-prototype patch Bounds; exact 3D distance and full frustum test'
      : 'detail grid patch bounds; horizontal distance to camera',
    placementAuthority: nativeExact
      ? `${SO_STYLIZED_UNITY_MEGA_NATIVE_DETAIL_API}:native-exact`
  ...
  group.userData.soStylizedUnityTerrainDetails = metadata;
  ...
  const update = (camera, { force = false } = {}) => {
    ...
    // Unity 6000.5 DetailRenderer::Render first compares
    ...
    for (let entryIndex = 0; entryIndex < prototypeEntries.length; entryIndex += 1) {
      ...
      for (const sourceMeshEntry of sourceMeshes) {
        ...
        for (const patchIndex of visiblePatches) {
          ...
          for (let sourceIndex = patch.start; sourceIndex < end; sourceIndex += 1) {
            const offset = sourceIndex * SO_STYLIZED_UNITY_MEGA_DETAIL_TRANSFORM_STRIDE;
      ...
      prototypeEntry.prototypeGroup.userData.soStylizedUnityTerrainDetail.activeInstanceCount =
        prototypeActiveCount;
      prototypeEntry.prototypeGroup.userData.soStylizedUnityTerrainDetail.activePatchCount =
 ...
 *
 * The returned `update(camera, { lodBias })` applies the exported Unity
 ...
 */
export function instantiateSoStylizedUnityMegaTerrainTrees({
  manifest,
  terrainIndex = 0,
  prefabLibrary,
  ...
  for (let instanceIndex = 0; instanceIndex < treeInstances.length; instanceIndex += 1) {
    ...
    wrapper.add(clone);
    wrapper.userData.soStylizedUnityTerrainTree = {
      instanceIndex,
      ...
      unityRotationRadians: sourceInstance.rotation,
      lodGroups: prefabRecord?.lodGroups ?? [],
    };

    const nodeByPrefabIndex = new Map();
    clone.traverse((object) => {
      const prefabNode = object.userData?.unityPrefabNode;
  ...
  const update = (camera, { lodBias = 1 } = {}) => {
    ...
    group.userData.soStylizedUnityTerrainTrees.lastLodBias = resolvedLodBias;
    group.userData.soStylizedUnityTerrainTrees.lastLodReport = report;
    return report;
  };
  const dispose = () => {
    group.removeFromParent();
  ...
  group.userData.soStylizedUnityTerrainTrees = {
...
export async function loadSoStylizedUnityMegaDetailDensityFields(
  terrain,
  {
    baseUrl = DEFAULT_SO_STYLIZED_UNITY_MEGA_SCENE_URL,
    fetchFn = globalThis.fetch?.bind(globalThis),
  } = {},
) {
  validateTerrainRecord(terrain);
  if (typeof fetchFn !== 'function') {
    throw new TypeError('Unity detail density loading requires fetch support.');
  }
  return Promise.all((terrain.detailPrototypes ?? []).map(async (prototype) => {
    if (!prototype?.data) {
      throw new Error(`Unity detail prototype ${prototype?.index ?? '?'} has no density sidecar.`);
    }
    const response = await fetchChecked(fetchFn, joinAssetUrl(baseUrl, prototype.data));
    const densityField = decodeSoStylizedUnityInt32(await response.arrayBuffer());
...
async function sha256Hex(arrayBuffer) {
  if (!globalThis.crypto?.subtle) {
    throw new Error('Native Unity detail parity requires Web Crypto SHA-256 support.');
  }
...
export async function loadSoStylizedUnityMegaNativeDetailTransformSets(
  terrain,
  {
    baseUrl = DEFAULT_SO_STYLIZED_UNITY_MEGA_SCENE_URL,
    fetchFn = globalThis.fetch?.bind(globalThis),
  ...
  validateTerrainRecord(terrain);
  if (typeof fetchFn !== 'function') {
    throw new TypeError('Unity native detail-transform loading requires fetch support.');
  }
  return Promise.all((terrain.detailPrototypes ?? []).map(async (prototype) => {
    const record = prototype?.nativeTransforms;
    if (!record?.data) {
      throw new Error(
        `Unity Mega parity mode requires native transforms for detail `
    ...
    if (actualSha256 !== record.sha256) {
      throw new Error(
        `Unity detail ${prototype.index} native transform SHA-256 drifted `
    ...
    const transformSet = Object.freeze({
      actualSha256,
      record,
      transforms: decodeSoStylizedUnityFloat32(buffer),
    });
...
/**
 * Load and construct the full Unity M_Demonstration_Mega terrain.
 *
 ...
 */
export async function createSoStylizedUnityMegaTerrain({
  baseUrl = DEFAULT_SO_STYLIZED_UNITY_MEGA_SCENE_URL,
  manifest = null,
  ...
  detailPlacementMode = SO_STYLIZED_UNITY_MEGA_DETAIL_PLACEMENT_MODES.nativeExact,
  ...
  if (typeof fetchFn !== 'function') {
    throw new TypeError('createSoStylizedUnityMegaTerrain requires fetch support.');
  }
  if (!Object.values(SO_STYLIZED_UNITY_MEGA_DETAIL_PLACEMENT_MODES)
    .includes(detailPlacementMode)) {
    throw new RangeError(`Unsupported Unity detail placement mode ${detailPlacementMode}.`);
  }
  ...
  if (!rawManifest.terrains?.[terrainIndex]?.position
    ...
    const authority = terrainNativeAuthority
      ?? await loadSoStylizedUnityTerrainNativeAuthority({ baseUrl, fetchFn });
    if (authority) {
      sourceManifest = applySoStylizedUnityTerrainNativeAuthority(rawManifest, authority);
    }
  }
  if (sourceManifest.schema !== SO_STYLIZED_UNITY_MEGA_TERRAIN_CONTRACT.schema
    || sourceManifest.schemaVersion !== SO_STYLIZED_UNITY_MEGA_TERRAIN_CONTRACT.schemaVersion) {
    throw new Error(
      `Unsupported Unity scene manifest ${sourceManifest.schema ?? '<missing>'}`
      + ` v${sourceManifest.schemaVersion ?? '<missing>'}.`,
    );
  }
  const terrain = validateTerrainRecord(sourceManifest.terrains?.[terrainIndex]);
  if (terrain.heightmapResolution !== SO_STYLIZED_UNITY_MEGA_TERRAIN_CONTRACT.heightmapResolution
    || terrain.alphamapWidth !== SO_STYLIZED_UNITY_MEGA_TERRAIN_CONTRACT.alphamapResolution
    || terrain.alphamapHeight !== SO_STYLIZED_UNITY_MEGA_TERRAIN_CONTRACT.alphamapResolution
    || terrain.alphamapLayers !== SO_STYLIZED_UNITY_MEGA_TERRAIN_CONTRACT.alphamapLayers) {
    throw new Error('The selected terrain is not the exported Unity M_Demonstration_Mega terrain.');
  }
  ...
  const [
    ...
    prefabLibrary && detailPopulation
      ? loadSoStylizedUnityMegaDetailDensityFields(terrain, { baseUrl, fetchFn })
      : Promise.resolve(null),
    prefabLibrary
      && detailPopulation
      && detailPlacementMode === SO_STYLIZED_UNITY_MEGA_DETAIL_PLACEMENT_MODES.nativeExact
      ? loadSoStylizedUnityMegaNativeDetailTransformSets(terrain, { baseUrl, fetchFn })
      : Promise.resolve(null),
  ]);
  ...
  const heights = decodeSoStylizedUnityFloat32(heightBuffer);
  ...
  mesh.frustumCulled = false;
  mesh.userData.soStylizedUnityMegaTerrain = {
    terrainIndex,
  ...
  additiveMesh.userData.soStylizedUnityMegaTerrain = {
    ...mesh.userData.soStylizedUnityMegaTerrain,
    addPass: true,
    layerIndices: [4],
    shadowCaster: false,
  ...
  root.userData.soStylizedUnityMegaTerrain = {
    sourceScene: sourceManifest.sourceScene,
    terrainIndex,
    nodeIndex: terrain.node,
    coordinateReflection: 'Unity +Z -> Three -Z',
  };
  ...
  const population = getSoStylizedUnityMegaTerrainPopulation(sourceManifest, terrainIndex);
  ...
  root.userData.soStylizedUnityMegaTerrain.population = {
  ...
  return {
    ...
    holes,
    controlTextures,
    sampleSplatLocal: (x, sourceZ) => sampleSoStylizedUnityMegaTerrainSplat(
      controlTextures,
      terrain.alphamapWidth,
      terrain.alphamapHeight,
      x,
