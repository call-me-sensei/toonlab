...
// used by the supplied So Stylized Unity assets. The Mega scene is emitted by
// UnitySceneExport.cs, while the SnowPines bridge consumes the supplied UE
...
export const SO_STYLIZED_UNITY_NORMAL_INTEGRATION_CONTRACT = freeze({
  decode: freeze({
    channels: 'RG',
    importerGreenTransform: 'multiply decoded Y by -1 when flipGreenChannel=true',
    z: 'sqrt(max(1e-16, 1 - saturate(dot(xy, xy))))',
  }),
  unityMegaScene: freeze({
    geometryCoordinates: '(unity.x, unity.y, -unity.z)',
    tangent: '(unity.x, unity.y, -unity.z, -unity.w)',
    textureFlipY: true,
    triangleWinding: 'swap source triangle indices 1 and 2',
    uv: 'Unity UV copied unchanged by UnitySceneExport.cs',
    zSign: -1,
  }),
  snowPinesUeGltf: freeze({
    geometryRelationToUnityMega: '(-unityMega.x, unityMega.y, -unityMega.z)',
    tangentWRelationToUnityMega: 'same',
    textureFlipY: false,
    uvRelationToUnityMega: '(unityMega.u, 1 - unityMega.v)',
  }),
...
/** Unity UnpackNormalMapRGorAG over the raw source texture sample. */
export function decodeSoStylizedUnityNormalNode(sampleNode, greenSign = 1) {
...
/** Shader Graph Normal Strength node, including its sub-one Z interpolation. */
export function applySoStylizedUnityNormalStrengthNode(input, strength) {
...
/** URP UnpackNormalScale: scale decoded XY while retaining reconstructed Z. */
export function applySoStylizedUnityNormalScaleNode(input, strength) {
  const strengthNode = strength?.isNode ? strength : float(finite(strength, 1));
  return vec3(input.xy.mul(strengthNode), input.z);
}

/** CPU mirror used by deterministic source-integration gates. */
export function decodeSoStylizedUnityNormalSample(sample, {
  ...
  if (!Number.isFinite(x) || !Number.isFinite(rawY)) {
    throw new TypeError('Unity normal sample must contain finite normalized R/G channels.');
  }
  ...
  throw new RangeError(`Unknown Unity normal strength mode: ${strengthMode}`);
}

/** Map a tangent-space normal through an exported glTF tangent basis. */
export function soStylizedUnityTangentNormalToWorld({
  normal,
  tangent,
  tangentNormal,
} = {}) {
...
export function reflectSoStylizedUnityVector(vector, zSign = -1) {
  if ((!Array.isArray(vector) && !ArrayBuffer.isView(vector)) || vector.length < 3) {
    throw new TypeError('Unity vector reflection requires a three-component array.');
  }
  return [
...
export function createSoStylizedUnityNormalIntegrationMetadata({
  coordinateZSign = -1,
