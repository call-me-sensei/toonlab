...
// - Universal Render Pipeline/Lit (the two embedded pine snow-cover records)
//
// Graph topology comes from the supplied Unity 6000.5 Shader Graph assets and
// ShaderUtil-generated URP 17.5 passes. Material values and TextureImporter
...
export const SO_STYLIZED_UNITY_SCENE_SNOW_SHADER = 'Shader Graphs/S_Snow';
export const SO_STYLIZED_UNITY_SCENE_BASIC_SHADER = 'Shader Graphs/S_StylizedBasic';
export const SO_STYLIZED_UNITY_SCENE_URP_LIT_SHADER = 'Universal Render Pipeline/Lit';

export const DEFAULT_SO_STYLIZED_UNITY_SCENE_BASIC_BASE_URL =
...
export const SO_STYLIZED_UNITY_SCENE_SNOW_GRAPH = Object.freeze({
  sourceEngine: 'Unity 6000.5.4f1 / URP 17.5.0',
  sourceGraph: 'Environment/Misc/Shaders/S_Snow.shadergraph',
  ...
  sourceSubgraph: 'Environment/Misc/Shaders/SG_Snow.shadersubgraph',
...
export const SO_STYLIZED_UNITY_SCENE_STYLIZED_BASIC_GRAPH = Object.freeze({
  sourceEngine: 'Unity 6000.5.4f1 / URP 17.5.0',
  sourceGraph: 'Materials/Shaders/S_StylizedBasic.shadergraph',
  ...
  connectedFormula: Object.freeze({
    ...
    hue:
      'RandomRange(SHADERGRAPH_OBJECT_POSITION.xz*10,-_Hue_Variation,+_Hue_Variation)+_Hue_Shift',
    normalTs:
...
export const SO_STYLIZED_UNITY_SCENE_URP_LIT_SOURCE = Object.freeze({
  sourceEngine: 'Unity 6000.5.4f1 / URP 17.5.0',
  shader: 'Packages/com.unity.render-pipelines.universal/Shaders/Lit.shader',
  shaderGuid: '933532a4fcc9baf4fa0491de14d08ed7',
  shaderSha256: 'd012fadd60a3e5c19a57d501a3e010ad0ee6067465fee135a421afea08b4da45',
  litInput: 'Packages/com.unity.render-pipelines.universal/Shaders/LitInput.hlsl',
  litInputSha256: '0a755082971cd052ea2c3595feefb28d5357db6b47ea406f592ce9d9ca40f0a6',
  forwardInclude: 'Packages/com.unity.render-pipelines.universal/Shaders/LitForwardPass.hlsl',
  forwardIncludeSha256: '272b5be87520963b145ecfc690499319cabaf1a2c9f3c97e1c032cd4beef8192',
  shadowInclude: 'Packages/com.unity.render-pipelines.universal/Shaders/ShadowCasterPass.hlsl',
  shadowIncludeSha256: '7db96c7b79c5c5ba306708efd96a74ab7799d13f0426f2c8f2fc34aeabc350e6',
  depthInclude: 'Packages/com.unity.render-pipelines.universal/Shaders/DepthOnlyPass.hlsl',
...
const unitySceneBasicHueNormalized = wgslFn(`
  fn unitySceneBasicHueNormalized(inputColor: vec3<f32>, offset: f32) -> vec3<f32> {
...
function requiredTextureRecord(manifest, textureIndex, materialName) {
  ...
  if (!record?.exactSourceCopy || record?.importer?.present !== true) {
    throw new Error(
      `${materialName} texture ${textureIndex} lacks an exact copy or TextureImporter record.`,
    );
  }
  return record;
}
...
function setOpaqueUrpState(material) {
...
export function evaluateSoStylizedUnitySnowTriplanarWeights(normal) {
  ...
  const safe = Array.from(normal).slice(0, 3).map((channel) => (
    Math.max(Math.abs(finite(channel)), UNITY_FLOAT_EPSILON)
  ));
  const sum = safe[0] + safe[1] + safe[2];
  return Object.freeze(safe.map((channel) => channel / sum));
}

/** CPU mirror of Shader Graph Unity_RandomRange_float. */
export function evaluateSoStylizedUnityBasicRandomRange(seed, minValue, maxValue) {
...
/** CPU mirror of Shader Graph Unity_Hue_Normalized_float. */
export function evaluateSoStylizedUnityBasicHueNormalized(color, offset = 0) {
...
export function resolveSoStylizedUnitySceneSnowInputs(materialRecord) {
  if (materialRecord?.shaderName !== SO_STYLIZED_UNITY_SCENE_SNOW_SHADER) {
    throw new TypeError(`Expected ${SO_STYLIZED_UNITY_SCENE_SNOW_SHADER}.`);
  }
...
export function resolveSoStylizedUnitySceneBasicInputs(materialRecord) {
  if (materialRecord?.shaderName !== SO_STYLIZED_UNITY_SCENE_BASIC_SHADER) {
    throw new TypeError(`Expected ${SO_STYLIZED_UNITY_SCENE_BASIC_SHADER}.`);
  }
  const materialName = materialRecord.name ?? `material-${materialRecord.index ?? 'unknown'}`;
  const properties = indexSoStylizedUnityMaterialProperties(materialRecord);
  return Object.freeze({
...
export function resolveSoStylizedUnitySceneUrpLitInputs(materialRecord) {
  if (materialRecord?.shaderName !== SO_STYLIZED_UNITY_SCENE_URP_LIT_SHADER) {
    throw new TypeError(`Expected ${SO_STYLIZED_UNITY_SCENE_URP_LIT_SHADER}.`);
  }
  const materialName = materialRecord.name ?? `material-${materialRecord.index ?? 'unknown'}`;
  const properties = indexSoStylizedUnityMaterialProperties(materialRecord);
  const values = {
  ...
  if (unsupported.length || activeMaps.length || (materialRecord.keywords ?? []).length) {
    throw new Error(
      `${materialName} no longer matches the audited opaque, map-free URP/Lit records.`,
    );
  }
  return Object.freeze(values);
}

export function isSoStylizedUnitySceneBasicMaterialRecord(materialRecord) {
  return materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_SNOW_SHADER
    || materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_BASIC_SHADER
    || materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_URP_LIT_SHADER;
}

function unitySourceWorldPosition() {
  return vec3(positionWorld.x, positionWorld.y, positionWorld.z.negate());
}

function unitySourceWorldNormal() {
  return vec3(
...
export async function buildSoStylizedUnitySceneSnowMaterial(
  materialRecord,
  manifest,
  {
    baseUrl = DEFAULT_SO_STYLIZED_UNITY_SCENE_BASIC_BASE_URL,
    textureLoader,
  } = {},
) {
  const values = resolveSoStylizedUnitySceneSnowInputs(materialRecord);
  if (!(values.scale > 0)) throw new RangeError(`${values.materialName} snow scale must be positive.`);
  const snowMap = await loadSoStylizedUnitySceneTexture(
    manifest,
    values.textureIndex,
  ...
  const projected = unitySourceWorldPosition().div(values.scale);
  const safeNormal = max(abs(unitySourceWorldNormal()), vec3(UNITY_FLOAT_EPSILON));
  ...
  material.userData.soStylizedUnityMaterial = {
    contract: { ...SO_STYLIZED_UNITY_SCENE_SNOW_GRAPH.connectedFormula },
    exactInputs: true,
    graphExact: true,
    linearColorProperties: { snowTint: [...tintLinear] },
    materialIndex: values.materialIndex,
    reconstruction: 'unity-scene-s-snow',
    sourceMaterial: values.materialName,
    sourceShader: SO_STYLIZED_UNITY_SCENE_SNOW_SHADER,
  };
  material.userData.soStylizedUnitySceneBasic = {
    family: 'snow',
    generatedPasses: SO_STYLIZED_UNITY_SCENE_SNOW_GRAPH.generatedPasses,
    materialGuid: materialRecord.asset?.guid ?? null,
    parameters: values,
    renderState: sourceRenderRecord(materialRecord),
    source: SO_STYLIZED_UNITY_SCENE_SNOW_GRAPH,
  ...
  material.userData.soStylizedUnityNormalIntegration =
    createSoStylizedUnityNormalIntegrationMetadata({
      ...
      family: 'unity-mega-scene-snow',
      textureFlipY: true,
    });
  installOpaquePassesAndLighting(material, SO_STYLIZED_UNITY_SCENE_SNOW_SHADER);
  return material;
}

/** Build the exact S_StylizedBasic graph used by MV_BeachShells. */
export async function buildSoStylizedUnitySceneStylizedBasicMaterial(
  materialRecord,
  manifest,
  {
    baseUrl = DEFAULT_SO_STYLIZED_UNITY_SCENE_BASIC_BASE_URL,
  ...
  const values = resolveSoStylizedUnitySceneBasicInputs(materialRecord);
  ...
  const loadedEntries = await Promise.all(requestedTextures.map(async ([label, index]) => [
    label,
    await loadSoStylizedUnitySceneTexture(
      manifest,
      index,
      { baseUrl, ...(textureLoader ? { textureLoader } : {}) },
    ),
  ...
  const baseColorLinear = linearizeSoStylizedUnityColorProperty(values.baseColor);
  ...
  colorNode = unitySceneBasicHueNormalized(colorNode, hueOffset);
  ...
  if (values.normalMap && hasTangents) {
    flipGreenChannel = maps.normal.userData?.soStylizedUnityTexture?.flipGreenChannel === true;
    const decoded = decodeSoStylizedUnityNormalNode(
      texture(maps.normal).sample(sourceUv).rgb,
      flipGreenChannel ? -1 : 1,
    );
    normalNode = normalize(TBNViewMatrix.mul(
      applySoStylizedUnityNormalStrengthNode(decoded, values.normalStrength),
    ));
  }
  ...
  material.aoNode = float(1);
  material.userData.soStylizedUnityMaterial = {
    contract: { ...SO_STYLIZED_UNITY_SCENE_STYLIZED_BASIC_GRAPH.connectedFormula },
    exactInputs: true,
    graphExact: !values.normalMap || hasTangents,
    linearColorProperties: { baseColor: [...baseColorLinear] },
    materialIndex: values.materialIndex,
    reconstruction: 'unity-scene-s-stylized-basic',
    sourceMaterial: values.materialName,
    sourceShader: SO_STYLIZED_UNITY_SCENE_BASIC_SHADER,
  };
  material.userData.soStylizedUnitySceneBasic = {
    family: 'stylized-basic',
    generatedPasses: SO_STYLIZED_UNITY_SCENE_STYLIZED_BASIC_GRAPH.generatedPasses,
    geometryCapabilities: { hasTangents },
    materialGuid: materialRecord.asset?.guid ?? null,
    parameters: values,
    renderState: sourceRenderRecord(materialRecord),
    source: SO_STYLIZED_UNITY_SCENE_STYLIZED_BASIC_GRAPH,
  ...
  material.userData.soStylizedUnityNormalIntegration =
    createSoStylizedUnityNormalIntegrationMetadata({
      ...
      family: 'unity-mega-scene-stylized-basic',
      flipGreenChannel,
      textureFlipY: true,
    });
  installOpaquePassesAndLighting(material, SO_STYLIZED_UNITY_SCENE_BASIC_SHADER);
  return material;
}

/** Build either supplied map-free, opaque URP/Lit pine snow-cover material. */
export function buildSoStylizedUnitySceneUrpLitMaterial(materialRecord) {
  const values = resolveSoStylizedUnitySceneUrpLitInputs(materialRecord);
  const baseColorLinear = linearizeSoStylizedUnityColorProperty(values.baseColor);
  const material = new MeshPhysicalNodeMaterial();
  material.name = `Unity:${values.materialName}`;
  setOpaqueUrpState(material);
  ...
  material.userData.soStylizedUnityMaterial = {
    contract: { ...SO_STYLIZED_UNITY_SCENE_URP_LIT_SOURCE.connectedFormula },
    exactInputs: true,
    graphExact: true,
    linearColorProperties: { baseColor: [...baseColorLinear] },
    materialIndex: values.materialIndex,
    reconstruction: 'unity-urp-lit-map-free-opaque',
    sourceMaterial: values.materialName,
    sourceShader: SO_STYLIZED_UNITY_SCENE_URP_LIT_SHADER,
  };
  material.userData.soStylizedUnitySceneBasic = {
    activePasses: SO_STYLIZED_UNITY_SCENE_URP_LIT_SOURCE.activePasses,
    family: 'urp-lit',
    materialGuid: materialRecord.asset?.guid ?? null,
    parameters: values,
    renderState: sourceRenderRecord(materialRecord),
    source: SO_STYLIZED_UNITY_SCENE_URP_LIT_SOURCE,
  };
  material.userData.soStylizedUnityNormalIntegration =
    createSoStylizedUnityNormalIntegrationMetadata({
      coordinateZSign: -1,
      decode: 'geometry-only; _NORMALMAP keyword absent selects float3(0,0,1)',
      family: 'unity-mega-scene-urp-lit',
      textureFlipY: null,
    });
  installOpaquePassesAndLighting(material, SO_STYLIZED_UNITY_SCENE_URP_LIT_SHADER);
  return material;
}

/** Dispatch one of the three audited opaque scene families. */
export async function buildSoStylizedUnitySceneBasicMaterial(
  materialRecord,
  manifest,
  options = {},
) {
  if (materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_SNOW_SHADER) {
    return buildSoStylizedUnitySceneSnowMaterial(materialRecord, manifest, options);
  }
  if (materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_BASIC_SHADER) {
    return buildSoStylizedUnitySceneStylizedBasicMaterial(materialRecord, manifest, options);
  }
  if (materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_URP_LIT_SHADER) {
    return buildSoStylizedUnitySceneUrpLitMaterial(materialRecord);
  }
  throw new TypeError(`Unsupported Unity scene basic shader: ${materialRecord?.shaderName ?? 'missing'}.`);
}
