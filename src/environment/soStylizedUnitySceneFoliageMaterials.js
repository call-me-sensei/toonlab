// Manifest-driven reconstruction of Unity's S_FoliageShader family.
//
...
// graph topology and constants come from the supplied Unity 6000.5 Shader
...
import { createSoStylizedUnityNormalIntegrationMetadata } from './soStylizedUnityNormalIntegration.js';
import { installSoStylizedUnityUrpLighting } from './soStylizedUnityUrpLighting.js';

export const SO_STYLIZED_UNITY_SCENE_FOLIAGE_SHADER =
  'Shader Graphs/S_FoliageShader';

export const DEFAULT_SO_STYLIZED_UNITY_SCENE_FOLIAGE_BASE_URL =
  '/assets-local/sostylized-unity/mega-scene';

export const SO_STYLIZED_UNITY_SCENE_FOLIAGE_NOISE_PROPERTY =
...
/**
 * Constants serialized by S_FoliageShader.shadergraph and its two connected
 * subgraphs. Positions are the exact UInt16 Gradient key times divided by
 * 65535, not rounded values copied from an inspector.
 */
export const SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH = Object.freeze({
  sourceGraph: 'Environment/Foliage/Shaders/S_FoliageShader.shadergraph',
  sourceGraphSha256: '1426bd360f44c10510f77a70450c86feca99f132819af6fe78130daabf369dd7',
  cameraDitherGraph: 'Materials/Shaders/SG_CameraDithering.shadersubgraph',
  cameraDitherGraphSha256: '95586ca209f762a059f221bbccab74df9685c4cddc997121596d4578bf1f45dd',
  distanceFadeGraph: 'Materials/Shaders/SG_DistanceFade.shadersubgraph',
  ...
  // UnitySceneExport reflects every object-local Z coordinate when it emits
...
const unitySceneFoliageDither = wgslFn(`
  fn unitySceneFoliageDither(inputValue: f32, pixelPosition: vec2<f32>) -> f32 {
...
// GradientNoiseNode.m_HashType=0 maps to Shader Graph's Tchou hash. This is
// the generated function in GradientNoiseNode.cs with identical quintic
// interpolation and +0.5 output bias.
const unitySceneFoliageGradientNoise = wgslFn(`
  fn unitySceneFoliageGradientNoise(sourceUv: vec2<f32>, scale: f32) -> f32 {
    let p = sourceUv * scale;
    let ip = floor(p);
    var fp = fract(p);
    let d00 = dot(unitySceneFoliageGradientDirection(ip), fp);
    let d01 = dot(unitySceneFoliageGradientDirection(ip + vec2<f32>(0.0, 1.0)), fp - vec2<f32>(0.0, 1.0));
    let d10 = dot(unitySceneFoliageGradientDirection(ip + vec2<f32>(1.0, 0.0)), fp - vec2<f32>(1.0, 0.0));
    let d11 = dot(unitySceneFoliageGradientDirection(ip + vec2<f32>(1.0, 1.0)), fp - vec2<f32>(1.0, 1.0));
    fp = fp * fp * fp * (fp * (fp * 6.0 - 15.0) + 10.0);
    return mix(mix(d00, d01, fp.y), mix(d10, d11, fp.y), fp.x) + 0.5;
  }

  fn unitySceneFoliageGradientDirection(p: vec2<f32>) -> vec2<f32> {
    let x = unitySceneFoliageHashTchou(p);
    return normalize(vec2<f32>(x - floor(x + 0.5), abs(x) - 0.5));
  }

  fn unitySceneFoliageHashTchou(p: vec2<f32>) -> f32 {
...
// HueNode.m_HueMode=1 is Shader Graph's normalized HSV path. These statements
// are a direct WGSL transcription of Unity_Hue_Normalized in HueNode.cs.
const unitySceneFoliageHueNormalized = wgslFn(`
  fn unitySceneFoliageHueNormalized(inputColor: vec3<f32>, offset: f32) -> vec3<f32> {
...
export function resolveSoStylizedUnitySceneFoliageInputs(materialRecord) {
  if (materialRecord?.shaderName !== SO_STYLIZED_UNITY_SCENE_FOLIAGE_SHADER) {
    throw new TypeError(
      `Expected ${SO_STYLIZED_UNITY_SCENE_FOLIAGE_SHADER}; received `
...
export function isSoStylizedUnitySceneFoliageRecord(materialRecord) {
  return materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_FOLIAGE_SHADER;
}

/** CPU mirror of SampleGradientV1 for source/verifier probes. */
export function sampleSoStylizedUnitySceneFoliageGradient(value) {
  const keys = SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.gradient;
...
/**
 * Recover the supplied Unity world's XZ basis from ToonLab's reflected Three
 * basis. The Mega exporter preserves X/Y and negates every Unity Z value, so
 * world/object-position driven texture and random seeds must negate Z once
 * before evaluating the source Shader Graph.
 */
export function evaluateSoStylizedUnitySceneFoliageSourceWorldXZ(position) {
  if ((!Array.isArray(position) && !ArrayBuffer.isView(position))
    || position.length < 3) {
    throw new TypeError('Unity foliage world position must contain three components.');
  }
  const x = Number(position[0]);
  const reflectedZ = Number(position[2]);
  if (!Number.isFinite(x) || !Number.isFinite(reflectedZ)) {
    throw new TypeError('Unity foliage world X/Z must be finite.');
  }
  return Object.freeze([x, -reflectedZ]);
}

/** Reflect one Unity object-space vector into the exported Three mesh basis. */
export function reflectSoStylizedUnitySceneFoliageObjectVector(vector) {
  if ((!Array.isArray(vector) && !ArrayBuffer.isView(vector))
    || vector.length < 3) {
    throw new TypeError('Unity foliage object vector must contain three components.');
  }
  const result = [Number(vector[0]), Number(vector[1]), -Number(vector[2])];
  if (!result.every(Number.isFinite)) {
    throw new TypeError('Unity foliage object vector components must be finite.');
  }
  return Object.freeze(result);
}

function sampleUnityGradientNode(value) {
  const keys = SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.gradient;
...
/**
 * Build one exact S_FoliageShader material from the canonical Unity scene
 * manifest.
 *
 * `geometryHints.objectPositionNode` is required for exact per-instance hue
 * and object-distance fading when the consumer packs many Unity objects into
 ...
 */
export async function buildSoStylizedUnitySceneFoliageMaterial(
  materialRecord,
  manifest,
  {
    baseUrl = DEFAULT_SO_STYLIZED_UNITY_SCENE_FOLIAGE_BASE_URL,
    geometryHints = {},
    state = null,
    textureLoader,
  } = {},
) {
  const values = resolveSoStylizedUnitySceneFoliageInputs(materialRecord);
  ...
  const vertexWeight = geometryHints.vertexColorNode
    ...
    : hasVertexColors
      ...
      : vec3(1);
  const linearColors = {
    bottom: linearizeSoStylizedUnityColorProperty(values.bottomColor),
    specular: linearizeSoStylizedUnityColorProperty(values.specularColor),
    textureTint: linearizeSoStylizedUnityColorProperty(values.textureTint),
    tip: linearizeSoStylizedUnityColorProperty(values.tipColor),
  };
  ...
  if (values.useTexture) {
    ...
    if (!values.useSolidTipColor) {
      ...
      const gradientTip = sampleUnityGradientNode(tipNoise);
      const tipDistance = distanceRamp(
        distance(cameraPosition, fragmentWorldPosition),
        SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.tipDistanceRange[0],
        SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.tipDistanceRange[1],
      );
  ...
  const hueSeed = vec2(
    ...
    .mul(SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.hueObjectPositionScale);
  const random = fract(sin(hueSeed.dot(vec2(
    ...SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.randomDot,
  ))).mul(SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.randomMultiplier));
  const hueOffset = mix(
    float(-values.hueVariation),
    float(values.hueVariation),
    random,
  ).add(values.hueShift);
  colorNode = unitySceneFoliageHueNormalized(colorNode, hueOffset);
  ...
  const cameraDither = unitySceneFoliageDither(
    cameraVisibility.mul(
      SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.cameraDitherMultiplier,
    ),
    screenCoordinate.xy,
  );
  ...
  const distanceDither = unitySceneFoliageDither(
    fragmentDistanceVisibility.mul(
      SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.distanceDitherMultiplier,
    ),
    screenCoordinate.xy,
  );
  ...
  const windNoise = unitySceneFoliageGradientNoise(
    windUv,
  ...
  const positionNode = vertexDistanceVisibility
    .lessThan(SO_STYLIZED_UNITY_SCENE_FOLIAGE_GRAPH.distanceFadeOutThreshold)
  ...
  // URP leaves the interpolated normal unchanged on Cull Off foliage. Avoid
  ...
  installSoStylizedUnityMaterialPassCoupling(material, {
    ...
    positionNode,
    shaderName: SO_STYLIZED_UNITY_SCENE_FOLIAGE_SHADER,
  });
  material.userData.soStylizedUnityMaterial = {
    contract: {
      ...
      reflectedWorldBasis: 'source Unity XZ = ToonLab vec2(world.x,-world.z)',
      lighting: 'URP Universal Lit specular workflow; Specular port is direct F0',
      normal: 'Cull Off; unmodified interpolated geometry normal on back faces',
      sampler: 'canonical manifest TextureImporter state',
    ...
    reconstruction: 'unity-s-foliage-record',
    sourceMaterial: values.materialName,
    sourceShader: SO_STYLIZED_UNITY_SCENE_FOLIAGE_SHADER,
    switches: {
  ...
  material.userData.soStylizedUnityNormalIntegration =
    createSoStylizedUnityNormalIntegrationMetadata({
      ...
      family: 'unity-mega-scene-foliage',
      textureFlipY: true,
    });
