// Source-faithful reconstruction of the three supplied Unity water graphs:
// S_StylizedWater, S_WaterWaves, and S_Waterfall.
//
// This module is a transcription of the supplied Shader Graph JSON,
// ShaderUtil-generated URP 17.5 ForwardLit source, material YAML, and exported
// TextureImporter records. It deliberately does not add refraction, vertex
...
export const SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_SHADER =
  'Shader Graphs/S_StylizedWater';
export const SO_STYLIZED_UNITY_SCENE_WATER_WAVES_SHADER =
  'Shader Graphs/S_WaterWaves';
export const SO_STYLIZED_UNITY_SCENE_WATERFALL_SHADER =
  'Shader Graphs/S_Waterfall';

export const DEFAULT_SO_STYLIZED_UNITY_SCENE_WATER_BASE_URL =
...
// The source graph reads URP's camera depth texture. Three's equivalent copies
...
class SoStylizedUnityViewportDepthTextureNode extends ViewportDepthTextureNode {
  constructor(uvNode = screenUV, levelNode = null, depthTexture = null) {
    ...
    initialDepthTexture.name =
      'Unity Stylized Water adaptive viewport depth';
...
const soStylizedUnityViewportDepthTexture = (uvNode = screenUV) =>
  new SoStylizedUnityViewportDepthTextureNode(uvNode);
...
const freezeSource = ({
  ...
  renderState,
  shader,
  sourceEngine: 'Unity 6000.5.4f1 / URP 17.5.0',
...
export const SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_GRAPH = freezeSource({
  shader: SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_SHADER,
  ...
  disconnected: [
    ...
    'caustic distortion Texture2D slot has no material texture and samples Unity default white',
...
export const SO_STYLIZED_UNITY_SCENE_WATER_WAVES_GRAPH = freezeSource({
  shader: SO_STYLIZED_UNITY_SCENE_WATER_WAVES_SHADER,
...
export const SO_STYLIZED_UNITY_SCENE_WATERFALL_GRAPH = freezeSource({
  shader: SO_STYLIZED_UNITY_SCENE_WATERFALL_SHADER,
...
export const SO_STYLIZED_UNITY_STYLIZED_THRESHOLD_SOURCE = Object.freeze({
...
export const SO_STYLIZED_UNITY_SCENE_WATER_RENDERER_BOUNDARIES = Object.freeze({
  depthCopy:
    'S_StylizedWater actively copies the current Three framebuffer depth; unlike URP _CameraDepthTexture, a transparent surface that wrote depth earlier can be present in that copy.',
  fog:
    'Generated ForwardLit requests URP fog; ToonLab applies the shared Unity fog stage after scene compositing, so overlapping transparent layers do not have identical per-fragment fog/blend ordering.',
  nonForwardPasses:
  ...
  sorting:
    'Unity queues 3000/3001/3002 are retained as metadata; Three transparent sorting is object-depth based, and Waterfall/Waves are material groups on one mesh.',
});
...
function optionalTextureIndex(properties, name, materialName) {
  requiredProperty(properties, name, materialName);
  return readSoStylizedUnityTextureIndex(properties, name);
}
...
export function resolveSoStylizedUnitySceneStylizedWaterInputs(materialRecord) {
  if (materialRecord?.shaderName !== SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_SHADER) {
    throw new TypeError(`Expected ${SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_SHADER}.`);
  }
  const p = indexSoStylizedUnityMaterialProperties(materialRecord);
...
export function resolveSoStylizedUnitySceneWaterWavesInputs(materialRecord) {
  if (materialRecord?.shaderName !== SO_STYLIZED_UNITY_SCENE_WATER_WAVES_SHADER) {
    throw new TypeError(`Expected ${SO_STYLIZED_UNITY_SCENE_WATER_WAVES_SHADER}.`);
  }
  const p = indexSoStylizedUnityMaterialProperties(materialRecord);
...
export function resolveSoStylizedUnitySceneWaterfallInputs(materialRecord) {
  if (materialRecord?.shaderName !== SO_STYLIZED_UNITY_SCENE_WATERFALL_SHADER) {
    throw new TypeError(`Expected ${SO_STYLIZED_UNITY_SCENE_WATERFALL_SHADER}.`);
  }
  const p = indexSoStylizedUnityMaterialProperties(materialRecord);
...
export function isSoStylizedUnitySceneWaterFamilyRecord(materialRecord) {
  return materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_SHADER
    || materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_WATER_WAVES_SHADER
    || materialRecord?.shaderName === SO_STYLIZED_UNITY_SCENE_WATERFALL_SHADER;
}

export function evaluateSoStylizedUnityThreshold(value, {
  enabled = true,
  gradientMin = 0,
  threshold = 0.5,
} = {}) {
...
export function sampleSoStylizedUnityWaterGradientCpu(value) {
  return [
...
export function evaluateSoStylizedUnityWaterWavesCpu({
  mask = 1,
  roughA = 0,
  roughB = 0,
} = {}, values) {
  const foamColorLinear = linearizeSoStylizedUnityColorProperty(values.foamColor);
  const threshold = evaluateSoStylizedUnityThreshold((roughA + roughB) * mask, {
...
export function evaluateSoStylizedUnityWaterfallCpu({
  detailRough = 0,
  edgeBase = 1,
  edgeEmission = 1,
  mainRoughA = 0,
  ...
  const topColorLinear = linearizeSoStylizedUnityColorProperty(values.topColor);
  const bottomColorLinear = linearizeSoStylizedUnityColorProperty(values.bottomColor);
  const foamColorLinear = linearizeSoStylizedUnityColorProperty(values.foamColor);
  const main = evaluateSoStylizedUnityThreshold(mainRoughA + mainRoughB, {
    enabled: values.thresholdRendering >= 0.5,
    gradientMin: values.noiseThresholdGradientMin,
    threshold: values.noiseThreshold,
  }) * values.noiseOpacity;
  const detail = evaluateSoStylizedUnityThreshold(detailRough, {
    enabled: values.thresholdRendering >= 0.5,
...
export function evaluateSoStylizedUnityWaterDepthCpu({
  distanceToCamera = 0,
  fresnel = 0,
  sceneEyeDepth = 0,
  surfaceEyeDepth = 0,
  ...
  const gradient = sampleSoStylizedUnityWaterGradientCpu(gradientTime);
...
export function evaluateSoStylizedUnityWaterFlipbookCpu(baseUv, elapsed, values) {
...
async function loadTextures(manifest, values, names, options) {
  const entries = await Promise.all(Object.entries(names).map(async ([key, index]) => {
    ...
    const loaded = await loadSoStylizedUnitySceneTexture(manifest, index, options);
...
function unityPreserveSpecularTransparentState(material, graph, alphaNode) {
  ...
  material.userData.soStylizedUnityTransparency = {
    ...graph.renderState,
    rendererQueueGap: SO_STYLIZED_UNITY_SCENE_WATER_RENDERER_BOUNDARIES.sorting,
  };
}
...
function atlasNormalSample(normalMap, baseUv, elapsed, values, strength) {
  ...
  const decoded = decodeSoStylizedUnityNormalNode(source, 1);
  return {
    decoded,
    strengthened: applySoStylizedUnityNormalStrengthNode(decoded, strength),
  };
}

function sourceWorldXZ(worldPositionNode) {
  // UnitySceneExport reflects geometry Z. Reconstruct Unity absolute-world XZ
...
async function buildStylizedWater(materialRecord, manifest, options) {
  const values = resolveSoStylizedUnitySceneStylizedWaterInputs(materialRecord);
  const linearColors = Object.freeze({
    shorelineFoamColor: Object.freeze(
      linearizeSoStylizedUnityColorProperty(values.shorelineFoamColor),
    ),
    specularColor: Object.freeze(
      linearizeSoStylizedUnityColorProperty(values.specularColor),
    ),
    specularColorFar: Object.freeze(
      linearizeSoStylizedUnityColorProperty(values.specularColorFar),
    ),
  });
  ...
  const unityWorldXZ = sourceWorldXZ(fragmentWorld);
  ...
  const sceneEyeDepth = linearDepth(soStylizedUnityViewportDepthTexture(screenUV))
  ...
  const shorelineNoise = texture(maps.shorelineNoise)
    .sample(unityWorldXZ.div(values.shorelineFoamNoiseScale)).r;
  ...
  const baseNormalSample = atlasNormalSample(
    maps.normal,
    unityWorldXZ.div(values.waterScale),
    elapsed,
    values,
    values.normalStrength,
  );
  const detailBaseUv = unityWorldXZ.div(values.detailScale)
  ...
  const opacityBase = waterGradient.a
    ...
    .mul(shorelineFade);
  const finalNormalTs = applySoStylizedUnityNormalStrengthNode(
    blendedNormalTs,
    opacityBase,
  );
  ...
  const reflectionNormalTs = applySoStylizedUnityNormalStrengthNode(
    finalNormalTs,
  ...
  const reflectedUnity = sourceViewDirection.reflect(reflectionNormalTs);
  const reflectedRuntime = vec3(
    reflectedUnity.x,
    reflectedUnity.y,
    reflectedUnity.z.negate(),
  );
  ...
  unityPreserveSpecularTransparentState(
    material,
  ...
  material.userData.soStylizedUnityMaterial = {
    ...
    normalIntegration: createSoStylizedUnityNormalIntegrationMetadata({
      coordinateZSign: -1,
      decode: 'RG UnpackNormal + Shader Graph NormalStrength + NormalBlend',
    ...
    rendererBoundaries: SO_STYLIZED_UNITY_SCENE_WATER_RENDERER_BOUNDARIES,
    ...
    sourceGraph: SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_GRAPH,
...
async function buildWaterWaves(materialRecord, manifest, options) {
  const values = resolveSoStylizedUnitySceneWaterWavesInputs(materialRecord);
  const foamColorLinear = Object.freeze(
    linearizeSoStylizedUnityColorProperty(values.foamColor),
  );
  const maps = await loadTextures(manifest, values, {
  ...
  // Unity feeds the original geometric normal to both faces. Override Three's
  ...
  material.userData.soStylizedUnityMaterial = {
    ...
    rendererBoundaries: SO_STYLIZED_UNITY_SCENE_WATER_RENDERER_BOUNDARIES,
    sceneDepth: false,
    sourceGraph: SO_STYLIZED_UNITY_SCENE_WATER_WAVES_GRAPH,
    sourceMaterial: values.materialName,
    sourceShader: materialRecord.shaderName,
    vertexDeformation: false,
  };
...
async function buildWaterfall(materialRecord, manifest, options) {
  const values = resolveSoStylizedUnitySceneWaterfallInputs(materialRecord);
  const linearColors = Object.freeze({
    bottomColor: Object.freeze(
      linearizeSoStylizedUnityColorProperty(values.bottomColor),
    ),
    foamColor: Object.freeze(
      linearizeSoStylizedUnityColorProperty(values.foamColor),
    ),
    topColor: Object.freeze(
      linearizeSoStylizedUnityColorProperty(values.topColor),
    ),
  });
  const maps = await loadTextures(manifest, values, {
  ...
  material.userData.soStylizedUnityMaterial = {
    ...
    sceneDepth: false,
    sourceGraph: SO_STYLIZED_UNITY_SCENE_WATERFALL_GRAPH,
    sourceMaterial: values.materialName,
    sourceShader: materialRecord.shaderName,
...
export async function buildSoStylizedUnitySceneWaterFamilyMaterial(
  materialRecord,
  manifest,
  {
    baseUrl = DEFAULT_SO_STYLIZED_UNITY_SCENE_WATER_BASE_URL,
    geometryHints = null,
    state = null,
    textureLoader = null,
  } = {},
  ...
  throw new TypeError(`Unsupported Unity water shader ${materialRecord?.shaderName ?? 'unknown'}.`);
}

export function createSoStylizedUnitySceneWaterPassReport(manifest) {
  return Object.freeze([
    ...
    const graph = shader === SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_SHADER
      ? SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_GRAPH
      : shader === SO_STYLIZED_UNITY_SCENE_WATER_WAVES_SHADER
        ? SO_STYLIZED_UNITY_SCENE_WATER_WAVES_GRAPH
        : SO_STYLIZED_UNITY_SCENE_WATERFALL_GRAPH;
    return Object.freeze({
      ...
      sceneColor: false,
      sceneDepth: shader === SO_STYLIZED_UNITY_SCENE_STYLIZED_WATER_SHADER,
      shadowCaster: false,
      shader,
      vertexDeformation: false,
    });
