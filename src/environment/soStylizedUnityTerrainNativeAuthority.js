// Native Unity Terrain renderer authority that is intentionally kept outside
...
// Unity Terrain.GetPosition() without rewriting that pinned evidence.

export const SO_STYLIZED_UNITY_TERRAIN_NATIVE_AUTHORITY_FILE =
  'terrain-native-authority.json';

export const SO_STYLIZED_UNITY_TERRAIN_NATIVE_AUTHORITY_CONTRACT = Object.freeze({
...
function validateAuthorityRecord(record, terrain, index) {
  if (!record || record.index !== index || record.node !== terrain.node) {
    throw new Error(`Unity Terrain native authority record ${index} does not match the scene manifest.`);
  }
  if (record.terrainData?.guid !== terrain.terrainData?.guid
    || record.terrainData?.localFileId !== terrain.terrainData?.localFileId) {
    throw new Error(`Unity Terrain native authority record ${index} targets a different TerrainData asset.`);
  }
  if (!Array.isArray(record.position)
    || record.position.length !== 3
    || !record.position.every(Number.isFinite)) {
    throw new Error(`Unity Terrain native authority record ${index} has an invalid position.`);
  }
  if (record.renderTransformAuthority
    !== 'UnityEngine.Terrain.GetPosition(): translation only; rotation and scale ignored') {
    throw new Error(`Unity Terrain native authority record ${index} has an unknown transform contract.`);
  }
  if (!Array.isArray(record.surfaceProbes) || record.surfaceProbes.length !== 81) {
    throw new Error(`Unity Terrain native authority record ${index} must retain the 9x9 probe grid.`);
  }
}
 ...
 */
export function applySoStylizedUnityTerrainNativeAuthority(manifest, authority) {
  if (!manifest || !authority) {
    throw new TypeError('A Unity scene manifest and Terrain native authority sidecar are required.');
  }
  const contract = SO_STYLIZED_UNITY_TERRAIN_NATIVE_AUTHORITY_CONTRACT;
  if (authority.schema !== contract.schema || authority.schemaVersion !== contract.schemaVersion) {
    throw new Error(
      `Unsupported Unity Terrain native authority ${authority.schema ?? '<missing>'}`
      + ` v${authority.schemaVersion ?? '<missing>'}.`,
    );
  }
  if (authority.sourceScene !== manifest.sourceScene) {
    throw new Error('Unity Terrain native authority belongs to a different source scene.');
  }
  if (!Array.isArray(manifest.terrains)
    || !Array.isArray(authority.terrains)
    || authority.terrains.length !== manifest.terrains.length) {
    throw new Error('Unity Terrain native authority inventory differs from the scene manifest.');
  }
  ...
  if (authorityByIndex.size !== authority.terrains.length) {
    throw new Error('Unity Terrain native authority contains duplicate terrain indices.');
  }
  const terrains = manifest.terrains.map((terrain, index) => {
    ...
    for (const [field, value] of [
      ...
      if (terrain[field] !== undefined && !valuesEqual(terrain[field], value)) {
        throw new Error(`Unity Terrain ${field} conflicts with its native authority sidecar.`);
      }
    }
    if (terrain.renderTransformAuthority !== undefined
      && terrain.renderTransformAuthority !== record.renderTransformAuthority) {
      throw new Error('Unity Terrain transform contract conflicts with its native authority sidecar.');
    }
    return {
      ...terrain,
      position: [...record.position],
...
export async function loadSoStylizedUnityTerrainNativeAuthority({
  baseUrl,
  fetchFn = globalThis.fetch?.bind(globalThis),
  required = false,
} = {}) {
  if (typeof fetchFn !== 'function') {
    if (required) throw new TypeError('Unity Terrain native authority loading requires fetch support.');
    return null;
  }
  const url = joinAssetUrl(baseUrl, SO_STYLIZED_UNITY_TERRAIN_NATIVE_AUTHORITY_FILE);
  const response = await fetchFn(url, { cache: 'no-cache' });
  if (!response?.ok) {
    if (!required && response?.status === 404) return null;
    throw new Error(
      `Unity Terrain native authority unavailable (${response?.status ?? 'no response'}): ${url}`,
    );
  }
