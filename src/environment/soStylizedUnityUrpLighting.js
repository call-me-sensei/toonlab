// Unity URP 17.5 Universal Lit lighting bridge.
//
// This ports the active forward-light BRDF convention rather than feeding
// Unity material inputs through Three's GGX/Lambert implementation. The two
// engines expose similarly named PBR inputs, but URP's direct and baked-GI
// diffuse terms do not contain Lambert 1/PI and URP uses its optimized
...
const UNITY_DIELECTRIC_SPECULAR = 0.04;
const UNITY_ONE_MINUS_DIELECTRIC_SPECULAR = 0.96;
// HLSL `half` lower bounds used by InitializeBRDFDataDirect.
const UNITY_HALF_MIN_SQRT = 0.0078125;
const UNITY_HALF_MIN = 0.00006103515625;

export const SO_STYLIZED_UNITY_URP_LIGHTING_SOURCE = Object.freeze({
  captureReport:
    'assets-local/sostylized-unity/mega-scene-native-pc-current/unity-reference.txt',
  captureReportSha256:
  ...
  sceneYaml: 'Assets/SoStylized-Unity/Demo/M_Demonstration_Mega.unity',
  sceneYamlSha256:
...
/**
 * Renderer-boundary conventions accepted by the Unity BRDF.
 *
 ...
 * explicit boundary. Unity Stage removes its deliberate PI pre-scale to
 * recover URP light radiance. The UE source stage divides raw UE radiance by
 * PI because UE Default Lit contains Lambert's 1 / PI while literal URP Lit
 * does not. Without that cross-engine conversion Unity-derived rocks become
 * PI times brighter than the UE materials they replace.
 */
export const SO_STYLIZED_UNITY_URP_INPUT_ADAPTERS = Object.freeze({
  unityStage: Object.freeze({
    directInput:
      'Unity Light radiance pre-multiplied by PI for stock Three Lambert coexistence',
    directNormalization: THREE_LAMBERT_INPUT_SCALE_INVERSE,
    id: 'unity-stage',
    indirectInput:
      'Unity bakedGI pre-multiplied by PI before entering Three irradiance',
    indirectNormalization: THREE_LAMBERT_INPUT_SCALE_INVERSE,
  }),
  ueCapturedSceneSh: Object.freeze({
    directInput:
      'raw UE source-stage analytic-light radiance using UE Lambert energy convention',
    directNormalization: THREE_LAMBERT_INPUT_SCALE_INVERSE,
    id: 'ue-captured-scene-sh',
    indirectInput:
      'Three LightProbe cosine-convolved irradiance from the UE captured-scene SH',
    indirectNormalization: THREE_LAMBERT_INPUT_SCALE_INVERSE,
  }),
});

const DEFAULT_INPUT_ADAPTER = SO_STYLIZED_UNITY_URP_INPUT_ADAPTERS.unityStage;

export function resolveSoStylizedUnityUrpInputAdapter(
  inputAdapter = DEFAULT_INPUT_ADAPTER.id,
) {
  const id = typeof inputAdapter === 'string'
    ? inputAdapter
    : inputAdapter?.id;
  const resolved = Object.values(SO_STYLIZED_UNITY_URP_INPUT_ADAPTERS)
    .find((candidate) => candidate.id === id);
  if (!resolved) {
    throw new RangeError(
      `Unknown Unity URP lighting input adapter "${String(id)}".`,
    );
  }
  return resolved;
}

export const SO_STYLIZED_UNITY_URP_LIGHTING_CONTRACT = Object.freeze({
  source: 'URP 17.5 ShaderLibrary/BRDF.hlsl + Lighting.hlsl',
  directDiffuse: 'radiance * BRDFData.diffuse (no 1/PI)',
  indirectDiffuse: 'bakedGI * BRDFData.diffuse (no 1/PI)',
  directSpecular: 'URP DirectBRDFSpecular optimized GGX',
  ...
  dielectricSpecular: UNITY_DIELECTRIC_SPECULAR,
...
export function evaluateSoStylizedUnityUrpDiffuseDecomposition({
  brdfDiffuse = [1, 1, 1],
  directInput = [0, 0, 0],
  indirectInput = [0, 0, 0],
  inputAdapter = DEFAULT_INPUT_ADAPTER.id,
  ...
  const adapter = resolveSoStylizedUnityUrpInputAdapter(inputAdapter);
...
function unitySpecularReflectivity(color) {
  return max(color.r, max(color.g, color.b));
}

function unityBrdfInputs(workflow, specularF0Node, diffuseAlphaNode = float(1)) {
  if (workflow === 'specular') {
    // Shader Graph's Specular port is already the final Unity F0. Do not use
    ...
    const unitySpecularF0 = vec3(specularF0Node);
    const reflectivity = unitySpecularReflectivity(unitySpecularF0);
    return {
      ...
      // in the Unity BRDF bridge avoids Three premultiplying the final RGB.
      diffuse: diffuseColor.rgb
        .mul(float(1).sub(reflectivity))
        .mul(diffuseAlphaNode),
      specular: unitySpecularF0,
    };
  }
  return {
    diffuse: diffuseColor.rgb
      .mul(float(1).sub(metalness))
      .mul(UNITY_ONE_MINUS_DIELECTRIC_SPECULAR)
      .mul(diffuseAlphaNode),
    specular: mix(
      vec3(UNITY_DIELECTRIC_SPECULAR),
      diffuseColor.rgb,
      metalness,
    ),
  };
...
export class SoStylizedUnityUrpLightingModel extends PhysicalLightingModel {
  constructor({
    ...
    this.inputAdapter = resolveSoStylizedUnityUrpInputAdapter(inputAdapter);
  ...
  direct({ lightDirection, lightColor, reflectedLight }) {
    ...
    // see one consistent URP-domain light input. Unity Stage removes its
    // deliberate PI pre-scale; UE source stage converts UE Lambert energy to
    // URP's no-PI BRDF convention.
    // Three can carry analytic light attenuation as RGBA for transmitted
    // coloured shadows. URP's Light.color is RGB, so consume RGB explicitly.
    const unityLightColor = vec3(lightColor)
    ...
    // URP likewise computes one `radiance` half3 before evaluating the BRDF.
    const radiance = nDotL.mul(unityLightColor).toVar('unityRadiance');
    const brdf = unityBrdfInputs(
      this.workflow,
      this.specularF0Node,
    ...
    // URP consumes Shader Graph's literal `1 - Smoothness`. Three's global
  ...
  // The captured Unity scene has RenderSettings.skybox=null and no custom
  // reflection cubemap. URP still executes GlossyEnvironmentReflection(), but
...
/** Install URP lighting on an existing MeshPhysicalNodeMaterial instance. */
export function installSoStylizedUnityUrpLighting(material, options = {}) {
  if (!material?.isNodeMaterial) return material;
  const prior = material.userData?.soStylizedUnityUrpLighting;
  const inputAdapter = resolveSoStylizedUnityUrpInputAdapter(
  ...
  material.setupLightingModel = () => new SoStylizedUnityUrpLightingModel({
    diffuseAlphaNode,
    inputAdapter,
  ...
  material.userData.soStylizedUnityUrpLighting = {
