// Provenance and identity gate for the native ToonLab Mega A/B.
//
...
export const TOONLAB_MEGA_CURRENT_COMPARISON_EVIDENCE = Object.freeze({
  schema: 'toonlab.mega-comparison-evidence',
  schemaVersion: 2,
  baseUrl: '/assets-local/toonlab/mega-scene-native-pc-current',
...
export function createToonLabMegaComparisonIdentityReport({
  baseUrl = TOONLAB_MEGA_CURRENT_COMPARISON_EVIDENCE.baseUrl,
  frameParityReport,
  manifest,
  ...
  nativeImageWidth,
  nativeReferenceUrl,
  viewportHeight = TOONLAB_MEGA_CURRENT_COMPARISON_EVIDENCE.viewport.height,
  viewportWidth = TOONLAB_MEGA_CURRENT_COMPARISON_EVIDENCE.viewport.width,
} = {}) {
  if (!manifest || !frameParityReport) {
    throw new TypeError('manifest and numerical frameParityReport are required.');
  }
  const evidence = TOONLAB_MEGA_CURRENT_COMPARISON_EVIDENCE;
