// Numerical source-frame verification for the ToonLab Mega comparison.
//
...
import {
  reflectToonLabMegaPosition,
  reflectToonLabMegaQuaternion,
} from './toonLabMegaScene.js';

export const TOONLAB_MEGA_PARITY_VIEWPORT = Object.freeze({
  width: 1920,
  height: 1080,
  aspect: 1920 / 1080,
});
 ...
 */
export function projectToonLabMegaSourcePoint(
  sourceWorldPoint,
  cameraNode,
  cameraRecord,
  { aspect = TOONLAB_MEGA_PARITY_VIEWPORT.aspect } = {},
) {
 ...
 */
export function attachToonLabMegaCameraToRenderScene(
  camera,
  renderScene,
  {
    cameraNode = null,
  ...
  if (cameraNode) {
    const expectedPosition = reflectToonLabMegaPosition(cameraNode.worldPosition);
    const expectedQuaternion = reflectToonLabMegaQuaternion(cameraNode.worldRotation);
...
function compareNodeFrame(sceneRoot, manifest) {
  ...
  for (const node of manifest.nodes ?? []) {
    ...
    const expectedPosition = reflectToonLabMegaPosition(node.worldPosition);
    const expectedQuaternion = reflectToonLabMegaQuaternion(node.worldRotation);
...
function compareTerrainFrame(terrainRuntime, manifest) {
  ...
  if (position?.count === resolution * resolution
    ...
    for (const sourceZ of terrainProbeIndices(resolution)) {
      for (const x of terrainProbeIndices(resolution)) {
        ...
        const expectedWorld = reflectToonLabMegaPosition(
  ...
  for (const index of new Set([
    ...
    const expected = reflectToonLabMegaPosition(sourceWorld.toArray());
  ...
  for (const entry of terrainRuntime.details?.prototypeEntries ?? []) {
    ...
    for (const index of new Set([
      ...
      const sourceLocal = reflectedLocal.clone();
      sourceLocal.z = -sourceLocal.z;
      const expected = reflectToonLabMegaPosition(
 ...
 */
export function createToonLabMegaFrameParityReport({
  camera,
  manifest,
  renderScene,
  sceneRoot,
  terrainRuntime = null,
  width = TOONLAB_MEGA_PARITY_VIEWPORT.width,
  height = TOONLAB_MEGA_PARITY_VIEWPORT.height,
  positionTolerance = DEFAULT_POSITION_TOLERANCE,
  quaternionToleranceRadians = DEFAULT_QUATERNION_TOLERANCE_RADIANS,
  ...
  for (const landmark of collectProjectionLandmarks(manifest, terrainRuntime)) {
    const source = projectToonLabMegaSourcePoint(
      landmark.sourceWorld,
      cameraNode,
      cameraRecord,
      { aspect },
    );
    const reflected = reflectToonLabMegaPosition(landmark.sourceWorld);
