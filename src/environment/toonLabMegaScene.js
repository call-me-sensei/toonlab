// Runtime loader for the ToonLab-authoritative M_Demonstration_Mega export.
//
...
} from './toonLabSceneFoliageMaterials.js';
import {
  buildToonLabSceneTreeMaterial,
  isToonLabSceneTreeMaterialRecord,
} from './toonLabSceneTreeMaterials.js';
import {
...
} from './toonLabSceneBasicMaterials.js';
import {
  buildToonLabSceneWaterFamilyMaterial,
  isToonLabSceneWaterFamilyRecord,
  TOONLAB_SCENE_WATERFALL_SHADER,
} from './toonLabSceneWaterMaterials.js';
import { createToonLabPassCouplingReport } from './toonLabMaterialPassCoupling.js';
import { resolveToonLabShadowCasterPass } from './toonLabShadows.js';
...
export async function buildToonLabMegaMaterial(record, manifest, {
  baseUrl = DEFAULT_TOONLAB_MEGA_BASE_URL,
  geometry = null,
  geometryHints = null,
...
export async function applyToonLabMegaMaterials(root, manifest, options = {}) {
  ...
  const replacements = new Map(await Promise.all([...carriers].map(async ([key, usage]) => [
    key,
    await buildToonLabMegaMaterial(
...
export function applyToonLabMegaRendererState(root, manifest) {
...
export function reflectToonLabMegaPosition(source, target = new THREE.Vector3()) {
...
export function reflectToonLabMegaQuaternion(source, target = new THREE.Quaternion()) {
...
export function applyToonLabMegaCameraRecord(
  camera,
  cameraRecord,
...
export function calculateToonLabMegaLodSelection(
  groupObject,
  groupRecord,
  camera,
  { lodBias = 1 } = {},
  ...
  const localReferencePoint = reflectToonLabMegaPosition(
 ...
 */
export function updateToonLabMegaLods(
  root,
  manifest,
  camera,
  { lodBias = 1 } = {},
  ...
  for (const group of groups) {
    ...
    const selection = calculateToonLabMegaLodSelection(
      groupObject,
      group,
      camera,
      { lodBias },
...
export async function loadToonLabMegaScene({
  baseUrl = DEFAULT_TOONLAB_MEGA_BASE_URL,
  ...
  if (!manifestResponse.ok) {
    throw new Error(`ToonLab Mega manifest unavailable (${manifestResponse.status}).`);
  }
  const rawManifest = await manifestResponse.json();
  if (rawManifest.schema !== 'toonlab.scene-export') {
    throw new Error(`Unsupported ToonLab Mega scene schema: ${rawManifest.schema ?? 'missing'}.`);
  }
  ...
  const rendererState = applyToonLabMegaRendererState(root, manifest);
