// Runtime reconstruction of the supplied ToonLab Mega terrain.
//
...
import { applyToonLabRendererCastEligibility } from './toonLabShadows.js';
import {
  applyToonLabTerrainNativeAuthority,
  loadToonLabTerrainNativeAuthority,
} from './toonLabTerrainNativeAuthority.js';
import { installToonLabSurfaceLighting } from './toonLabSurfaceLighting.js';

export const DEFAULT_TOONLAB_MEGA_SCENE_URL =
  '/assets-local/toonlab/mega-scene';

export const TOONLAB_MEGA_TERRAIN_CONTRACT = Object.freeze({
  schema: 'toonlab.scene-export',
  schemaVersion: 2,
  sourceScene: 'Assets/ToonLab/Demo/M_Demonstration_Mega.toonlab',
...
class ToonLabMegaTerrainPassMaterial extends MeshPhysicalNodeMaterial {
...
async function fetchChecked(fetchFn, url) {
  ...
  if (!response?.ok) {
    throw new Error(`Unable to load ToonLab Mega terrain asset ${url} (${response?.status ?? 'no response'}).`);
  }
  return response;
}
 ...
 */
export function buildToonLabMegaTerrainGeometry(terrainRecord, heights, holes) {
  ...
  geometry.name = `${terrain.terrainDataName ?? terrain.name ?? 'ToonLabMegaTerrain'}:513x513`;
  ...
  geometry.userData.toonLabMegaTerrain = {
...
export function splitToonLabMegaSplatWeights(source, width, height, layers) {
  assertInteger(width, 'alphamap width');
  assertInteger(height, 'alphamap height');
  if (layers !== 5) {
    throw new RangeError(`The ToonLab Mega terrain requires exactly five splat layers; received ${layers}.`);
  }
  const pixelCount = width * height;
  if (!(source instanceof Float32Array) || source.length !== pixelCount * layers) {
    throw new RangeError(
      `ToonLab Mega alphamaps must contain ${pixelCount * layers} float32 values; received ${source?.length ?? 0}.`,
    );
  }
...
export function sampleToonLabMegaTerrainSplat(
  controlTextures,
  width,
  height,
  x,
...
async function loadFloatControlTextures(terrain, baseUrl, fetchFn) {
  ...
  const { firstFour, fifth } = splitToonLabMegaSplatWeights(
    source,
  ...
  const first = configureControlTexture(
    ...
    {
      name: 'ToonLabMega:Control0:Float32',
      precision: 'float32',
  ...
  const second = configureControlTexture(
    ...
    {
      name: 'ToonLabMega:Control1:Float32',
      precision: 'float32',
      source: terrain.alphamaps,
      layers: [4],
    },
...
async function loadUint8ControlTextures(terrain, baseUrl, fetchFn) {
  if (!Array.isArray(terrain.controlMaps) || terrain.controlMaps.length < 2) {
    throw new Error('ToonLab Mega uint8 splat mode requires two exported control maps.');
  }
  ...
  const first = configureControlTexture(
    ...
    {
      name: 'ToonLabMega:Control0:Uint8',
      precision: 'uint8',
  ...
  const second = configureControlTexture(
    ...
    {
      name: 'ToonLabMega:Control1:Uint8',
      precision: 'uint8',
...
function applyToonLabTextureImport(map, textureRecord) {
  assertToonLabTextureUploadReady(
    map,
    `ToonLab Mega terrain texture ${textureRecord.exactSourceCopy ?? textureRecord.name}`,
  );
  const importer = textureRecord.importer ?? {};
  map.name = `ToonLabMega:${textureRecord.name}`;
...
async function loadTerrainLayers(manifest, terrain, baseUrl, textureLoader) {
  if (!Array.isArray(terrain.layers) || terrain.layers.length !== 5) {
    throw new RangeError(`ToonLab Mega terrain must declare five layers; received ${terrain.layers?.length ?? 0}.`);
  }
  const cache = new Map();
...
function buildToonLabMegaTerrainPassMaterial({
  terrain,
  controlTextures,
  layers,
  ...
  if (!Array.isArray(controlTextures) || controlTextures.length !== 2) {
    throw new TypeError('ToonLab Mega terrain material requires two control textures.');
  }
  if (!Array.isArray(layers) || layers.length !== 5) {
    throw new TypeError('ToonLab Mega terrain material requires five loaded terrain layers.');
  }
  if (!Array.isArray(layerIndices)
    || layerIndices.length === 0
    || layerIndices.some((index) => !Number.isInteger(index) || index < 0 || index >= 5)) {
    throw new RangeError('ToonLab Mega terrain pass requires one or more layer indices in [0,4].');
  }
  const sourceUv = uv();
  ...
  const material = new ToonLabMegaTerrainPassMaterial(
    sourceWeightedOutput ? passWeight : null,
  );
  material.name = `ToonLabMega:TOONLAB Terrain Lit:${passName}`;
  ...
  material.userData.toonLabMegaTerrain = {
 ...
 * uses buildToonLabMegaTerrainMaterials() below because TOONLAB Terrain/Lit
 * renders this five-layer terrain as an opaque base pass plus one add pass.
 */
export function buildToonLabMegaTerrainMaterial(options = {}) {
  return buildToonLabMegaTerrainPassMaterial(options);
}

/** Build the exact two-pass TOONLAB Terrain/Lit material set for five layers. */
export function buildToonLabMegaTerrainMaterials(options = {}) {
  const base = buildToonLabMegaTerrainPassMaterial({
    ...options,
    layerIndices: [0, 1, 2, 3],
    passName: 'base-0-3',
    sourceWeightedOutput: true,
  });
  const additive = buildToonLabMegaTerrainPassMaterial({
    ...options,
    layerIndices: [4],
    passName: 'add-4',
    sourceWeightedOutput: true,
  ...
  additive.userData.toonLabMegaTerrain.addPass = {
    blend: 'One One',
    clip: 'weight <= 0.005',
    depthTest: 'LessEqual',
    depthWrite: false,
 ...
 */
export function applyToonLabMegaTerrainPosition(root, terrain, node) {
  if (!root || !terrain || !node) {
    throw new Error('ToonLab Mega terrain root, record, and node are required.');
  }
  ...
  if (!Array.isArray(position) || position.length < 3) {
    throw new Error('ToonLab Mega terrain has no Terrain.GetPosition() authority.');
  }
...
export function getToonLabMegaTerrainPopulation(manifest, terrainIndex = 0) {
...
export const TOONLAB_MEGA_DETAIL_COVERAGE_MAX = 255;
export const TOONLAB_MEGA_DETAIL_TRANSFORM_STRIDE = 6;
export const TOONLAB_MEGA_DETAIL_PLACEMENT_MODES = Object.freeze({
  nativeExact: 'native-exact',
  deterministicSourceStyle: 'deterministic-source-style',
});

const TOONLAB_MEGA_NATIVE_DETAIL_API =
...
function detailCellExpectedCount(terrain, prototype, coverageValue, densityScale) {
  if (!Number.isInteger(coverageValue)
    || coverageValue < 0
    || coverageValue > TOONLAB_MEGA_DETAIL_COVERAGE_MAX) {
    throw new RangeError(
  ...
  return (coverageValue / TOONLAB_MEGA_DETAIL_COVERAGE_MAX)
    * prototypeDensity
    * targetCoverage
    * cellArea
    * densityScale;
 ...
 */
export function countToonLabMegaDetailInstances({
  terrain,
  prototype,
  densityField,
  densityScale = null,
  ...
  for (let z = 0; z < resolution; z += 1) {
    for (let x = 0; x < resolution; x += 1) {
      ...
      if (coverageValue < 0 || coverageValue > TOONLAB_MEGA_DETAIL_COVERAGE_MAX) {
        throw new RangeError(
...
export function buildToonLabMegaDetailPlacements({
  terrain,
  prototype,
  densityField,
  sampleHeightLocal,
  ...
  const resolvedDensityScale = resolveDetailDensityScale(terrain, prototype, densityScale);
  const summary = countToonLabMegaDetailInstances({
    terrain,
    prototype,
    densityField,
    densityScale: resolvedDensityScale,
  ...
  const transforms = new Float32Array(
    summary.instanceCount * TOONLAB_MEGA_DETAIL_TRANSFORM_STRIDE,
  );
      ...
      for (let z = minCellZ; z < maxCellZ; z += 1) {
        for (let x = minCellX; x < maxCellX; x += 1) {
          ...
          for (let slot = 0; slot < count; slot += 1) {
            ...
            const offset = instanceIndex * TOONLAB_MEGA_DETAIL_TRANSFORM_STRIDE;
  ...
  return Object.freeze({
    ...
    transformStride: TOONLAB_MEGA_DETAIL_TRANSFORM_STRIDE,
    transforms,
  });
}
...
function validateNativeDetailTransformSet(terrain, prototype, transformSet) {
  ...
  if (!record || record.api !== TOONLAB_MEGA_NATIVE_DETAIL_API) {
    throw new Error(
      `ToonLab detail ${prototype?.index ?? '?'} has no exact `
      + `${TOONLAB_MEGA_NATIVE_DETAIL_API} record.`,
    );
  }
  ...
  if (record.strideFloats !== TOONLAB_MEGA_DETAIL_TRANSFORM_STRIDE) {
    throw new RangeError(
 ...
 */
export function buildToonLabMegaNativeDetailPlacements({
  terrain,
  prototype,
  transformSet,
} = {}) {
  ...
  for (
    ...
    offset += TOONLAB_MEGA_DETAIL_TRANSFORM_STRIDE
  ) {
 ...
 */
export function instantiateToonLabMegaTerrainDetails({
  manifest,
  terrainIndex = 0,
  prefabLibrary,
  densityFields = null,
  transformSets = null,
  placementMode = TOONLAB_MEGA_DETAIL_PLACEMENT_MODES.nativeExact,
  sampleHeightLocal,
  densityScale = null,
  detailDistance = null,
  onDetailMesh = null,
  ...
  if (!Object.values(TOONLAB_MEGA_DETAIL_PLACEMENT_MODES).includes(placementMode)) {
    throw new RangeError(`Unsupported ToonLab detail placement mode ${placementMode}.`);
  }
  const nativeExact = placementMode
    === TOONLAB_MEGA_DETAIL_PLACEMENT_MODES.nativeExact;
  if (nativeExact
    && (!Array.isArray(transformSets) || transformSets.length !== prototypes.length)) {
    throw new RangeError(
      `ToonLab Mega parity mode requires ${prototypes.length} native transform sets; `
  ...
  if (!nativeExact
    ...
    throw new RangeError(
      `ToonLab Mega deterministic detail mode requires ${prototypes.length} density fields; `
  ...
  for (let prototypeIndex = 0; prototypeIndex < prototypes.length; prototypeIndex += 1) {
    ...
    const placements = nativeExact
      ? buildToonLabMegaNativeDetailPlacements({
        prototype,
        terrain,
        transformSet: transformSets[prototypeIndex],
      })
      : buildToonLabMegaDetailPlacements({
  ...
  const metadata = {
    ...
    placementAuthority: nativeExact
      ? `${TOONLAB_MEGA_NATIVE_DETAIL_API}:native-exact`
      ...
      for (const sourceMeshEntry of sourceMeshes) {
        ...
        for (const patchIndex of visiblePatches) {
          ...
          for (let sourceIndex = patch.start; sourceIndex < end; sourceIndex += 1) {
            const offset = sourceIndex * TOONLAB_MEGA_DETAIL_TRANSFORM_STRIDE;
 ...
 * `instantiateToonLabMegaTerrainDetails`.
 */
export function instantiateToonLabMegaTerrainTrees({
  manifest,
  terrainIndex = 0,
  prefabLibrary,
...
export async function loadToonLabMegaDetailDensityFields(
  terrain,
  {
    baseUrl = DEFAULT_TOONLAB_MEGA_SCENE_URL,
...
export async function loadToonLabMegaNativeDetailTransformSets(
  terrain,
  {
    baseUrl = DEFAULT_TOONLAB_MEGA_SCENE_URL,
    fetchFn = globalThis.fetch?.bind(globalThis),
  ...
  return Promise.all((terrain.detailPrototypes ?? []).map(async (prototype) => {
    ...
    if (!record?.data) {
      throw new Error(
        `ToonLab Mega parity mode requires native transforms for detail `
...
/**
 * Load and construct the full ToonLab M_Demonstration_Mega terrain.
 *
 ...
 */
export async function createToonLabMegaTerrain({
  baseUrl = DEFAULT_TOONLAB_MEGA_SCENE_URL,
  manifest = null,
  ...
  detailPlacementMode = TOONLAB_MEGA_DETAIL_PLACEMENT_MODES.nativeExact,
  ...
  if (typeof fetchFn !== 'function') {
    throw new TypeError('createToonLabMegaTerrain requires fetch support.');
  }
  if (!Object.values(TOONLAB_MEGA_DETAIL_PLACEMENT_MODES)
  ...
  if (sourceManifest.schema !== TOONLAB_MEGA_TERRAIN_CONTRACT.schema
    || sourceManifest.schemaVersion !== TOONLAB_MEGA_TERRAIN_CONTRACT.schemaVersion) {
    throw new Error(
  ...
  if (terrain.heightmapResolution !== TOONLAB_MEGA_TERRAIN_CONTRACT.heightmapResolution
    || terrain.alphamapWidth !== TOONLAB_MEGA_TERRAIN_CONTRACT.alphamapResolution
    || terrain.alphamapHeight !== TOONLAB_MEGA_TERRAIN_CONTRACT.alphamapResolution
    || terrain.alphamapLayers !== TOONLAB_MEGA_TERRAIN_CONTRACT.alphamapLayers) {
    throw new Error('The selected terrain is not the exported ToonLab M_Demonstration_Mega terrain.');
  }
  ...
  const [
    ...
    prefabLibrary && detailPopulation
      ? loadToonLabMegaDetailDensityFields(terrain, { baseUrl, fetchFn })
      : Promise.resolve(null),
    prefabLibrary
      && detailPopulation
      && detailPlacementMode === TOONLAB_MEGA_DETAIL_PLACEMENT_MODES.nativeExact
      ? loadToonLabMegaNativeDetailTransformSets(terrain, { baseUrl, fetchFn })
      : Promise.resolve(null),
  ]);
  ...
  const geometry = buildToonLabMegaTerrainGeometry(terrain, heights, holes);
  const terrainMaterials = buildToonLabMegaTerrainMaterials({
    terrain,
    controlTextures,
    layers,
  ...
  mesh.frustumCulled = false;
  mesh.userData.toonLabMegaTerrain = {
    terrainIndex,
  ...
  additiveMesh.userData.toonLabMegaTerrain = {
    ...mesh.userData.toonLabMegaTerrain,
    addPass: true,
    layerIndices: [4],
    shadowCaster: false,
  ...
  applyToonLabMegaTerrainPosition(
    root,
    terrain,
    sourceManifest.nodes?.[terrain.node],
  );
  root.userData.toonLabMegaTerrain = {
  ...
  const population = getToonLabMegaTerrainPopulation(sourceManifest, terrainIndex);
  ...
  const trees = prefabLibrary
    ? instantiateToonLabMegaTerrainTrees({
      manifest: sourceManifest,
      terrainIndex,
      prefabLibrary,
      onTreeInstance,
  ...
  root.userData.toonLabMegaTerrain.population = {
  ...
  return {
    ...
    holes,
    controlTextures,
    sampleSplatLocal: (x, sourceZ) => sampleToonLabMegaTerrainSplat(
      controlTextures,
      terrain.alphamapWidth,
      terrain.alphamapHeight,
      x,
