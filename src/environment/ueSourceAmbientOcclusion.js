...
const UE_SSAO_REORDER = Object.freeze([
  0, 11, 7, 3,
  10, 4, 15, 12,
  6, 8, 1, 14,
  13, 2, 9, 5,
...
const UE_SSAO_SAMPLE_OFFSETS = Object.freeze([
  Object.freeze([0, -0.43]),
  Object.freeze([0.406, 0.5698]),
  Object.freeze([-0.58, 0.814]),
]);

export const UE_SOURCE_AMBIENT_OCCLUSION_CONTRACT = Object.freeze({
  ...
  runtimeBridge: 'screen-radius Three GTAO horizon sampler with UE noise and response',
  ...
  remainingBridges: Object.freeze([
    'UE classic WedgeWithNormal sampler and HZB mip selection (runtime currently uses Three GTAO horizon integration)',
    'UE half-resolution setup pass, 24-look-up coarse AO, and four-tap normal/depth-aware upsample blend',
    'UE deferred application to material AO, SkyLight/indirect diffuse, and reflection environment instead of multiplying completed scene color',
    'UE PF_B8G8R8A8/PF_FloatRGBA quantization and pixel-exact temporal randomization ordering',
  ]),
});
...
export function computeUeSourceSsaoLevel(settings, scaleToFullRes = 1) {
  ...
  return {
    ...
    sampleLookups: scale > 1
      ...
      : UE_SSAO_SAMPLE_OFFSETS.length * settings.sampleSteps * 2,
    sampleSetSize: scale > 1 ? 6 : UE_SSAO_SAMPLE_OFFSETS.length,
...
/**
 * Resolve the effective SnowPines desktop SSAO state after project defaults,
 * the unbound post-process volume, and Epic post-process scalability.
 */
export function resolveUeSourceAmbientOcclusionSettings(
  postProcessSettings = {},
  {
    cvars = {},
  ...
  const settings = {
    ...
    radiusInWorldSpace,
    radiusScale,
    sampleOffsets: UE_SSAO_SAMPLE_OFFSETS.map((offset) => [...offset]),
  ...
  settings.fullResolution = computeUeSourceSsaoLevel(settings, 1);
  settings.halfResolution = levels >= 2
    ? computeUeSourceSsaoLevel(settings, 2)
    : null;
...
export function evaluateUeSourceAmbientOcclusionResponse(
  rawOcclusion,
  settings = {},
  sceneDepthCm = 0,
) {
  const resolved = settings.fullResolution
    ? settings
    : resolveUeSourceAmbientOcclusionSettings(settings);
...
export function computeUeSourceSsaoRandomizationData(extent = 64) {
  const size = Math.max(4, Math.trunc(finite(extent, 64)));
  const bases = UE_SSAO_REORDER.map((reordered) => {
  ...
  for (let y = 0; y < size; y += 1) {
    for (let x = 0; x < size; x += 1) {
      ...
      // makes its `0.5 + 0.5*a` reconstruction equal UE's basis length.
...
export function createUeSourceSsaoRandomizationTexture() {
  const { data, extent } = computeUeSourceSsaoRandomizationData();
  ...
  result.name = 'UE.SourceSSAO.Randomization';
...
/**
 * WebGPU bridge for UE's view-locked SSAO radius and random basis.
 *
 ...
 * defaults.
 */
export class UeSourceAmbientOcclusionNode extends GTAONode {
  static get type() {
    return 'UeSourceAmbientOcclusionNode';
  }
  ...
  constructor(depthNode, normalNode, camera, settings = {}, {
    ...
    this.settings = settings.fullResolution
      ? settings
      : resolveUeSourceAmbientOcclusionSettings(settings);
    this.contract = UE_SOURCE_AMBIENT_OCCLUSION_CONTRACT;
    this.temporalSampleIndex = nodeObject(temporalSampleIndex ?? uniform(0));
    this._sourceNoiseTexture = createUeSourceSsaoRandomizationTexture();
    this._noiseNode = texture(this._sourceNoiseTexture);
    this.radius.value = this.settings.fullResolution.radiusInShader;
    // Three's exponent controls sample placement; UE AO Power is an output
    ...
    // same full-resolution budget as UE shader-quality 2.
    this.samples.value = 4;
    this.resolutionScale = 1;
    // UE offsets its exact 4x4 random basis using the active 8-sample TAA
...
export function createUeSourceAmbientOcclusionResponseNode(
  rawOcclusionNode,
  sceneViewZNode,
  settings = {},
) {
...
export function ueSourceAmbientOcclusion(
  depthNode,
  normalNode,
  sceneViewZNode,
  camera,
  ...
  const settings = resolveUeSourceAmbientOcclusionSettings(
    postProcessSettings,
    options,
  );
  ...
  node.outputNode = settings.enabled
    ? createUeSourceAmbientOcclusionResponseNode(
