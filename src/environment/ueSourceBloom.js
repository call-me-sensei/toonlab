...
const UE_LEGACY_LUMINANCE = Object.freeze([0.3, 0.59, 0.11]);
...
function reportBloomDebug(stage) {
  if (globalThis.__TOONLAB_UE_BLOOM_DEBUG__ !== true) return;
  console.info(`[UE Source Bloom] ${stage}`);
}

export const UE_SOURCE_STANDARD_BLOOM_STAGES = Object.freeze([
 ...
 * SnowPines post-process volume. Values omitted by the asset export are UE
 * 5.8 FPostProcessSettings defaults, not visual calibration knobs.
 */
export function resolveUeSourceBloomSettings(settings = {}) {
  ...
  const stages = UE_SOURCE_STANDARD_BLOOM_STAGES.map((fallback, index) =>
  ...
  return {
    ...
    gaussianIntensity,
    intensity,
    legacyLuminanceFactors: [...UE_LEGACY_LUMINANCE],
...
export function evaluateUeSourceBloomThreshold(color, settings = {}) {
  const resolved = settings.stageCount === undefined
    ? resolveUeSourceBloomSettings(settings)
    : settings;
 ...
 */
export function computeUeSourceGaussianKernel(kernelRadius, {
  crossCenterWeight = 0,
  filterSizeScale = 1,
...
export function computeUeSourceBloomStageResolutions(width, height) {
...
export function computeUeSourceBloomDcGain(settings = {}) {
  const resolved = settings.stageCount === undefined
    ? resolveUeSourceBloomSettings(settings)
    : settings;
...
/**
 * UE 5.8 desktop Standard/Gaussian bloom for source-renderer comparisons.
 *
 * The pass preserves UE's half-resolution threshold setup, six-level
 ...
 * Three's UnrealBloomPass-derived node because that node has a different
 * high-pass curve, mip count, kernels, weights, and normalization.
 */
export class UeSourceStandardBloomNode extends TempNode {
  static get type() {
    return 'UeSourceStandardBloomNode';
  }

  constructor(inputNode, settings = {}) {
    super('vec4');
    this.inputNode = nodeObject(inputNode);
    this.settings = resolveUeSourceBloomSettings(settings);
    this._sourceTargets = Array.from(
      { length: BLOOM_STAGE_COUNT },
      (_, index) => createBloomRenderTarget(`UE.SourceBloom.source${index + 1}`),
    );
    this._horizontalTargets = Array.from(
      { length: BLOOM_STAGE_COUNT },
      (_, index) => createBloomRenderTarget(`UE.SourceBloom.horizontal${index + 1}`),
    );
    this._verticalTargets = Array.from(
      { length: BLOOM_STAGE_COUNT },
      (_, index) => createBloomRenderTarget(`UE.SourceBloom.vertical${index + 1}`),
    );
    this._halfResolutionSceneTarget = createBloomRenderTarget(
      'UE.SourceBloom.halfResolutionScene',
    );
    ...
    this.contract = Object.freeze({
      ...
      remainingBridges: Object.freeze([
        'UE half arithmetic versus WebGPU f32 shader arithmetic',
        'UE pre-exposure is reduced to the SnowPines fixed-exposure identity',
        'exact edge results can differ where WebGPU emulates UE border sampling',
        'UE fast-blur optimization above the native source resolution is not active in this module',
      ]),
    });
  }
  ...
  setSize(width, height) {
    ...
    this._resolutions = computeUeSourceBloomStageResolutions(
      resolvedWidth,
      resolvedHeight,
    );
    this._kernels = this._resolutions.map((resolution, index) => {
      ...
      const radius = resolution.width
        ...
        * PERCENT_TO_SCALE
        * DIAMETER_TO_RADIUS;
      return computeUeSourceGaussianKernel(radius, this.settings);
    });
  ...
  updateBefore(frame) {
    ...
    try {
      ...
      _quadMesh.name = 'UE Source Bloom [ Scene Downsample / 1:2 ]';
      ...
      _quadMesh.name = 'UE Source Bloom [ Threshold Setup / 1:2 ]';
      _quadMesh.render(renderer);
      ...
      for (let index = 1; index < BLOOM_STAGE_COUNT; index += 1) {
        ...
        _quadMesh.name = `UE Source Bloom [ Downsample ${index + 1} / 6 ]`;
        _quadMesh.render(renderer);
      ...
      for (let index = BLOOM_STAGE_COUNT - 1; index >= firstActiveStage; index -= 1) {
        ...
        _quadMesh.name = `UE Source Bloom [ Gaussian X ${index + 1} / 6 ]`;
        _quadMesh.render(renderer);
        ...
        _quadMesh.name = `UE Source Bloom [ Gaussian Y ${index + 1} / 6 ]`;
        _quadMesh.render(renderer);
      ...
      if (!this._lastErrorReported) {
        console.error('[UE Source Bloom] update failed', error);
  ...
  setup(builder) {
    ...
    this._halfResolutionMaterial.name = 'UE.SourceBloom.HalfResolutionScene';
    ...
    this._highPassMaterial.name = 'UE.SourceBloom.Setup';
    ...
    this._downsampleMaterial.name = 'UE.SourceBloom.DownsampleHigh';
    ...
    const filter = Fn(() => {
      const centerUv = uv();
      const color = vec3(0).toVar('ueBloomFilterColor');
    ...
    this._filterMaterial.name = 'UE.SourceBloom.Gaussian';
...
export function ueSourceStandardBloom(node, settings = {}) {
  return new UeSourceStandardBloomNode(node, settings);
}
