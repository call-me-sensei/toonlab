 ...
 * The frustum vertices are Three view-space metres; UE performs the same math
 * in centimetres, then rounds the final radius up to a whole centimetre.
 */
export function computeUeStableCascadeSphere({
  farVertices,
  nearVertices,
  projectionMatrix,
}) {
  if (!nearVertices?.length || !farVertices?.length || !projectionMatrix?.elements) {
    throw new Error('UE cascade fitting requires near/far vertices and a projection matrix.');
  }
  ...
  // UE clamps to at least one centimetre and then CeilToFloat()s the sphere
...
/** UE snaps in normalized shadow space by 2 * MaxDownsampleFactor / resolution. */
export function computeUeShadowSnapPeriod(
  radiusMeters,
  resolution,
...
/**
 * UE allocates classic Metal CSMs with a four-texel border inside the
 ...
 */
export function computeUeShadowMapLayout(
  physicalResolution,
  borderTexels = 4,
) {
...
/**
 * Resolve UE's raster-cascade fade extension in metres. The final raster
 * cascade is extended too when a distance-field cascade follows it.
 */
export function computeUeCascadeRange({
  splitFar,
  splitNear,
...
/**
 * CSM implementation using UE 5.8's stable-sphere fit, centimetre rounding,
 ...
 * visibility transition, and the uniform part of UE's cascade raster bias.
 *
 * The constant orthographic caster bias is algebraically moved into the
 * receiver comparison. UE's caster-normal slope term still has to run while
 ...
 */
export class UeSourceCsmShadowNode extends CSMShadowNode {
  constructor(light, data = {}) {
    ...
    this.useUeReceiverFilter = data.useUeReceiverFilter !== false;
    this._ueCascadeFits = [];
    this._ueCascadeRanges = [];
    this._ueShadowContracts = [];
    this._ueShadowLayouts = [];
  }
  ...
  _updateShadowBounds() {
    ...
    for (let index = 0; index < this.frustums.length; index += 1) {
      ...
      const range = computeUeCascadeRange({
      ...
      const shadowContract = computeUeDirectionalShadowBiasContract({
      ...
      if (this.useUeReceiverFilter) {
        applyUeDirectionalShadowFilterContract(
          this.lights[index].shadow,
          shadowContract,
        );
      } else {
        // Diagnostic only: retain UE's cascade fit while asking Three to
        ...
        this.lights[index].shadow.ueSourceFilter = 'Three diagnostic';
        if (!this.lights[index].shadow.ueLightDirectionToLight?.isVector3) {
          this.lights[index].shadow.ueLightDirectionToLight = new Vector3(0, 1, 0);
        }
      }

      this._ueCascadeFits[index] = fit;
      this._ueShadowContracts[index] = shadowContract;
      this._ueShadowLayouts[index] = {
        x: layoutX,
        y: layoutY,
      };
      this._ueCascadeRanges[index] = new Vector3(
        range.splitNear,
        range.splitFar,
        extendedFarDepth,
      );
  ...
  _setupUeTransitions() {
    const ranges = reference('_ueCascadeRanges', 'vec3', this)
      .setGroup(renderGroup)
      .setName('ueCascadeRanges');
    const pixelDepth = positionView.z.negate().toVar('uePixelDepthMeters');
    const lastCascade = this.cascades - 1;

    return Fn((builder) => {
      this.setupShadowPosition(builder);
      const result = vec4(1, 1, 1, 1).toVar('ueCsmShadowValue');
  ...
  setup(builder) {
    ...
    return this._setupUeTransitions();
  }
  ...
  updateBefore() {
    ...
    for (let index = 0; index < this.frustums.length; index += 1) {
      const fit = this._ueCascadeFits[index];
      ...
      const snapX = computeUeShadowSnapPeriod(
        fit.radius,
        this._ueShadowLayouts[index].x.interior,
        this.maxDownsampleFactor,
      );
      const snapY = computeUeShadowSnapPeriod(
        fit.radius,
        this._ueShadowLayouts[index].y.interior,
        this.maxDownsampleFactor,
      );
      ...
      // UE's clamped directional subject interval is symmetric around the
      ...
      shadowCamera.updateProjectionMatrix();
      cascadeLight.shadow.ueLightDirectionToLight
