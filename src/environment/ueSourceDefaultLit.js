// UE 5.8 legacy MSM_DEFAULT_LIT lighting bridge for ordinary opaque source
...
const UE_DIELECTRIC_F0_SCALE = 0.08;
const UE_DEFAULT_SPECULAR = 0.5;
const UE_NO_V_EPSILON = 1e-5;
const UE_NUMERIC_EPSILON = 1e-12;

export const UE_SOURCE_DEFAULT_LIT_SOURCE = Object.freeze({
  brdf: 'Engine/Shaders/Private/BRDF.ush',
  brdfSha256:
    '0de81cc25c9b035a77aeb0e2f1be3e730c0f117f9250fe365104f30119b5e906',
  capture: 'assets-local/sostylized/demo-scenes/Demonstration_SnowPines.json',
  captureSha256:
...
/**
 * Exactness boundary for the active SnowPines legacy Default Lit path.
 *
 ...
 * but Three's DFG LUT/PMREM bytes are not UE's PreIntegratedGF/reflection
 * capture buffers, and the active UE SSR/AO composition is renderer-owned.
 */
export const UE_SOURCE_DEFAULT_LIT_CONTRACT = Object.freeze({
  ambientOcclusion:
  ...
  roughDiffuse: false,
  source: 'UE 5.8 legacy MSM_DEFAULT_LIT; Substrate disabled',
  ...
  remainingBridges: Object.freeze([
    'UE PreIntegratedGF texels are not exported; runtime uses Three DFGLUT only for the same split-sum F0/F90 boundary.',
    'Three PMREM filtering/encoding is not UE reflection-capture GatherRadiance filtering/encoding.',
    'SnowPines selects screen-space reflections; UE SSR tracing, hit validation, temporal denoise, and reflection fallback remain renderer work.',
    'UE material/screen AO, GetSpecularOcclusion, bent-normal/distance-field occlusion, and indirect-only composition remain renderer work.',
    'Rect/finite-area lights, anisotropy, and nonzero source-angle/radius energy normalization are outside the zero-size SnowPines light scope.',
  ]),
});
...
/** CPU oracle for UE's legacy GBuffer material conversion. */
export function evaluateUeSourceDefaultLitMaterialInputs({
  baseColor = [1, 1, 1],
  metallic = 0,
  roughness = 0.5,
  specular = UE_DEFAULT_SPECULAR,
} = {}) {
  ...
  const dielectricF0 = UE_DIELECTRIC_F0_SCALE * materialSpecular;
 ...
 */
export function evaluateUeSourceDefaultLitDirect({
  baseColor = [1, 1, 1],
  lightColor = [1, 1, 1],
  ...
  const material = evaluateUeSourceDefaultLitMaterialInputs({
    baseColor,
    metallic,
    roughness,
    specular,
  ...
  if (!(rawNoL > 0)) {
    ...
    return Object.freeze({
      ...
      nDotH: 0,
      nDotL: 0,
      nDotV: saturateNumber(Math.abs(dot3(surfaceNormal, view)) + UE_NO_V_EPSILON),
      specular: black,
      total: black,
      vDotH: 0,
    });
...
export function evaluateUeSourceDefaultLitIndirectDiffuse({
  baseColor = [1, 1, 1],
  irradiance = [0, 0, 0],
  metallic = 0,
  roughness = 0.5,
...
/**
 * CPU oracle for UE's split-sum environment BRDF after the renderer supplies
 * its preintegrated AB sample. This verifies the exact F0/F90 boundary without
 * pretending that Three's DFG LUT texels equal UE's PreIntegratedGF texels.
 */
export function evaluateUeSourceDefaultLitEnvBrdf({
  f0 = [0.04, 0.04, 0.04],
  preintegratedAb = [1, 0],
  radiance = [1, 1, 1],
} = {}) {
...
export function evaluateUeSourceDefaultLitSpecularOcclusion({
  ambientOcclusion = 1,
  nDotV = 1,
  roughness = 0.5,
} = {}) {
...
function ueDefaultLitInputs({
  ...
  const dielectricF0 = materialSpecular.mul(UE_DIELECTRIC_F0_SCALE);
  return {
...
export class UeSourceDefaultLitLightingModel extends PhysicalLightingModel {
  constructor({
    metalnessNode = metalness,
    perceptualRoughnessNode = materialRoughness,
    specularNode = float(UE_DEFAULT_SPECULAR),
  } = {}) {
  ...
  direct({ lightDirection, lightColor, reflectedLight }) {
    const inputs = ueDefaultLitInputs(this);
    ...
    const nDotV = clamp(abs(rawNoV).add(UE_NO_V_EPSILON), 0, 1);
    ...
    const distribution = a2.div(max(d.mul(d).mul(Math.PI), UE_NUMERIC_EPSILON));
  ...
  indirectSpecular(builder) {
    ...
    // The source formula uses UE's PreIntegratedGF. DFGLUT is intentionally a
  ...
  // UE applies material/screen AO in deferred diffuse/reflection composition.
...
/** Install UE legacy Default Lit on an ordinary opaque node material. */
export function installUeSourceDefaultLitLighting(material, options = {}) {
  if (!material?.isNodeMaterial) return material;
  material.setupLightingModel = () => new UeSourceDefaultLitLightingModel({
    metalnessNode:
    ...
    specularNode:
      ...
      ?? float(UE_DEFAULT_SPECULAR),
  });
  material.userData.ueSourceDefaultLitLighting = {
