...
export const UE_DOF_MAX_FOREGROUND_RADIUS = -0.025;
export const UE_DOF_MAX_BACKGROUND_RADIUS = 0.025;
 ...
 * UE works in centimetres internally; the public contract also exposes metres
 * for direct use with the glTF/Three scene.
 */
export function resolveUeSourceDepthOfFieldContract(cameraState = {}, {
  ...
  return {
    ...
    engine: 'Unreal Engine 5.8 DiaphragmDOF',
    fStop,
    fieldOfViewDegrees,
    focusDistanceCm,
    ...
    infinityBackgroundCocRadiusPixels:
      ...
      * Math.min(infinityBackgroundCocRadius, UE_DOF_MAX_BACKGROUND_RADIUS),
    maxBackgroundCocRadius: UE_DOF_MAX_BACKGROUND_RADIUS,
    maxForegroundCocRadius: UE_DOF_MAX_FOREGROUND_RADIUS,
    maxKernelRadiusPixels: Math.max(finite(horizontalResolution, 1280), 1)
      * UE_DOF_MAX_BACKGROUND_RADIUS,
...
/** Exact active UE physical CoC equation, without inactive depth blur/offset. */
export function evaluateUeSourceCocRadius(sceneDepthMeters, contract) {
...
/**
 * Uses UE's exact physical CoC and clamp with Three's existing gather backend.
 ...
 */
export function createUeSourceDepthOfFieldNode(
  inputNode,
  viewZNode,
  contract,
) {
  ...
  // to abs(UE physical CoC / 0.025), including foreground/far classification.
  ...
  node.ueContract = contract;
  node.ueUniforms = {
    focusDistance,
    infinityRadius,
    kernelRadiusPixels,
  };
  node.ueRemainingBridges = Object.freeze([
    'UE DiaphragmDOF gather/scatter classification and dilation',
    'UE energy-conserving authored blade shape and squeeze',
    'UE half-resolution tile flattening and temporal stability',
    'UE foreground hole filling and translucency scheduling',
  ]);
  return node;
}
