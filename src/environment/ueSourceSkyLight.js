...
import { resolveUeSkyLightIntensity } from './ueSourceLighting.js';

const UE_DIFFUSE_CUBEMAP_SIZE = 32;
...
export const UE_SOURCE_SKYLIGHT_CONTRACT = Object.freeze({
  capture: Object.freeze({
    captureEmissiveOnly: 'USkyLightComponent.bCaptureEmissiveOnly',
    diffuseCubemapSize: UE_DIFFUSE_CUBEMAP_SIZE,
  ...
  remainingBridges: Object.freeze([
    'UE does not expose its stored processed SkyLight cubemap or irradiance SH through reflected component metadata; ToonLab recaptures the reconstructed scene.',
    'Three PMREM uses a GGX VNDF convolution instead of UE reflection-capture filtering and encoding.',
  ...
  source: 'UE 5.8 ReflectionEnvironmentCapture + ReflectionEnvironmentDiffuseIrradiance',
  stage: 'partial',
});

const UE_SOURCE_SKYLIGHT_NATIVE_SH_BRIDGES = Object.freeze(
  UE_SOURCE_SKYLIGHT_CONTRACT.remainingBridges.slice(1),
);
...
function unrealPositionMeters(transform = {}) {
 ...
 * radiance, while LightColor * Intensity is applied after convolution in UE.
 */
export function resolveUeSourceSkyLightContract(component = {}, {
  intensityScale = 1,
} = {}) {
  ...
  const diffuseMipLevel = Math.max(
    0,
    Math.round(Math.log2(captureResolution / UE_DIFFUSE_CUBEMAP_SIZE)),
  );
  ...
  return {
    ...
    capturePosition: unrealPositionMeters(component.transform),
    captureResolution,
    ...
    intensity: resolveUeSkyLightIntensity(properties, intensityScale),
...
/** CPU form of Three/UE's three-band cosine-convolved SH evaluation. */
export function evaluateUeSourceSkySh(coefficients, normal) {
...
/** Build Three's raw radiance-SH container from UE coefficients that have
 ...
 */
export function createUeSourceSkyShFromCoefficients(coefficients) {
  if (!Array.isArray(coefficients) || coefficients.length !== 9) {
...
/**
 * Apply UE's linear SkyLight color to a Three SH container.
 *
 ...
 */
export function tintUeSourceSkySh(sh, tint) {
...
/**
 * Three's stock LightProbe node omits UE's nonnegative SH clamp and has no
 ...
 */
export class UeSourceCapturedSkyLightNode extends LightProbeNode {
  static get type() {
    return 'UeSourceCapturedSkyLightNode';
  }

  setup(builder) {
    const source = this.light?.userData?.ueSourceSkyLight;
    if (!source) {
      super.setup(builder);
      return;
    }
...
export function installUeSourceSkyLightNode(renderer) {
  if (!renderer?.library?.lightNodes) {
    throw new Error('A WebGPU renderer node library is required.');
  }
  renderer.library.lightNodes.set(LightProbe, UeSourceCapturedSkyLightNode);
}
...
function restoreSourceCaptureTransforms(root) {
  ...
  root.traverse((object) => {
    const source = object.userData?.ueSourceSkyCapture;
...
function createLowerHemisphere(contract) {
  ...
  mesh.name = 'UE_SkyLightCapture_LowerHemisphere';
...
async function resolveDiffuseSourceMip(renderer, cubeTarget, contract) {
  if (contract.diffuseMipLevel === 0 && contract.captureResolution === UE_DIFFUSE_CUBEMAP_SIZE) {
    return cubeTarget;
  }
  const target = createCubeTarget(UE_DIFFUSE_CUBEMAP_SIZE, { mipmaps: false });
  const material = new MeshBasicNodeMaterial();
 ...
 * the supplied SnowPines component. The complete scene is rendered with its
 * analytic fog node; only SkyLight feedback and post processing are removed.
 */
export async function createUeSourceCapturedSkyLight({
  component,
  ...
  const contract = resolveUeSourceSkyLightContract(component, { intensityScale });
  const nativeSh = diffuseCoefficients == null
    ? null
    : createUeSourceSkyShFromCoefficients(diffuseCoefficients);
  const bridges = nativeSh
    ? [...UE_SOURCE_SKYLIGHT_NATIVE_SH_BRIDGES]
    : [...UE_SOURCE_SKYLIGHT_CONTRACT.remainingBridges];
  ...
  try {
    ...
    const tintedSh = tintUeSourceSkySh(rawSh, captureTint);
    ...
    probe.name = 'UE Source Captured SkyLight';
