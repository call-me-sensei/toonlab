// UE 5.8 Gen4 temporal-AA resolve, camera jitter, and DitherTemporalAA for the
// licensed source renderer. The active SnowPines MainUpsampling / High path is
...
export const DEFAULT_UE_SOURCE_TEMPORAL_DITHER_NOISE_URL =
  '/assets-local/sostylized/ue-temporal-dither/Good64x64TilingNoiseHighFreq.png';

export const UE_SOURCE_TEMPORAL_CONTRACT = Object.freeze({
...
export const UE_SOURCE_TAA_SAMPLE_OFFSETS = Object.freeze([
  Object.freeze([-1, -1]),
  Object.freeze([0, -1]),
  Object.freeze([1, -1]),
  Object.freeze([-1, 0]),
...
/** UE TemporalAA.usf's unnormalized, luma-times-four YCoCg transform. */
export function ueSourceRgbToYCoCg(rgb) {
...
/** Reciprocal of {@link ueSourceRgbToYCoCg}. */
export function ueSourceYCoCgToRgb(yCoCg) {
...
export function computeUeSourceTaaSpatialWeight(
  deltaX,
  deltaY,
  {
...
export function computeUeSourceTaaBlendWeight({
  cameraCut = false,
...
export function evaluateUeSourceTaaCurrentFilter(samplesRgb, jitter, {
  frameExposureScale = 1,
  invFilterScaleFactor = 1,
  upscaleFactor = 1,
} = {}) {
  if (!Array.isArray(samplesRgb) || samplesRgb.length !== 9) {
    throw new Error('UE Gen4 TAA current filter requires nine RGB samples.');
  }
  const dKo = [Number(jitter?.[0]) || 0, Number(jitter?.[1]) || 0];
  const weighted = [0, 0, 0];
  const finalWeights = [];
  let weightSum = 0;
  UE_SOURCE_TAA_SAMPLE_OFFSETS.forEach(([x, y], index) => {
    const transformed = ueSourceRgbToYCoCg(samplesRgb[index]);
    const spatialWeight = computeUeSourceTaaSpatialWeight(
      x - dKo[0],
      y - dKo[1],
  ...
  return {
    dKo,
    filteredRgb: ueSourceYCoCgToRgb(filteredYCoCg),
    filteredTemporalWeight: computeUeSourceTaaSpatialWeight(
      dKo[0],
      dKo[1],
      { invFilterScaleFactor, upscaleFactor },
    ) * upscaleFactor ** 2,
...
export function evaluateUeSourceTaaNeighborhoodBounds(samplesRgb, jitter, {
  upscaleFactor = 1,
} = {}) {
  if (!Array.isArray(samplesRgb) || samplesRgb.length !== 9) {
    throw new Error('UE Gen4 TAA neighborhood requires nine RGB samples.');
  }
  const dKo = [Number(jitter?.[0]) || 0, Number(jitter?.[1]) || 0];
  const transformed = samplesRgb.map(ueSourceRgbToYCoCg);
...
export function computeUeSourceTaaHistoryTaps(uvValue, sizeValue) {
...
export const SO_STYLIZED_DITHER_TEMPORAL_AA_GRAPHS = Object.freeze([
  '/Game/SoStylized/Environment/Foliage/Materials/M_Foliage.M_Foliage',
  ...
  '/Game/SoStylized/Environment/Sky/Materials/M_StylizedClouds.M_StylizedClouds',
  '/Game/SoStylized/Environment/Trees/Materials/M_Leaves.M_Leaves',
  '/Game/SoStylized/Environment/Water/Materials/M_Sandfall.M_Sandfall',
  '/Game/SoStylized/Environment/Water/Materials/M_Waterfall.M_Waterfall',
...
export const SO_STYLIZED_DITHER_TEMPORAL_AA_RUNTIME_BINDINGS = Object.freeze([
  '/Game/SoStylized/Environment/Foliage/Materials/M_Foliage.M_Foliage',
  '/Game/SoStylized/Environment/Rocks/Materials/M_Rock.M_Rock',
  '/Game/SoStylized/Environment/Sky/Materials/M_StylizedClouds_Lite.M_StylizedClouds_Lite',
  '/Game/SoStylized/Environment/Trees/Materials/M_Leaves.M_Leaves',
...
export function ueHalton(index, base) {
...
export function computeUeSourceTaaJitter(sampleIndex, {
  filterSize = UE_SOURCE_TEMPORAL_CONTRACT.filterSize,
  sequenceLength = UE_SOURCE_TEMPORAL_CONTRACT.sequenceLength,
  temporalUpsampling = UE_SOURCE_TEMPORAL_CONTRACT.temporalUpsampling,
} = {}) {
  const index = wrapIndex(sampleIndex, sequenceLength);
  const u1 = ueHalton(index + 1, 2);
  const u2 = ueHalton(index + 1, 3);

  // r.TemporalAA.Upsampling=1 makes the SnowPines view take this exact UE
  // branch, even at the authored 100% screen percentage.
  if (temporalUpsampling) return [u1 - 0.5, u2 - 0.5];

  // Preserve the ordinary perspective-TAA branch for diagnostics. UE windows
...
export function computeUeSourceTemporalDither({
  alpha,
  noise = 0,
  pixelX,
  pixelY,
  random = UE_SOURCE_TEMPORAL_CONTRACT.dither.randomDefault,
  sampleIndex = 0,
} = {}) {
  const x = Math.trunc((Number(pixelX) || 0) + (Number(sampleIndex) || 0));
  const y = Math.trunc((Number(pixelY) || 0) + (Number(sampleIndex) || 0));
  const period = UE_SOURCE_TEMPORAL_CONTRACT.dither.regularPatternPeriod;
  const regular = ((x + 2 * y) % period + period) % period;
  return (Number(alpha) || 0)
    + (regular + (Number(noise) || 0) * (Number(random) || 0))
      * UE_SOURCE_TEMPORAL_CONTRACT.dither.coefficient
    - 0.5;
}

const noiseTexturePromises = new Map();

export async function loadUeSourceTemporalDitherNoiseTexture({
  textureLoader = new THREE.TextureLoader(),
  url = DEFAULT_UE_SOURCE_TEMPORAL_DITHER_NOISE_URL,
} = {}) {
  ...
  if (!noiseTexturePromises.has(key)) {
    noiseTexturePromises.set(key, textureLoader.loadAsync(key).then((result) => {
      ...
      result.anisotropy = 1;
      result.userData.ueSourceTemporalDither = UE_SOURCE_TEMPORAL_CONTRACT.dither;
...
export function ueSourceDitherTemporalAA(
  alphaNode,
  state,
  randomNode = float(UE_SOURCE_TEMPORAL_CONTRACT.dither.randomDefault),
) {
  ...
  const regular = mod(
    ...
    UE_SOURCE_TEMPORAL_CONTRACT.dither.regularPatternPeriod,
  );
  ...
  return float(alphaNode)
    .add(regular.add(noise).mul(UE_SOURCE_TEMPORAL_CONTRACT.dither.coefficient))
    .sub(0.5);
}

export class UeSourceTemporalAANode extends TRAANode {
  constructor(beautyNode, depthNode, velocityNode, camera, {
    ...
    // matching branch in UE Gen4 TAA. setup() replaces that resolve entirely.
    ...
    this.contract = Object.freeze({
      activeHistoryFormat: UE_SOURCE_TEMPORAL_CONTRACT.historyFormat,
      activePermutation: 'MainUpsampling / High / nine samples / YCoCg',
      currentFrameFilter: UE_SOURCE_TEMPORAL_CONTRACT.inputFilter,
      dynamicAntighost:
        'ported from raw Three velocity; exact UE encoded mobility ownership is unavailable',
      historyFilter: UE_SOURCE_TEMPORAL_CONTRACT.historyFilter,
      sourceCurrentFrameWeight: UE_SOURCE_TEMPORAL_CONTRACT.currentFrameWeight,
      historyResolve:
        'UE Gen4 MainUpsampling High resolve; explicit WebGPU boundary gaps remain',
      jitter: 'UE 5.8 exact 8-sample Halton(2,3) temporal-upsample sequence',
      materialDither: 'UE 5.8 DitherTemporalAA exact graph and noise texture',
      method: 'AAM_TemporalAA / MainUpsampling',
      neighborhoodClamp: UE_SOURCE_TEMPORAL_CONTRACT.neighborhoodClamp,
      remainingBridges: Object.freeze([
        'UE responsive-AA stencil classification (no stencil MRT is available)',
        'UE encoded primitive mobility ownership; raw velocity only identifies actual motion',
        'UE stochastic PF_FloatRGBA half quantization and exact half arithmetic',
        'generic pre-exposure changes (identity for frozen SnowPines exposure)',
      ...
      runtimeResolveCurrentFrameWeight: UE_SOURCE_TEMPORAL_CONTRACT.currentFrameWeight,
      sequenceLength: this.sequenceLength,
      workingColorSpace: UE_SOURCE_TEMPORAL_CONTRACT.workingColorSpace,
    });
  ...
  setViewOffset(width, height) {
    ...
    this._writeSampleIndex();
    const jitter = computeUeSourceTaaJitter(this._jitterIndex, {
      sequenceLength: this.sequenceLength,
  ...
  setup(builder) {
    ...
    // copy plumbing, then replace only its resolve graph with UE 5.8's active
    ...
    const sampleHistoryCatmullRom = (inputUv, textureSize) => {
      ...
      const result = vec4(0).toVar('ueTaaCatmullRomHistory');
    ...
    const resolve = Fn(() => {
      ...
      const nearestPixel = floor(ppCo).toVar('ueTaaNearestPixel');
      const nearestTexel = clampTexel(ivec2(nearestPixel));
      const dKo = ppCo.sub(nearestPixel.add(0.5));

      const samples = UE_SOURCE_TAA_SAMPLE_OFFSETS.map(([x, y]) => {
        const texel = clampTexel(nearestTexel.add(ivec2(x, y)));
        return rgbToYCoCg(this.beautyNode.load(texel).rgb);
      });

      // Nine-tap current-frame reconstruction with UE's HDR luma weighting.
      const filteredAccumulator = vec3(0).toVar('ueTaaFilteredAccumulator');
      const filteredWeight = float(0).toVar('ueTaaFilteredWeight');
      UE_SOURCE_TAA_SAMPLE_OFFSETS.forEach(([x, y], index) => {
      ...
      const neighborMinimum = samples[4].toVar('ueTaaNeighborMinimum');
      const neighborMaximum = samples[4].toVar('ueTaaNeighborMaximum');
      UE_SOURCE_TAA_SAMPLE_OFFSETS.forEach(([x, y], index) => {
        if (index === 4) return;
        const delta = vec2(x, y).sub(dKo);
      ...
      // Use UE's diagonal AA_CROSS=1 depth dilation to select a foreground
      // velocity tap. Three stores decoded NDC velocity directly.
      const closestTexel = nearestTexel.toVar('ueTaaClosestVelocityTexel');
      ...
      const clampedHistory = historyBeforeClamp
        ...
        .toVar('ueTaaClampedHistory');

      // UE's encoded velocity distinguishes movable ownership even at zero
      ...
      const anyCurrentDynamic = centerIsDynamic.toVar('ueTaaAnyCurrentDynamic');
      ...
      const blendFinal = mix(
        filteredTemporalWeight.mul(UE_SOURCE_TEMPORAL_CONTRACT.currentFrameWeight),
        0.2,
        motionAmount,
      ).toVar('ueTaaBlendFinal');
      ...
      // PF_FloatRGBA alpha carries UE's current-center mobility bit for the
  ...
  reset(sampleIndex = 0) {
    ...
    // next beauty buffer, matching UE's history invalidation on a camera cut.
    this._forceSourceHistoryReset = true;
...
export function ueSourceTraa(beautyNode, depthNode, velocityNode, camera, options = {}) {
  return new UeSourceTemporalAANode(
