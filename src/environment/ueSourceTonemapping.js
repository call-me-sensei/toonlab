...
export const UE_SOURCE_TONE_MAPPING = 1001;

/**
 * Output-device state used by the supplied macOS SnowPines reference capture.
 *
 ...
 * Unreal therefore writes power-2.2 codes. The screenshot helper only tags
 * those bytes with an sRGB PNG chunk; it does not convert the transfer curve.
 */
export const UE_SOURCE_SNOWPINES_CAPTURE_OUTPUT = Object.freeze({
  displayGamma: 2.2,
...
export function evaluateUeSourceOutputTransfer(
  linearColor,
  output = UE_SOURCE_SNOWPINES_CAPTURE_OUTPUT,
) {
...
// reordering. Keep these in the same row-major order as UE's HLSL sources.
...
export const UE_SOURCE_EXPAND_GAMUT_AP1 = Object.freeze([
...
const EXPAND_GAMUT_AP1 = mat3(
  UE_SOURCE_EXPAND_GAMUT_AP1[0][0],
  UE_SOURCE_EXPAND_GAMUT_AP1[0][1],
  UE_SOURCE_EXPAND_GAMUT_AP1[0][2],
  UE_SOURCE_EXPAND_GAMUT_AP1[1][0],
  UE_SOURCE_EXPAND_GAMUT_AP1[1][1],
  UE_SOURCE_EXPAND_GAMUT_AP1[1][2],
  UE_SOURCE_EXPAND_GAMUT_AP1[2][0],
  UE_SOURCE_EXPAND_GAMUT_AP1[2][1],
  UE_SOURCE_EXPAND_GAMUT_AP1[2][2],
);
...
export function resolveUeSourceFilmSettings(settings = {}) {
...
/**
 * UE 5.8's filmic tonemapper, including ACES glow/red modifiers, the source
 ...
 * stock ACES fit.
 */
export function createUeSourceToneMapping(settings = {}, {
  outputTransfer = null,
} = {}) {
  const film = resolveUeSourceFilmSettings(settings);
  ...
  // zero-luminance select is the analytic limit of UE's expression and avoids
  ...
  // global controls. UE treats FVector4.w as an overall multiplier/addend;
  // notably SnowPines [1.1,1.1,1.1,1.1] therefore means 1.21 saturation.
  ...
  const preToneColor = colorAP1.toVar('uePreToneColor');

  let colorAP0 = AP1_TO_AP0.mul(colorAP1).toVar('ueColorAP0');
  ...
  let centeredHue = hue.toVar('ueCenteredHue');
  ...
  let workingColor = AP0_TO_AP1.mul(colorAP0).max(0).toVar('ueWorkingColor');
  ...
  let toneColor = mix(resolvedToe, resolvedShoulder, curveBlend).toVar('ueToneColor');
...
export const ueSourceToneMapping = createUeSourceToneMapping();
