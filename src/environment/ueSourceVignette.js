...
/**
 * Resolve UE 5.8's default desktop cosine-fourth vignette contract.
 * `aspectRatio` means height / width, matching PostProcessTonemap.usf.
 */
export function resolveUeSourceVignetteSettings(settings = {}) {
...
export function evaluateUeSourceVignetteMask(uv, settings = {}) {
  const resolved = settings.type === undefined || settings.aspectRatio === undefined
    ? resolveUeSourceVignetteSettings(settings)
    : settings;
...
/**
 * Exact active SnowPines vignette node. Apply it to scene color and bloom
 * together before the film curve, as PostProcessTonemap.usf does.
 */
export function createUeSourceVignetteMask(settings = {}) {
  const resolved = resolveUeSourceVignetteSettings(settings);
...
export const UE_SOURCE_VIGNETTE_DEFAULTS = Object.freeze({
