...
export function createToonRockMaterial({
  ...
  if (
    ...
    const sourceProjection = texture(textures.sourceRock).sample(uv()).rgb;
    colorNode = mix(
      colorNode,
      toonLabContrast(
  ...
  if (
    ...
    const sourceSmoothness = texture(textures.sourceRoughness).sample(uv()).g.oneMinus();
    rockSmoothness = mix(
      rockSmoothness,
      sourceSmoothness,
      resolvedAssetIntegration.sourceAlbedoStrength,
